...
· ✅ **Phase 16 — Standards Crosswalk & Readiness/Gap Engine v2 — COMPLETE 2026-08-04, PO-approved.** Branch `phase-16-standards-crosswalk` (not yet merged/pushed). ADR [0093](docs/decisions/0093-phase-16-standards-crosswalk-replan.md) (D1–D10 + Amendments 1–3) · full phase log → [phase-16-standards-crosswalk.md](docs/progress/phase-16-standards-crosswalk.md) · QA [APPROVED](docs/reviews/phase-16-review.md) · as-built → [accreditation-track.md §16](docs/phases/accreditation-track.md).
  · **Shipped:** 5 tables + 15 DEFINER doors + 2 dispatch predicates (10 artifact kinds, evidence freshness, worst-wins hospital rollup with `standard_ownerships`), commission + hospital UI, flag **ON** via Migration G (`20260904000100`). Gate: pgTAP **151 files / 4623** on a fresh reset · **962/962 E2E accounted, 0 assertion failures** · `p0-authz-invariant.sh` **INVARIANT HOLDS** · lint/typecheck/Vitest 901/`next build` green.
  · **6 bugs found + closed** (BUG-P16-001…006). ⚠ **The phase's defining pattern: FOUR separate instances of a full green bar covering code nothing reached** — throwing query stubs, a stale `gen:types`, and two RSC boundary crashes — all invisible because the routes sat behind a flag seeded OFF. **For a flag-gated phase, "it compiles" and "it works" are unrelated claims.** Every one was caught by *executing* (a probe, a real E2E run, a fresh reset), none by review, typecheck or build. → [route-gate-assertions.md](docs/testing/route-gate-assertions.md).
  · **Deliberate deltas from spec:** framework packs are **not** seeded — Migration F shipped **JCI-only** (parallel session); the **ONA skeleton stays parked** on backend's two honest "I don't know" answers about ONA's real citation structure, and its draft CSV is filler to overwrite wholesale, not a base to edit. `level` + per-level cumulative readiness shipped **as specified** and are E2E-proven against self-built leveled fixtures.
  · **Open, non-blocking:** FUP-P16-2 (two accreditation reads live in `actions.ts`, Rule 9 debt from BUG-P16-002) · FUP-P16-4 (10 files outside accreditation carry the pluralization pattern that shipped 2 bugs; every word they pluralize is regular today, so no live defect).
  · ▶ **NEXT: the user-gated pilot deploy** — origin push + Coolify + remote `db push`. ⚠ The remote push needs the **user's own auth**; background agents are auto-denied.
