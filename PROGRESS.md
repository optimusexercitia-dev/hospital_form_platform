...
- **🔓 DB WINDOW CLOSED 2026-09-02 (`authz-ae4-scope-reaches-fix`, off `authz-ae4-catalog`) — the local stack is free; the perf fixture IS loaded (reset before using it).** PO ruling 3's spin-off, built + measured: `20261003007310` + pgTAP `412` + ADR 0180, then **IA-F9 run 5**. ⭐ **ACCEPTANCE STILL NOT MET — a PARTIAL result, recorded as one.** P2/P3/P4/DC1/DC2 **PASS**; **P1 FAIL** (`Seq Scan on hospitals` **8240→4120**, the untargeted half; 603 executed) · **P5 FAIL 5.28×/4.99×** (was 6.19×/6.21×; K=4 **not moved** — 25 % over). ⛔ **The residue is NOT a plan defect:** DC1b 17–18× says `scope_reaches` is still dominant as *volume* — ~170 000 lookups re-resolving the same 20 assignment facts per protected row — which is `entailed_grants`' invocation structure, a **different increment**. Record → acceptance doc **§12**; P1's wording → `FUP-AE4-P1-BOUNDS-A-SYNTAX-NOT-A-PROPERTY` (PO). ⛔ No merge, no `db:push`, no `git push` — the AE4 hold stands.
