...
· 🔵 **Phase 16 — Standards Crosswalk & Readiness/Gap Engine v2 — IN PROGRESS** (branch `phase-16-standards-crosswalk`, started 2026-08-03). Authority: ADR [0093](docs/decisions/0093-phase-16-standards-crosswalk-replan.md) (D1–D10 + **Amendments 1–2**) · plan: [phase-16-standards-crosswalk-program.md](docs/plans/phase-16-standards-crosswalk-program.md) (migrations A–G, pgTAP 278–284, 5 E2E specs). Pre-pilot; **re-gates the pilot deploy**; folds ETH·E3b.
  ...
  · ▶ **Wave 2 IN FLIGHT** — frontend track (rollups/messages/actions → components → routes); backend track part 1 (flag helper → Migration C framework CRUD/clone → Migration D evidence/assessment/candidates, pgTAP 280–281). **Migration E (the three read doors) is deliberately held for its own turn** — highest-risk item in the phase.
...
🔴 **BUG-P16-002 — every Phase 16 screen is dead on arrival: all 7 query-layer functions still `throw new Error('not implemented')`.** Filed 2026-08-03 (lead), **P0, blocks the Phase Gate.** `src/lib/queries/accreditation.ts` was committed in Wave 0 as the *contract* (`de2404c`) with throwing bodies, and **its implementation was never scheduled** — Wave 2's backend brief covered migrations and RPCs only. Meanwhile all five routes wired themselves to it:
...
**Fix (three halves; 1 done, 2 open):**
- ✅ **RPC half** (`20260903001400`, `ee2e52b`): `note_md = coalesce(excluded.note_md, standard_assessments.note_md)` — NULL leaves the note untouched, a real value (**including `''`**) overwrites. pgTAP C7b, mutation-proven (K9). Backend did this against the lead's "no coalesce" instruction and was **right to**: it is a defect in code it shipped that same turn, and clearing survives at the SQL layer via `''`. Data loss is stopped.
- 🔴 **OPEN — the partial fix introduced a silent no-op.** `optionalString` in `src/lib/accreditation/actions.ts:107` is `return value || undefined`, so an **emptied textarea yields `undefined`, never `''`**. Combined with the coalesce, **a user cannot clear an assessment note**: they empty the field, save, and it silently returns. The action must distinguish *field absent* from *field present-but-empty* and send `''` for the latter.
- 🔴 **OPEN — the root cause is still there.** No read path returns a single standard's `note_md`, so the textarea still renders empty when a note exists (and the read-only view still under-reports "Nenhuma observação registrada"). Backend adds a member-gated single-standard assessment read; frontend prefills from it.
⚠ **The standing lesson here: a fix that removes a symptom can hide the defect it was masking.** With the coalesce in, the missing read path no longer destroys data — so the pressure to build it drops, and the new "cannot clear" bug is invisible unless someone explicitly tests clearing. **pgTAP must assert BOTH round-trips**: (a) write a note → save a status-only change → note SURVIVES; (b) write a note → explicitly clear it → note is GONE. Relates to ADR 0093 D8.
