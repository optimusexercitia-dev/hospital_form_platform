...
**3 open bugs** (filed 2026-07-27, FF-1 test pass). All three are application-code defects, not spec
issues — each traced to a specific line via live-catalog/source verification, not inferred.

| ID | Phase | Severity | Description / repro | Expected | Actual | Spec | Owner | Status |
| -- | ----- | -------- | ------------------- | -------- | ------ | ---- | ----- | ------ |
| BUG-FF1-001 | FF-1 | **Blocker** | `addGroupInstance` / `removeGroupInstance` / `reorderGroupInstances` (`src/lib/responses/actions.ts:416-454`) are BE-0's original contract-first stubs — each body is literally `throw new Error('not implemented')`. No later BE commit (BE-3 `8db6c10`, BE-6/7/8/9 `30585d4`) ever filled them in, despite `wizard-runner.tsx:98-106` wiring the wizard's `addInstance`/`removeInstance`/`reorderInstances` directly to them. The underlying DB RPCs (`public.add_group_instance` etc.) are real and correct — confirmed via `pg_proc` and by calling them directly over REST under a respondent's own JWT, which works. Repro: sign in as `staff1.ccih@test.local`, open any form with a `repeating_group`, click "Adicionar repetição" → unhandled server exception, button stays disabled forever (`saving` never clears). | Clicking "Adicionar repetição" / the remove or reorder icon buttons calls the RPC and updates the instance list. | Every click throws `Error: not implemented` server-side; the wizard's entire repeating-group fill UI (add/remove/reorder) is non-functional. | `e2e/ff1-repeating-groups.spec.ts` FF1-3 (full failure), FF1-9 (full failure) — violates ADR 0087 §Gate "E2E: wizard instance add/remove/reorder → resume → submit" and Acceptance #2/#9 in the phase brief | backend | open |
| BUG-FF1-002 | FF-1 | **Blocker** | `parseItemFields` (`src/lib/forms/actions.ts:851-856`, shared by `addItem`/`updateItem`) unconditionally clears `required` whenever `visible_when` is non-null: `const required = parsedVisible.visibleWhen === null && formData.get('required') === 'on'`. This is unchanged pre-FF-1 code (introduced `89989df`, "decision #9" / defence for the OLD `form_items_conditional_not_required` CHECK) that nobody removed when BE-1 dropped that CHECK and FE-4 removed the checkbox's disabled state. The builder UI now *looks* like it allows "obrigatória" beside a condition (checkbox clickable, explanatory copy shown, FE-4's own editor test passes because it only asserts the checkbox isn't disabled) — but the value is silently discarded on save, for BOTH top-level items and repeating-group children. Repro: in the builder, add any input question with a condition, check "Resposta obrigatória", save; `select required from form_items where id=…` reads `false`. | ADR 0087 ruling 4: "se tipo = medicação, nome do medicamento é obrigatório" (a required item, conditionally visible) must be authorable and enforced when visible, at top level and per-instance. | `required` is always `false` for any conditional item regardless of the checkbox; `submit_response` therefore never blocks on it even when the condition is met and the field is left blank. | `e2e/ff1-repeating-groups.spec.ts` FF1-5 (soft-fails on the DB-truth assertion; the rest of the test routes around it via SQL repair and confirms the *completeness authority* itself — `submit_response`/`response_required_complete` — is correct once `required` actually lands, isolating the defect to the authoring layer) — violates ADR 0087 ruling 4 / Acceptance #4 | frontend | open |
| BUG-FF1-003 | FF-1 | **Critical** | The sign-off UI never surfaces repeating-group instance answers. Three independent sites confirm this end-to-end, not just at the type level: `src/components/signoffs/types.ts`'s `ClientResponseForSignoff` has no `instances` field; `src/components/signoffs/adapt.ts`'s `toClientResponseForSignoff` builds its return object from `data` (B2's `ResponseForSignoff`, which per BE-4c *does* carry `.instances`) but never copies it; `review-and-sign.tsx`'s `SectionBody` builds its render list via `section.items.flatMap((it) => it.itemType === "group" ? it.children : [it])` — a `repeating_group` item falls through as `[it]`, `isInputItem('repeating_group')` is false, so it renders via `DisplayBlock`, which returns `null` for anything but `section_text`/`image`. The whole container and every instance's answers are simply absent from the DOM. Live repro captured (see spec): a section containing only a `repeating_group` renders in `/manage/assinaturas/[responseId]` as sign-off chrome only ("Assinatura da chefia", the coordinator's own note field, "Assinar seção") with zero trace of the response's actual content. | A staff_admin reviewing a response with repeating-group answers sees every instance's answers by value before signing (BE-4c's stated purpose: "a sign-off view that silently omits answers passes any test that only asserts what *is* rendered"). | The reviewer sees nothing of the repeating group's content and can sign a section blind to its actual answers. | `e2e/ff1-repeating-groups.spec.ts` FF1-7 (full failure) — violates ADR 0087 Acceptance #7 / BE-4c's own stated purpose | frontend | open |
...
| Date | Phase | Spec(s) | Result | Notes |
| ---- | ----- | ------- | ------ | ----- |
| 2026-07-27 | FF-1 | `e2e/ff1-repeating-groups.spec.ts` (new, 9 tests, `--project=chromium --workers=1`) | 5 passed / 4 failed | FF1-1 (builder authoring + depth cap) ✅ · FF1-2 (ruling 2 condition scoping, incl. publish-time server rejection) ✅ · FF1-3 (wizard add/remove/reorder/resume) ❌ BUG-FF1-001 · FF1-4 (ruling 3 empty-instance pruning, tested at the RPC/server-authority layer) ✅ · FF1-5 (ruling 4 conditional+required) ❌ BUG-FF1-002 (soft-fail; DB completeness authority independently confirmed correct once `required` lands) · FF1-6 (ruling 6 plain-group rendering) ✅ · FF1-7 (sign-off shows instance answers) ❌ BUG-FF1-003 · FF1-8 (dashboard explode + BE-8 denominator fix) ✅ · FF1-9 (keyboard-only instance controls) ❌ blocked by BUG-FF1-001. Not yet run: full `npm run e2e:prod` regression gate (out of scope until the 3 bugs above are fixed — re-running the full suite now would not change the phase-gate verdict). |
