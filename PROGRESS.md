...
- **🔒 DB WINDOW OPEN 2026-09-02 (`authz-ae4-scope-reaches-fix`, off `authz-ae4-catalog`) — do not reset or migrate the local stack.** PO ruling 3's spin-off: the `authz.scope_reaches` ascent fix (`20261003007310`) + pgTAP `412`, then a **fifth** IA-F9 acceptance run against it. Reset + full fixture reload + pass A/B; expect ~1 h. ⛔ No merge, no `db:push`, no `git push` — the AE4 hold stands.
