...
| CH-TEST | `e2e/charters-cadence.spec.ts` (§9) + one keyboard flow. **Carry-forward: exercise a literal `case_restricted`/`participants_only` action item ABSENT from the suggestion end-to-end** (CH-BE-3 pgTAP proved the filter mechanism via an `assignees_only` item; this is the integration-level confidentiality proof). Also prove the two deferred round-trips: charter save (freq + regimento link) + agenda-copy onto a new scheduled meeting (originals untouched). | tester | ⏳ active |
