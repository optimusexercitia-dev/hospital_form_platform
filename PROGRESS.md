...
| FE-4 | Fix **BUG-CORR-001**: stale "reabri-la depois" dialog string (`conclude-narrative-button.tsx`) + remove vestigial `canReopen` prop/guard (`case-narrative-card.tsx`, `case-phase-list.tsx`) | frontend | 🏗️ in progress |
...
| QA | Phase review (neutralization-oracle on new guard arms; `prosecdef` + `pg_policies` sweep) | qa | 🔜 after e2e:prod green |
