...
| 2026-08-14 | **DM5 S3 · TESTER · quick-loop, chromium, `--workers=1`, Gotenberg sidecar up.** ⚠ NOT `e2e:prod` (that gate is lead-owned; a `next build` would collide with the shared `next dev` this run reused). Task 1: re-pointed `pdf-printing-meetings.spec.ts:303-308`'s retired `printed_documents.storage_path` read to `document_version_files`(rendition `printed_pdf`)→`file_objects.storage_bucket`/`storage_path` (D7/D11) — same property (PHI ata → PHI tier), new locator; bucket now CHECK-pinned by `file_objects_bucket_from_tier` rather than merely observed. Task 2: new `D18` test in the same file — the twin (print's own `documents` row exists + is referenced by `printed_documents`, so absence is the filter's doing) + the exclusion (a meeting print never appears in the "Anexos" panel) + the corridor (`%PDF-` bytes still download after both). Task 3 swept `pdf-printing-meetings` (6/6), `pdf-printing` (8/9, 1 environment red), `dm3-wave-b-documents` (11/11), `documents-redesign` (11/11), `dm5-nsp-evidence` (8/8), `dm4-referral-documents` (10/10), `phase17-documents` (13/13) | **67/68 GREEN, 1 environment red (not a regression).** The one failure is `pdf-printing.spec.ts`'s "full lifecycle" test, root-caused to pre-existing fixture-pool contamination from an earlier full run of the SAME unmodified spec (matches this file's own S3 step-2 note: "lead ran... `pdf-printing` 9/9" — that run is what seeded it; no reset ran since) — filed as **BUG-DM5-S3-ENV-FIXTURE-POOL-1** (Bug Log), category (c) environment, no code fix owed, clears on `supabase db reset --local`. **NOT COVERED, named:** a `form_response` print's D18 exclusion has no UI surface to test against — `form_response` is not in `DocumentHomeResourceType`, so no Wave-A Documentos panel is ever rendered for that home (0 importers); the document-DETAIL half of D18 (`documents.ts`'s own `getDocument`, distinct from `controlled-documents.ts`'s same-named export) also has **zero importers anywhere under `src/`** — no route mounts it, so it is unverifiable through the UI today, independent of D18. `case`/`interview` prints stay untested because unmintable (pre-existing, noted in this file's S3 section). |
...
- 🟡 **FUP-DM5-DEAD-CORE-PROJECTION** — `queries/documents.ts`'s `getDocument` has **zero importers**; a **same-named** export in `queries/controlled-documents.ts` is what all five detail routes call. No defect — the reachable one selects `from('controlled_documents')`, so prints are excluded **structurally** — but the lead's D18 detail ruling was implemented on the **unreachable** one and bought nothing (ADR 0120 D18 amendment). Decide delete-vs-keep; **do not strip the filter and keep the function** — frontend/backend
