...
| QA | Phase review — focus: (1) independently neutralization/mutation-verify `getEthicsDashboard` RLS-scoping (respondent/recused/non-granted contribute 0) + O-3 auto-derive visibility (finding/vote `coordinator_only`); (2) **re-audit BE-5/BE-7 keystones for vacuity** (backend self-caught one in `260` — confirm `269` multi-role + the fixed `266` genuinely go RED on revert); `prosecdef` + `pg_policies` sweep on the new write-gate | qa | ⛔ **CHANGES REQUESTED r1** (1 P0 · 0 MAJOR · 0 MINOR · 3 INFO) [review](docs/reviews/phase-E3a-review.md) — **P0-1**: `coordinator_only` case_events leak to non-coordinator **write**-grantees (`case_events_writer_write` FOR-ALL `USING` OR-s into SELECT, defeats the narrowing — ADR-0079); crux (dashboard RLS-scoping) + keystone-vacuity re-audit both **PASS/non-vacuous** |
| BE-P0 | **Fix P0-1** — close `case_events_writer_write`'s un-narrowed SELECT read-arm (reader-non-writer split → make `case_events_select` the sole SELECT authority; preserve write authority + the WITH CHECK visibility gate) so a non-coordinator write-grantee sees **0** `coordinator_only` events (still sees `case_readers`); add a **mutation-proven** write-grantee SELECT keystone. | backend | ✅ `a64e61a` — both FOR-ALL write policies split into command-specific INSERT/UPDATE/DELETE (SELECT = `case_events_select` only); keystones `267` #14-17 mutation-proven (recreate leaky arm → write-grantee sees `coordinator_only` → RED); `can_write ⊆ can_read` confirmed (#15 no read regression); also closed the latent respondent-staff_admin bypass; full suite **3852** PASS |
| QA-r2 | Re-confirm P0-1 closed (write-grantee live-leak → 0; keystone RED-on-revert; **write-authority not regressed by the policy split**) + full pgTAP green → APPROVED | qa | 🔍 in progress |
