...
| BE-3 | INVOKER RPCs `add_/remove_/reorder_group_instances` — atomic `max(position)+1`, collision-free reorder against the non-deferrable unique, `max_instances` (ruling 5) | backend | ✅ `20260828000200` — 3 INVOKER RPCs (HC0N0–HC0N4), flag-guarded; deferred-unique reorder; **+ Amendment 1.3**: `start_correction_draft`/`supersede_response` now copy the instance ROWS and remap the answers |
