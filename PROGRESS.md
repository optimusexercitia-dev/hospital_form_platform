...
| BE-3b | **NEW (D13):** `can_access_targeted_response` + `responses`/`answers` OR-arms + `target_case_response`/`submit_targeted_case_response` `HC0J9` | backend | **full** ✅ approved | 🏗️ building (enumerate live `responses`/`answers` policies first; over-grant twin) |
