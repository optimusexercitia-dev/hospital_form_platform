...
**1 open bug, 3 closed** (filed 2026-07-27, FF-1 test pass; re-verified same day). All four are
...
| BUG-FF1-004 | FF-1 | **Blocker** | Found during BUG-FF1-001/002/003 re-verification, not in the original pass (it was masked by BUG-FF1-001 — the wizard couldn't add a second instance at all until that landed). Every control id/name in `src/components/responses/wizard/input-item.tsx` is built purely from the QUESTION's static `item.id`, with **no instance scoping**: `useFieldIds(\`item-${item.id}\`, ...)` for text/number/date/time (used via `<FieldLabel htmlFor={controlProps.id}>`), and for choice options, `const id = \`item-${item.id}-opt-${i}\`` **plus** `name={\`item-${item.id}\`}` on each `<input type="radio">` (lines ~627/641, mirrored at ~723 for checkbox). Inside a `repeating_group`, the SAME question renders once per instance, so every instance mints the identical `id`/`name`. Two independent, live-verified consequences: **(a) accessibility** — `document.getElementById`/label-`for` resolution always returns the FIRST DOM match, so every instance after the first is unreachable by its accessible name (confirmed: `page.locator('#'+id).count()` = 2 for a 2-instance group; `getByRole('radio', { name: 'Tarde' })` scoped to instance 2 finds nothing). **(b) functional cross-instance corruption, choice types only** — the duplicated `name` attribute puts EVERY instance's radios for that question into the SAME native browser radio-exclusivity group (not scoped per instance), so checking instance 2's "Tarde" fights the browser's own native single-selection enforcement against instance 1's "Manhã": live-verified, `inst2`'s radio does not end up checked after the click (`Expected: checked, Received: unchecked`). Text/number/date/time fields (no `name` grouping) only hit (a), not (b). | Every instance's inputs are independently labelled and independently interactable, matching the ADR's per-instance isolation model. | Instance 2+ inputs are unlabelled to assistive tech; for `multiple_choice`/`checkbox` children specifically, browser-native radio-group exclusivity can silently corrupt a DIFFERENT instance's selection. | `e2e/ff1-repeating-groups.spec.ts` FF1-3 (full failure — the duplicate-id count, the unreachable-by-name check, and the checked-state corruption are all asserted explicitly), FF1-9 (soft-fail on the same duplicate-id/label check; its own add/reorder/remove/type flow still passes since it only exercises a text field, which doesn't hit the `name`-grouping half) — violates ADR 0087's per-instance isolation model (rulings 2/3, the "same-instance sibling" overlay-map design) and Acceptance #2 | frontend | open |
