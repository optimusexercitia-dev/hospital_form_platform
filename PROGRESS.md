...
- [ ] **Remote `db push`** of `20260910000100/200/300/400` — needs the human's authorization,
      sequenced with the AFF push `main` already owes. ⚠ Until pushed, deployed `main` breaks at
      runtime on the meetings list (`listMeetings` now reads `meeting_minutes_jobs`) — same class
      as the standing AFF gap.
- [ ] **Cloud storage upload cap** — plan-dependent, runbook §2 blank; must admit 500 MB.
- [ ] **T5 manual smoke** — real local `minute_generator` end-to-end (upload → callback → review
      → apply), per plan §T5.
- [ ] **QA r2 residuals R1–R3** — R1 aria-label names the resolution not the card · R2 the
      ≥8-tests click anomaly (verify on different hardware) · R3 consider reverting `server-only`
      on `hmac.ts` (QA endorses; it forced an inlined `signCallbackBody` copy in the E2E helper).
- [ ] Env vars on the deploy target: `MINUTES_SERVICE_URL/_API_KEY`, `MINUTES_CALLBACK_HMAC_SECRET`,
      `MINUTES_CALLBACK_BASE_URL` (runbook §3).
