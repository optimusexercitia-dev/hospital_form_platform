...
· 🔵 **Phase 16 — Standards Crosswalk & Readiness/Gap Engine v2 — IN PROGRESS** (branch `phase-16-standards-crosswalk`, started 2026-08-03). Authority: ADR [0093](docs/decisions/0093-phase-16-standards-crosswalk-replan.md) (D1–D10 + **Amendments 1–2**) · plan: [phase-16-standards-crosswalk-program.md](docs/plans/phase-16-standards-crosswalk-program.md) (migrations A–G, pgTAP 278–284, 5 E2E specs). Pre-pilot; **re-gates the pilot deploy**; folds ETH·E3b.
  ...
  · 🟡 **BUG-P16-005 open (minor, pt-BR)** — readiness dashboard rendered "padrãoes". A correct fix was found sitting **uncommitted and unclaimed** in the working tree (neither teammate claimed authorship); routed to frontend to own, complete and commit, along with a sweep for the same string-concatenation pluralization pattern and a judgement call on whether `conforme{s}` should agree with `padrões`. ⚠ The tester deliberately wrote its assertions with a **tolerant regex** so they would not encode the wrong spelling — correct instinct, but it means **no test will red if this regresses**; tighten once fixed.
