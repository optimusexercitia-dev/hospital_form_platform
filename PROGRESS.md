...
| 2026-08-18 | ⛔ **DM-FUP TRIAGE #8 IS WITHDRAWN THE SAME DAY — it reverses a ratified decision, and NOTHING WAS BUILT.** ADR [0104](docs/decisions/0104-pdf-document-printing-module.md) **D7** ratifies the `RASCUNHO` watermark whose derivation is *"Source not in final state at mint (**in_progress response**, …)"*, and its item 4 states outright: ***"Completeness does not gate minting — FINAL/RASCUNHO already encodes it."*** Draft prints are a **designed feature**. pgTAP `312` t6 pins it **by name** — *"creator sees his own in_progress draft (RASCUNHO prints are legal, D7)"* — so the ruling would have red-ed an existing keystone. ⭐ **The failure is upstream of the ruling: `FUP-DM5-DANGLING-PRINT`'s own option list proposed refusing the mint as *"narrowest, and arguably right — a draft is not a document of record"*, a justification an ADR had already refuted.** The lead repeated the item's framing instead of checking it, and the PO ruled on the lead's recommendation. ⚠ *An option list is an assertion too, and this one was authored inside the item it belongs to — the place least likely to be re-derived.* → [[verify-dont-comply]] · [[a-comment-is-an-assertion-that-goes-stale-silently]]. **Caught before implementation, by reading D7 while looking for a fixture. RE-RULE OWED** | ADR [0104](docs/decisions/0104-pdf-document-printing-module.md) D7 · FUP-DM5-DANGLING-PRINT-ON-DELETED-DRAFT |
...
| git pushed? | ✅ **YES, AND IN SYNC — re-measured 2026-08-18 by `git fetch` AFTER pushing**: `origin/main` = **`fc83fa0f`** = HEAD, **0 ahead / 0 behind**, tree clean. The three commits are `phase(DM5): complete` (`c0a0a9f4`), the pilot-gate correction (`0147d60c`) and `chore(graphify)` (`fc83fa0f`). ⭐ **This row has now been wrong twice and was rewritten twice in two days** — first `23b1d9cf`/"33 ahead", then `cd6b9b43`/"4 ahead", each true when written. *It is the fastest-staling fact in this file, and it is the one a session reads to decide whether the remote is safe to touch — **re-measure it, never quote it***. ⛔ **A git push is still NOT a `db push`** — see the row below | `git fetch && git rev-list --left-right --count origin/main...HEAD` |
...
- ⬛ **FUP-DM5-BYTE-PROOF-NOT-ATTEMPTED** — ✅ **RESOLVED 2026-08-18** (`src/lib/documents/actions.ts`): the one lane that deletes bytes now declares `'unavailable_on_platform'` instead of riding the DEFAULT. ⭐ **The pin was PROVEN ABLE TO FAIL** — the fix was reverted, the assertion red-ed on exactly `p_byte_proof`, then restored; it also forbids `local_volume_verified` (an overclaim — nothing in this lane walks the volume). ⚠ **Revisit if the Cloud probe finds a real orphan surface**: a fourth vocabulary value must be *earned by that measurement*, not pre-authorized. Original: `p_byte_proof` DEFAULTs to `'not_attempted'` and its **only** production caller (`reclassifyDocument`) omits it — **three lines after successfully deleting the bytes**. The one lane that can honestly claim a byte proof is the one that disclaims it, in the ADR 0121 D4 evidence a regulator reads. Errs conservatively, so no gate catches it. Nothing pins what any lane writes. ✅ **RULED 2026-08-18 (DM-FUP TRIAGE #2): write `unavailable_on_platform`** — the deciding fact is that **no app code branches on deployment**, so one literal serves both environments; an env-derived value and a call-time `list()` "verification" were both **explicitly rejected**, the latter because it manufactures a proof out of the metadata proxy — backend
- ⬛ **FUP-DM5-ATTACHMENTS-MODULE-SURVIVED-RETIREMENT** — ✅ **RESOLVED 2026-08-18**: `src/lib/attachments/actions.ts` **DELETED** (6 dead exports, zero importers) and the stale `openAttachment` reference in `queries/case-documents.ts` repointed to the live corridor. ⚠ **`constants.ts` RETAINED — it is live** (3 importers); deleting the directory on the item's title would have broken three query modules. ⭐ *The item's title named a directory; the measurement named a file — and they were not the same unit.* Original: `src/lib/attachments/` survives the retirement phase with 6 dead `'use server'` exports whose comments say *"until DM2 retires it"*. Dead app code, **not** a live byte path (catalog clean: 0/0/0/0). ⭐ Invisible to the S6 exit sweep because that sweep is bounded by **identifier** — *"does anything still point at the retired thing?"* and *"is the thing that pointed at it gone?"* are different questions, and DM5 only asked the first. ⚠ **SCOPE NARROWED 2026-08-18 by measurement — the item's title over-reaches.** The deletable unit is **`actions.ts` alone** (6 exports, **zero importers**; the only reference is a doc comment at `queries/case-documents.ts:17`). **`constants.ts` is LIVE** — 3 importers (`queries/attachments`, `queries/interviews`, and transitively `queries/meetings`) — and must NOT be deleted with it — frontend + backend
...
- 🔴 **FUP-DM5-NO-ANSWER-VS-NOTHING** — ⭐ **THE CLASS: an observable PROXY is substituted for the property that actually matters.** (⭕ *Headline corrected in this line 2026-08-18.* It printed the **withdrawn** *"an ACTION PERFORMED is recorded as the STATE ACHIEVED"* from 2026-08-17 onward: QA showed that fits instances 3/4/5 but **not** 1/2 — *"I could not look"* is not an action performed — so the covering sentence was promoted in the body and *action → state* kept as a named sub-class, **but the index was never updated, and the index is the version most readers see.**) Every instance substitutes an observable proxy for the property that matters, and every one fails in the **reassuring** direction; 6 instances, and 4/5/6 were each found *inside the fix for an earlier one*. ⛔ **Instance 3 is why this is 🔴 and is PERSISTED, not just tool output:** `disposed` means "metadata absent, bytes unknown" — a false compliance assertion under LGPD/ANVISA/CFM 1821. On Cloud it is **unverifiable**, not merely unchecked — backend/lead
...
- 🔴 **FUP-DM5-CLOUD-ORPHAN-SURFACE** — ⭕ **ESCALATED 2026-08-18: `FUP-DM4-PRODROW` NOW BLOCKS ON THIS PROBE (PO), and the probe has a REAL subject.** The 2026-08-17 11:37Z remote reset left `storage.objects` at 96 ins / 47 del / **0 live** ⇒ **~49 objects vanished with no DELETE** (buckets reconcile exactly, 16−12=4). A reset rebuilds `storage.objects` but does **not** necessarily delete the bytes, and whether it orphans them is **CLI-VERSION DEPENDENT** ([[remote-reset-storage-orphan-is-cli-version-dependent]]) — so there is now a **concrete population of ~49 likely-orphaned objects in production**, unenumerable because nothing points at them. ⚠ **TRIAGE #1's method is unchanged and unweakened**: still CONSTRUCT an orphan — the 49 can confirm the consequence but can never be the detector's positive control. ⛔ Still blocked on a human step (mint S3 keys in the dashboard). Original filing: UNSETTLED whether Cloud exposes **any** orphan-visible surface; the **S3-compatible endpoint is UNPROBED** and is the single measurement that would settle it. ⛔ **Blocks NO-ANSWER instance 3:** until settled, `disposed` cannot mean more than "metadata gone" on Cloud. Measured: with local proof off, an under-count delete **exits 0 while a real file survives** — 2 of D9's 4 controls lost, both byte-side. ✅ **METHOD RULED 2026-08-18 (DM-FUP TRIAGE #1): CONSTRUCT an orphan, do not probe for one** — the remote holds **0 objects**, so a read-only probe shows only that the endpoint answers, never that it would *reveal* an orphan ([[detector-that-finds-nothing-must-be-proven-able-to-find-something]]). On the **linked** project: upload a synthetic non-PHI byte → delete its `storage.objects` row → ask S3 → clean up. A throwaway project was **declined** (different storage config = an inference, not a measurement) — backend/lead
- 🟠 **FUP-DM5-DISPOSAL-JOB** — ⭐ **CRITICAL FUP C1, split into C1a (local) + C1b (Cloud) on 2026-08-18; the pilot bound is C1b.** `disposal_pending` has **FOUR SET-form inflow writers** — 3 `authenticated`-reachable plus `complete_document_reclassification` (service-role-only) — **and zero automated outflow**; no `pg_cron`, no cron schema, single-process Dockerfile. ⭕ *This line said "three inflow doors" until 2026-08-18 while the C1 entry above it said four — **one item, two numbers, one file**; the queue is fed wider than this line said.* PO ruled *document, do not build* → `docs/deployment/phi-disposal-runbook.md`; owner = the PO, executor = whoever holds service-role reach, **monthly + on a data-subject request**. ✅ **PO 2026-08-18: pilot risk ACCEPTED, BOUNDED — the runbook must run end-to-end against test data BEFORE any real patient record is loaded** (ADR 0121 **Amdt 3**). ⚠ `disposal_state` = **INTENT, not a destruction guarantee**, now ratified as such; **inverts ADR 0099 D10** — for PHI the stale row IS the harm. The decision is discharged; the **REHEARSAL is the deliverable** — PO, then backend
- 🔵 **FUP-DM5-Q1-OPEN-BYTES-CUT-BROKEN** — ⚠ **HALF RESOLVED 2026-08-17 (`24cee179`)**: the fail-open half is fixed and proven; the arm is still a no-op pending a **NAMED** successor (deliberately not re-pointed — a successor must be named, not guessed). ⭐ *A guard written to announce "MUTATION NO-OP" failed OPEN into an error instead*. ✅ **SUCCESSOR NAMED 2026-08-18 (DM-FUP TRIAGE #5): `app.resolve_document_version_bytes`** — derived from the catalog, not guessed: the public doors are thin (flag assert + dispatch) and this `app` kernel holds `can_read_document`, `can_read_referral_phi`, the D15 ceiling and the serving-state refusals. ⭐ **The item's "re-point it at a policy" framing had no target — there is NO `storage.objects` SELECT policy on document bytes at all**; the gate is `prosecdef = t` and **replaces** RLS, so this is a door-mutation arm. ⛔ **Build it inside Critical FUP C2 Tier 1**, not as a one-off — backend
...
- 🟠 **FUP-DM5-SIBLING-GUARD-DIFF** — **no authz arm can see a door that OMITS a check its siblings all make**; all five test gates that *are* there. Specimen: BUG-DM5-S3-INACTIVE-PRINT-1 (closed + rotated 2026-08-18 → [archive](docs/progress/bug-log-archive.md); the SPECIMEN outlives the fix). Wants a transitive catalog guard-set diff with a positive control. ⭕ **FOLDED into Critical FUP C2 Tier 1 on 2026-08-18** alongside `FUP-DM5-Q1-OPEN-BYTES-CUT-BROKEN` — all three want the same door-mutation machinery over `prosecdef` gates, and building it three times was declined — lead/backend
...
- 🟡 **FUP-DM5-DANGLING-PRINT-ON-DELETED-DRAFT** — ⭕ **PREVENTION SHIPPED 2026-08-18, ITEM STAYS OPEN ON TWO REMAINDERS** (`20260928000700`). New orphans are impossible; **pre-existing orphans are unrepaired, and the dangling `securable_resources` row is untouched** — a securable with no subject that every kernel arm joins through. ⚠ *Closing this on the guard alone would be the "a partial fix reads as a complete one" class* — the guard answers only the first of three questions the item asks. Original: a print minted from a **draft** outlives the deleted response: dangling `securable_resources` row, no UI surface, bytes persist. ⛔ **RULED THEN WITHDRAWN 2026-08-18 (DM-FUP TRIAGE #8) — nothing was built.** "Refuse the mint from a non-`submitted` response" **reverses ADR 0104 D7**, whose `RASCUNHO` watermark exists *for* in_progress responses and whose item 4 says *"Completeness does not gate minting"*; pgTAP `312` t6 pins it by name. ⭐ **The item's OWN option list carried the refuted justification** (*"a draft is not a document of record"*) and was quoted forward unchecked. ✅ **RE-RULED AND BUILT 2026-08-18 (#8b): refuse to DELETE a response with an ACTIVE print** — `20260928000700`, `app.guard_response_active_print` (BEFORE DELETE, `HC069`, DEFINER so an RLS-hidden print cannot make it fail open); pgTAP `312` 77→80 red-first with a revoke-then-delete **differential**. D7 preserved; not gated on C1. ⚠ **`active` only, measured:** `lookup_printed_document` never joins `responses`, so `/verificar` survives an orphan and a revoked print must keep its bytes. ⛔ **STILL OPEN, both flagged at ruling time**: pre-existing orphans, and the dangling `securable_resources` row every kernel arm joins through — backend
...
- ⬛ **FUP-DM5-DVF-FILEOBJ** — ✅ **RESOLVED 2026-08-18** (`20260928000600`): `UNIQUE (file_object_id)` makes the 1:1 binding structural instead of a property of caller discipline; pgTAP `328` **128→130**, red-first (K17b's duplicate insert **succeeded** before the migration, so the pin detects absence rather than passing for another reason). Knowingly forecloses rendition byte-sharing. ⛔ **LOCAL ONLY — census the remote for duplicates BEFORE `db push`**; local was 0 rows and proves nothing. Original: ⚠ **RE-CHECKED 2026-08-17**: still latent, but latency rests on **CALLER DISCIPLINE, not the schema** — `document_version_files`'s only unique constraint is `(document_version_id, rendition_kind)`; **nothing makes `file_object_id` unique**, so byte-sharing across versions is structurally permitted. ⛔ Binding input to ADR 0121's disposal lifecycle. ✅ **RULED 2026-08-18 (DM-FUP TRIAGE #4): add `UNIQUE (file_object_id)`** in a migration above `000500`, knowingly foreclosing rendition byte-sharing — a shared `file_object` means marking one row `disposal_pending` silently destroys another's bytes. ⛔ **Census the remote for duplicates BEFORE push**: local is 0 rows, so a green local reset proves nothing — [[backfill-guard-wrap-data-dependent-migration]] — backend
- ⬛ **FUP-DM5-SETLOCAL-MIGRATION** — ✅ **RESOLVED 2026-08-18: the gate is BUILT + WIRED.** `scripts/check-migration-set-local.mjs` = **`lint:set-local`**, the **6th** gate in `npm run lint` (2 above watermark · 411 grandfathered · 0 findings). ⭐ **Validated against ground truth, not against itself**: swept over all 413 migrations with the watermark ignored it returns **4 files / 6 sites with the exact line numbers Postgres named via its own `25P01` warnings**, while clearing the other 8 of the 12 files `grep` flags — the "syntax finds 12, behaviour finds 4" objection retired *empirically*. Positive control in 3 layers: 23 in-process fixtures (the load-bearing one being *bare `set local` after a CLOSED do-block* — a scanner that enters `$$` and never leaves would pass every file silently), end-to-end injection RED/green in an in-scope vs a grandfathered file, and scope fixtures pinning strictly-above. ⛔ **The watermark is a GRANDFATHER LINE — bumping it on a push would grandfather the files you just wrote**, flipping the rot direction from stricter to weaker. Original filing below — **INDEX LINE ADDED 2026-08-18. It had a full body and five doc mentions but NO index entry for its entire life** — `grep SETLOCAL PROGRESS.md` returned nothing, so the next rotation would have dropped it silently. ⭐ *This is the third time the register has re-earned its own warning: a body plus a narrative mention is not an index entry.* `set local` at migration top level is a **silent no-op** past every local gate; the behaviour-bounded defect set is **4 migrations / 6 sites** (syntax-bounded says 12 — the wrong boundary). ⛔ All are at or below `20260928000500` and **cannot be edited in place**, so the residual risk is future migrations only. ✅ **RULED 2026-08-18 (DM-FUP TRIAGE #7): build a WATERMARK-bounded static lint gate, no allowlist** — backend
...
- 🔴 **FUP-DM4-PRODROW** — ⛔ **CENSUS RUN 2026-08-18: THE SUBJECT IS GONE — AND IT WAS ERASED, NOT RECONCILED.** The production DB is **empty** (every table 0 · `auth.users` 0 · all 4 buckets 0 objects); head `20260928000500`, 411 migrations. ⭐ **`pg_stat` distinguishes truncation from deletion and that is the finding**: `auth.users` = **631 inserts / ZERO deletes / ZERO live** — a `DELETE` increments `n_tup_del`, so this is TRUNCATE/reset semantics; only **6 of 165** public tables ever saw one `DELETE`. ⭐⭐ **WHEN — the logs settle it: a REMOTE RESET at `2026-08-17 11:37:35Z`** (`CREATE TABLE IF NOT EXISTS schema_migrations` → all `CREATE EXTENSION` → migrations from `20260711…` **re-applied**; old migrations only re-run on an empty history table, and a `db push` skips them). The known `db push` came *after*, 2026-08-18 01:19. **No `TRUNCATE`/`DROP SCHEMA` in the window** — the emptying was the reset's schema rebuild. *The log supersedes the `pg_stat` inference.* ⭐ **This EXONERATES everyone and indicts the RECORD: the reset preceded TRIAGE #6 by ~14 h, so #6 was RULED ON STALE FACTS** — it sequenced "reset last, it would destroy the surface step 2 needs" against a remote where the surface was already gone. Nobody disobeyed; the ruling was moot when written. The unmanifested, uncounted byte destruction `FUP-DM5-STACK-CYCLE-DESTROYS-BYTES` flags and ADR 0120 **D9** exists to prevent did occur — only the tense changes, *would* → *did*. ⛔ **DOES NOT CLOSE AS "RECONCILED"** — the standing instruction is *"reconcile or quarantine explicitly, never invent success"*, and an empty table meets the letter of "no dangling rows" while delivering no per-row decision, no manifest, no record of what was destroyed. *The absence of the subject is not the discharge of the obligation.* ✅ **PO RULED 2026-08-18: STAYS OPEN, BLOCKED ON the C1b constructed-orphan probe** (both closure options declined). It now deliberately tracks **two** questions — the erased subject **and** the ~49 likely-orphaned objects. ⛔ **Carry forward:** the probe answers the *byte* question only; *"destroyed, not reconciled"* is not something it can change, so this may **never** close as "reconciled" — no per-row decision, no manifest, and that stays true forever. 🔴 **NEW**: `storage.objects` = 96 ins / 47 del / 0 live ⇒ **49 objects vanished without a DELETE** (buckets reconcile exactly, 16−12=4), bytes possibly surviving with nothing pointing at them and the metadata to find them gone — **permanently unmeasurable from SQL**. ⚠ Not established: **when, or by whom** (`pg_stat` has no timestamps and its counters are not durable evidence). Census + deriving queries: `docs/backend-state.md` § REMOTE CENSUS 2026-08-18. Original filing: the dangling frozen-snapshot PRODUCTION row + 3 unreferenced objects: reconcile at push/deploy (PO R2). ⚠ Must NOT delete DM4's M3/M4 guards. ⭕ **ITS TRIGGER FIRED 2026-08-18 when the push ran** — it is DUE, not deferred, and its own ⛔ *"do not query the linked project"* bar lifted at the same moment. ✅ **SEQUENCED 2026-08-18 (DM-FUP TRIAGE #6): read-only census → the #1 constructed-orphan probe → THEN reconcile-vs-reset, with a manifest.** ⛔ **The PO-sanctioned "cheap path" (a remote reset) collides with an open item** — a bare `db:reset:linked` is exactly the unmanifested, uncounted, unaudited byte destruction `FUP-DM5-STACK-CYCLE-DESTROYS-BYTES` flags, and it would destroy the surface #1 needs to measure. ⚠ The 2026-08-11 figures are **stale by design** — re-census, never act on them — lead/backend
