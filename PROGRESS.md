...
| --- | --- | --- | --- |
| **ETH·E4** — ethics participant seating & professional identity (ADR 0108) | ⛔ **CHANGES REQUESTED (r1)** — 1 P0 / 6 MAJOR / 11 MINOR. **P0:** ADR 0108 D5's exposure argument ("discloses no case linkage") is false — `retention_pin_reason='ethics_decision_issued'` + `retention_pinned_at` sit in the table-wide `authenticated` SELECT grant, so the widening tells every org manager that a named doctor was a `respondent_doctor` on a case with an issued decision (measured live: sibling-commission `staff_admin`, 0 rows pre-D5 → 1 row post-D5; live writer, product-reachable via `issue_decision`). Verified CLEAN and stated so: `set_primary_subject`'s deltas are surgical with the M7 tenancy cut preserved, the D5 keystone and all three write-door gates falsify under my own neutralization runs (8 oracles), the `ARM=census` correction is correct, FUP-ETH-CPF-1's latency claim holds | 2026-08-11 | [eth-e4](docs/reviews/eth-e4-review.md) |
