...
| **ETH·E4** | **Ethics participant seating & professional identity** [0108](docs/decisions/0108-eth-e4-participant-seating.md) · [plan](docs/phases/ethics-e4-participant-seating.md) · [locator contract](docs/phases/ethics-e4-locator-contract.md) — closes FUP-ETH-1 + FUP-FF5-2 | ⏳ **AWAITING HUMAN APPROVAL (gate step 4) 2026-08-11** — build + tests + QA all green; **nothing merged, nothing pushed** | ✅ lint 0/0 (incl. client-server 0 findings · vacuous 174 files/0) · typecheck · vitest 1218/1218 · `next build` exit 0 · pgTAP **5794/5794** on a fresh reset (353 registered = 353 files) · `ARM=census` 450/461 · `ARM=hat` · `ARM=floor` 79 allowlisted — all HOLD · the 3 write doors **COVERED** in the standing write-path harness (⛔ never `ARM=census` — ADR 0079 Am. 5) | ✅ **GREEN — the r2 unproven set is CLEARED.** The 7 never-verdicted specs + both ethics specs + the 2 heaviest `getCaseDetail` consumers: **140 passed · 0 failed · 0 infra · 0 flaky · 0 did-not-run · 7 batches**, accounted 140/141 (1 skip). Integrity checked FIRST: batches 1–7 no gaps · 1 `reset FAILED` (b5) **RECOVERED** at 21/21 · denominator reconciles. The new `toBeDisabled()` assertion is **proven falsifiable** (neutralize `Boolean(linkUserId)` → RED, exit 1; reverted, no residue) | **r1** 1 P0 · 6 MAJOR · 11 MINOR → **r2** CHANGES REQUESTED (narrow) → **r3 ✅ APPROVED** [review](docs/reviews/eth-e4-review.md) | ⏳ **pending** | – | 38 commits, branch only |
...
| Date | Run | Result |
| --- | --- | --- |
| 2026-08-11 | **ETH·E4 · LEAD · FINAL run of the session, @ `2efa691`** (clean reset + `REBUILD=1`), after QA r2's three blockers were fixed. ⭐ **Full context + resume steps: [eth-e4-handoff.md](docs/progress/eth-e4-handoff.md)** | **RED (UNRUN ONLY) — `999 passed · 0 failed · 52 infra · 2 flaky · 17 did-not-run · 17 batches` · accounted 1070/1076 (6 skips).** ⚠ **Zero assertion failures.** Integrity verified: **0 `reset FAILED`**, batches **1–17 no gaps**, every batch's own `accounted N/N` reconciles. Isolated to **`b16(infra-unproven 52, did-not-run 17)`** — batch 16 hit `server_dead=1` (104 conn errors) and its automatic `INFRA_RETRY` **died too**. Batches 5 and 17 also died but recovered on retry: **3 of 17 batches lost the standalone server in one run**, vs 1 of 17 earlier the same day — a **degrading infra characteristic, not an ETH·E4 defect** (`BATCH_TESTS=22` has twice rescued a dead group). ⛔ **The risk that matters:** the 7 unproven specs are `platform-org-admin-provisioning` · `process-template-narrative-slot-crud` · `process-template-versioning` · `processless-cases` · `qob-org-admin-content-wall` · `quality-oversight` · `recommend-result` — and **four of them exercise case-detail routes heavily** (11/8/8/1 refs), which is precisely what `45e68c5` changed by making 5 `getCaseDetail` reads **throw instead of silently degrading**. A red there is most likely a **previously-hidden defect surfacing, not a regression** — but it is unproven either way. **Re-run batch 16 before anything else.** For contrast, the immediately-prior full run @ `7e55f01` was **GREEN: 1068 passed · 0 failed · 0 infra · 0 did-not-run**, which is what proves the P0 column-list grant broke nothing suite-wide. |
...
| ~~**ETH·E4** — round 2~~ | ⛔ **CHANGES REQUESTED (r2)** — narrow: **P0-1 CLOSED** (verified 4 ways from the catalog; my own divergence oracles prove the `set_eq` keystone reds in BOTH directions; no view or sibling projection door was missed) and MAJOR-3/4/5 + m6(a–d) confirmed. **3 small blockers:** B-1 MAJOR-2(a) is NOT fixed — the throw reaches the action and is discarded at `add-participant-dialog.tsx:589`/`:619`, so a failed search still renders "Nenhum resultado…" and steers to `no_account`; B-2 MAJOR-6's professional lane is uncovered on a premise `pg_proc` contradicts (`create_professional_profile` is a bare INSERT, not get-or-create); B-3 NEW — `cases.ts:1333` silently empties the whole E4 roster. Open items judged: MAJOR-2(b) **not a blocker once B-1 lands** (the scope limit's population is near-disjoint from where the impedimento is load-bearing — verified); m2 **not a blocker**, the P0 fix shrank it. pgTAP re-verified on a fresh reset: **5794/5794** (the short intermediate counts are the E2E-mutated-DB class, void not flaky) | 2026-08-11 | [eth-e4 (r1+r2)](docs/reviews/eth-e4-review.md) |
| ~~**ETH·E4** — round 1~~ | ⛔ CHANGES REQUESTED (r1) — 1 P0 / 6 MAJOR / 11 MINOR. **P0:** ADR 0108 D5's exposure argument ("discloses no case linkage") is false — `retention_pin_reason='ethics_decision_issued'` + `retention_pinned_at` sit in the table-wide `authenticated` SELECT grant, so the widening tells every org manager that a named doctor was a `respondent_doctor` on a case with an issued decision (measured live: sibling-commission `staff_admin`, 0 rows pre-D5 → 1 row post-D5; live writer, product-reachable via `issue_decision`). Verified CLEAN and stated so: `set_primary_subject`'s deltas are surgical with the M7 tenancy cut preserved, the D5 keystone and all three write-door gates falsify under my own neutralization runs (8 oracles), the `ARM=census` correction is correct, FUP-ETH-CPF-1's latency claim holds | 2026-08-11 | [eth-e4](docs/reviews/eth-e4-review.md) |
| **ETH·E4** — PRE-BUILD plan review (ADR 0108) | ⛔ CHANGES REQUESTED — 7 findings, all addressed pre-build | 2026-08-11 | [eth-e4-plan](docs/reviews/eth-e4-plan-review.md) |
...
- 🔴 **FUP-ETH-CPF-1** — ETH·E4's D5 widening also exposes `professional_profiles.cpf` (a national ID) org-wide; ADR 0108 D5's exposure argument does not name it. **CLOSED in-phase** by the P0 column-list grant (`0882296`) — kept live only until QA re-review confirms → [body](docs/progress/follow-ups.md)
...
- 🟡 **FUP-F2-BUCKETS** — F2's legacy-bucket retirement was deferred in writing 4× and tracked NOWHERE (0 hits in this file until 2026-08-11); `meeting-attachments` still carries **both** live policies (`is_member_of` SELECT) with no writer — `interview-attachments` is already sealed, `referral-attachments` is NOT legacy. Measure remote objects BEFORE dropping (the fold-in is pure DDL) — backend
