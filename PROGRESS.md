...
| BE-9 | pgTAP: 209 §B/§C re-pin + the ADR-0087 §Gate keystones, **each mutation-proven** (revert the guard → keystone goes red) | backend | ✅ `270_ff1_repeating_groups.sql` 46 keystones + 209 §A / 51 re-pins. **Full ordered `supabase test db` on a fresh reset: 136 files / 3918 tests PASS**, re-run by backend. 4 mutation proofs executed (not just documented) — one exposed a missing keystone (G1b) and one exposed a vacuous assertion (M3) |
...
**1 open bug, 4 closed** (filed 2026-07-27, FF-1 test pass; re-verified same day, twice). All five are
application-code defects, not spec issues — each traced to a specific line via live-catalog/source
...
| BUG-FF1-005 | FF-1 | **Blocker** | Found while re-verifying BUG-FF1-004 (FF1-3's resume check, now reachable for the first time since neither 001 nor 004 blocked it before). `saveSection` (`src/lib/responses/actions.ts` ~line 319) destructures its `input` **without `instances`** and never passes `p_instance_answers` to the `save_section_answers` RPC call — despite the client (`collectSection` in `wizard-client.tsx`) correctly building an `instances: InstanceAnswers[]` array and sending it in every `saveSection`/`saveAndExit`/`persistSection` call. `save_section_answers` itself supports `p_instance_answers` correctly (confirmed: every RPC-bypass helper in this spec calls it directly and it works). Repro: fill a repeating group's children via the real wizard UI, click "Salvar e sair" (or "Próximo"/"Revisar" — same `persistSection` path), then query `select * from answers where response_id = … and group_instance_id is not null` → **zero rows**, despite `response_group_instances` existing correctly (add/remove/reorder, i.e. BUG-FF1-001, work fine). This was invisible to FF1-4/5/7/8 because **they all fill via a direct `save_section_answers` RPC call** (built specifically to route around the then-broken BUG-FF1-001 UI) — none of them exercises the wizard's own save action, so none could have caught this. FF1-3 is the only spec in this file that saves through the real UI. | The wizard's normal save actions ("Salvar e sair", "Próximo", "Revisar") persist per-instance answers exactly as they persist top-level ones. | Per-instance answers are silently dropped on every section save; a user who fills a repeating group and saves/navigates away loses all of it (would resurface as a `min_instances`-unmet rejection or a blank instance on resume/submit, not a visible error at save time). | `e2e/ff1-repeating-groups.spec.ts` FF1-3 (fails cleanly on a dedicated DB-truth assertion right after "Salvar e sair": `answers` with `group_instance_id is not null` must be non-empty, is 0) — violates ADR 0087 ruling 5's "ONE payload for both tiers" save-path shape and blocks Acceptance #2 (resume) end-to-end via the real UI | backend | open |
