...
- 🔴 **FUP-DM5-BACKUP-HAS-NO-CLOUD-FORM** — the § 6b backup procedure is `docker exec … tar`, **local-only by construction**, and on Cloud there is **no Storage backup at all**: managed backups/PITR **exclude Storage objects by documented design**, "Restore to a new project" does not copy them, and `supabase storage cp -r` has no streaming form so it writes **plaintext PHI to disk** ⇒ § 6b's "encrypted AT CREATION" is **unsatisfiable there**. ⭐ The residue `FUP-DM5-BACKUP-IS-PHI-EXPORT`'s close does not absorb — that item graded *"the mechanism as it applies to production"*, and the mechanism **cannot be issued in production**: the Cloud risk is an **absent** backup, not an over-wide one. PO decision owed (accept no recovery point pre-pilot, or name a mechanism — only the **UNPROBED** S3 endpoint could satisfy "encrypted at creation") — PO/backend/lead
- 🟠 **FUP-DM5-DB-DUMP-AND-SCRATCH-DB-UNGOVERNED** — § 6b's five values are scoped literally to *"a Storage backup" / "the archive"*, but the same section requires a `supabase db dump` restored into a **scratch database** to earn the words *"verified good"*. **Neither has a location, reader-set, retention or destruction rule**, and nothing says to drop the scratch DB — which the page itself calls *"a data leak wearing one"*. ⚠ Reachable on **Cloud today** (`db dump --linked` needs only the DB password), unlike its sibling. ⭐ `FUP-DM5-BACKUP-IS-PHI-EXPORT`'s own sting one level down, **inside the section that resolved it** — PO/backend
