...
- 🟠 **FUP-DOOR-SWEEP-DERIVER-POINTS-AT-ONE-ARM** — `scripts/door-sweep-cases.sh` derives **read AND write** policies but its paste-able command names only `p0-authz-door-audit.sh`, so an operator following its own output sweeps half the list and the write half goes silently unmeasured (measured 2026-08-27: 22 of 52). ⚠ Surfaced **only** because the read harness refuses to end CLEAN with unmatched cases. A gap in the §6 step-1 recipe itself; the fix is print-only (emit both commands, or split output by arm) so it cannot change what is derived → [body](docs/progress/follow-ups.md) — backend
- 🔴 **FUP-WRITEPATH-AUDIT-EXITS-CLEAN-HAVING-MEASURED-NOTHING** — `p0-authz-writepath-audit.sh` computes `(COVERED = the rest)` as a residual over an **empty set** and prints `EXITCODE=0` when every case ERRORed or was never selected (measured 2026-08-27: 13 drift-ERROR + 9 absent = **22 of 22 unmeasured, exit 0**). ⛔ Its sibling `p0-authz-door-audit.sh` handles the identical case as **exit 3 UNPROVEN (PARTIAL)** — same class, opposite handling, **two halves of the one gate CLAUDE.md §6 step 1 mandates every phase**, so anyone reading exit codes records the phase clean. ⭐ **Second instance** of the failure behind `.claude/rules/authz-gate-results-need-a-current-baseline.md` (`ARM=census` at exit 0 with ZERO gates enumerated). Also: **9 policies sit in NEITHER arm's domain**, and `scripts/door-sweep-cases.sh` derives a two-arm case list while printing one arm's command → [body](docs/progress/follow-ups.md) — backend/PO
