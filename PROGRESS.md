...
- **🔒 DB WINDOW OPEN 2026-09-02 (lead) — the local stack has ONE owner and it is this session.** ⛔ **Do not `supabase db reset`, run `test:db`, `e2e:prod` or any mutation harness while this line is here** — a reset lands silently in this window's evidence, and Playwright's webServer half-applies one by holding connections. Verified free before opening (`pg_stat_activity`: PostgREST + realtime only, no app server, no peer psql). ▶ **Queue, cheapest first:** (1) write-arm sweep, 4 cases · (2) rollback runbook §6 verification · (3) IA-F9 perf — ⚠ **its own reset-bracketed slot**, because `ANALYZE` ends AE0.2 comparability until the next reset (`FUP-PERF-ANALYZE-ENDS-AE0-COMPARABILITY`) · (4) **C2 Tier 1 (~6 h) — HELD**, pending the PO ruling on ADR 0162 §3 branch order. ⛔ Delete this line when the window closes.
