...
- **✅ CASE SURFACE SPLIT · INCREMENT 1 — COMPLETE 2026-08-21** (ADR
  [0134](docs/decisions/0134-case-surface-split-and-administrativo-case-read.md) D1–D5, D7 + Step 0).
  §6 steps **1–5 all done**; QA **APPROVED r3** ([review](docs/reviews/case-surface-split-increment-1-review.md));
  **PO approved 2026-08-21** and merged to `main`. `/casos` is now literally a reading surface — both
  ...
  Full narrative rotated verbatim → [case-surface-split-increment-1.md](docs/progress/case-surface-split-increment-1.md).
  ...
  cover Increment 2, any remote push, or the D1 wording amendment — **drafted 2026-08-22 as ADR 0134
  Amendment 3, status PROPOSED: it authorizes nothing and D1 as ratified still stands.** It replaces
  D1's sentence with a capability-invariance test (`/casos` may host an affordance whose door admits
  every member who can open the page), because D1's letter is **observably false on `main`** — two
  write affordances there never consulted `caps` at all (`notify_safety_event`, membership-only;
  `file_correction_request`, `can_read_case`-keyed — both re-measured from the catalog). ⛔ Its own
  test surfaced a **D6** gap, which Amendment 3 explicitly does NOT close:
  `FUP-S8-UNBOUNDED-BY-CASE-ACCESS-POLICY`, to be ruled **before M2 is written**.
