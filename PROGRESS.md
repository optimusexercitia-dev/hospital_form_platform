...
| C2-TIER1 | Command-door Tier 1 sweep — PHI-touching command doors, gate-aware closure | 🟢 in progress | `authz-c2-tier1` | [hub](docs/features/c2-tier1.md) |
...
| Date | Run | Result |
| --- | --- | --- |
| 2026-09-03 | **Run 7 — external-audit response; P2 re-specified + MEASURED** (⛔ build gates, **NOT** Gate AE4) | pgTAP **262f/8745 PASS** (+414, +7) · lint 12/12 · tsc 0 · `RESET=0 LOAD=0 PASSA=3 PASSB=3 P1PROBE=0` · ⭐ **P2PROBE=0 — first P2 verdict ever measured** · ⛔ first attempt VOIDed (NULL-not-0 counter); recorded → [§17.1](docs/design/authz-ae4-performance-acceptance.md) · ⛔ **no e2e** |
| 2026-09-03 | **AE4/IA-F9 statement-scoped resolution + QA corrections** (⛔ build gates, **NOT** Gate AE4) | pgTAP **261f/8738 PASS** · lint 12/12 · tsc 0 · vitest 151f/2056 · **4 ARMs HOLD** · sweep READ **CLEAN 1/1 re-measured**, WRITE **0 cases ruled** · **3 mutation verdicts RE-EARNED** (8733→8738) · **P5 0.00×, K=4 unmoved** · ⛔ **no e2e** → [§14–15](docs/design/authz-ae4-performance-acceptance.md) |
...
| Date | Decision | Ref |
| --- | --- | --- |
| 2026-09-03 | ⚠ **External AE4/IA-F9 audit disposed** — mechanism confirmed (P2's `loops=1` grep can't tell resolver invocations from off-path nodes), severity overstated (D=82 vs 13 real orgs); BLOCKER declined. P2 re-specified with an executable checker | ADR 0183 (owed) · [response](docs/reviews/authz-ae4-external-audit-response-2026-09-03.md) |
| 2026-09-03 | ⭐ **THE PERMISSION ANSWER IS COMPUTED ONCE PER STATEMENT, not per protected row.** ⛔ **Not a cache:** the map only PROPOSES a scope, `authz.has_permission` ITSELF confirms each — over-grant impossible by construction, a wrong map can only DENY. P5 **4.99× → 0.00×**, **K=4 NOT moved** | ADR [0182](docs/decisions/0182-statement-scoped-authorized-scope-ids.md) |
| 2026-09-03 | ⛔ **A CHANGE THAT FLATTENS A COST CURVE INVALIDATES EVERY CONTROL READING IT** — they fail in the direction that looks like success. DC1 *and* DC2 would have failed **because it worked**; both moved to the pre-change predicate, DC3 + P7 bound the live path. ⚠ Found by **RUNNING** | acceptance **§13.6** |
| 2026-09-03 | ⛔ **A HAND-TYPED EXPECTED VALUE CANNOT TELL A DEFECT FROM A DESIGN** — 413 pinned a malformed `search_path` copied from the broken catalog, so fixing it would have REDDENED the suite. Repaired via a SIBLING ref — ⛔ **reversed same-day**: both siblings PINNED, class swept by pgTAP `414` | ADR [0182](docs/decisions/0182-statement-scoped-authorized-scope-ids.md) § Corrections |
