...
> ### After the seven — the order the next session should work in
>
> 1. **Record step 4's answers** where they were asked (the ADRs **and** this file — *an approval's
>    SCOPE is a fact that must be written down*; one gate covering two units already cost a redundant round here).
> 2. **§6 step 5 — Record + rotation.** ⚠ **PROGRESS.md is ~121 KB against §7's "well under 60 KB".**
>    The phase QA's specific guidance: rotate the **Test Run Summary** RED-run narrative + the S3 row
>    to `test-run-archive.md`, and compress the two new **QA Verdicts** rows — both broke their own
>    retention contracts to restore currency. ⛔ **Do NOT rotate the follow-up index lines — that
>    re-creates phase-QA finding R3.** Everything else added at S6 belongs where it is.
> 3. **THEN** the `db push` question (see § "State" — ⛔ **a git push is NOT a `db push`**; the recusal
>    PHI fix and S4's retirement are on `origin/main` but **NOT on the database**).
>
> **Full reasoning for every item → [dm5-phase-review.md](docs/reviews/dm5-phase-review.md) §5 and
> [follow-ups.md](docs/progress/follow-ups.md).**

### 🔵 IN PROGRESS — **DM5: Wave D + retirement** (opened 2026-08-14) — the program's FINAL phase

> **Full detail → [dm5-wave-d-retirement.md](docs/progress/dm5-wave-d-retirement.md)** ·
> ⛔ **NEXT-SESSION RESUME POINT = the 🛑 START HERE block directly above this section, NOT the
> handoff.** ⚠ **[dm5-handoff.md](docs/progress/dm5-handoff.md) §13 is now DISCHARGED HISTORY** — it
