...
| M-B5 | backend | QA r1 fix wave (BLOCKER-1→A7, MAJOR-1→A8, MINORs 1–7; ADR 0104 Amendments A7/A8/A9 = the spec) | ✅ done 2026-08-08 — **FIX-A** migration `20260914000100`: `app.can_read_full_meeting_content` (mirrors the LIVE `_project_meeting_agenda_item` terms: respondent-title + presence-qualified deliberation-gated text) conjoined into the meeting arm (mint AND download); property-diff identical; **FIX-B** per-kind PHI gate = the ratified TRIO site 3 (marker recorded in-door, supersedes "exactly 2"); provider derives `contains_phi` from presence (case-linked item title / minutes_md / gated fields); action forwards it + derives `phi/` path (A8 ≠ D9 kept visibly distinct in comments); **FIX-C 1–7**: ha_b positive control t51 · revoke-without-sight PINNED both directions t52–t54 (D11 governance, lead-ruled) · 312 t45 de-confused · 313 t30 label · shared `meetingWatermarkFor` exported from `@/lib/pdf/documents/meeting` (frontend swaps its inline copy) · degenerate-branch variant re-cut + mutation-proven (empty-agenda edit → variant red, canonical green) · ENUM_FALLBACK '—' sweep. pgTAP 313 → 54 (A7 keystones t39–t48 with respondent chain + `explicit_grants_only` masked-member specimens); drills D9 5 red · D8 16 red · D2 9 red, restored 128/128; diff-scoped sweeps dispatch+helper COVERED 0 BLIND (door's authority = the swept dispatch + D2); census+floor HOLD; SMOKE 3/3 incl. coordinator masked-content mint (contains_phi=true, `phi/`, hash-match) + respondent denied mint (pt-BR 42501) AND open (0 rows); fresh reset 322==322 · test:db 174/5502 PASS · lint 0/0 · typecheck · vitest 82/1194. ⚠ one transient: the first post-Docker-restart reset ABORTED at migration 297/322 (exit 255 was REAL, caught by asserting registered==files, not the exit code) — re-run clean; the FAIL suite run on the partial DB was discarded |
| M-F1 | frontend | Mint surface + "Documentos emitidos" panel on the meeting detail screen — reuse `src/components/printing/` as-is; new components only if reuse is impossible (that's an abstraction-leak signal, report it) | ✅ done 2026-08-08 (`5510bd3`) — **zero printing components changed**; `PrintedDocumentsSection` took `sourceKind="meeting"` as-is, so the wiring is pure prop-threading on the meeting page (no abstraction leak to report). Flag read joined the page's existing parallel batch; watermark FINAL only from `signed`/`distributed` (mirrors the provider's derivation — ⚠ the slug is `distributed`, `distribuida` is only the pt-BR label); `canRevoke` reuses the page's existing `isCoordinator`; no mint surface for a non-member org_admin because the `isCommissionMember` gate already turns them away (domain gate, deliberately not compensated). Gates: lint + typecheck + real `next build` green. **Verified on a PROD standalone build** against the local stack: page 200, no RSC crash from the server-action props, section + mint button render, `listPrintedDocuments('meeting', …)` resolves through RLS to the empty state (no error notice), flight payload carries `draft` for this `held` meeting. NOT verified: the open-dialog interaction — the Browser pane was not compositing (rAF-throttled: React fibers present but Suspense reveal and Radix open never commit), which is a harness limitation, not a product defect; interactive coverage belongs to M-T1. **QA MINOR-5 fixed (`4b4293a`)**: the page's inline watermark derivation duplicated the provider's rule, so the dialog's promised mark and the renderer's stamped mark were kept in step by discipline rather than by construction; now consumes the shared pure `meetingWatermarkFor(status)` from `@/lib/pdf/documents/meeting` and the local copy is deleted (behaviour identical — `signed`/`distributed` → final, all else incl. `in_signature`/`cancelled` → draft). Lint + typecheck + `next build` green |
| M-T1 | tester | E2E: ata mint→download→verify chain + RASCUNHO (unapproved minutes) vs FINAL (approved) watermark assertion | ✅ done 2026-08-08 — `e2e/pdf-printing-meetings.spec.ts` (4 tests) + `e2e/helpers/pdf-printing-meetings.ts`; **4/4 GREEN, run twice on independent fresh resets (8/8 total)**; P1's UNTOUCHED `e2e/pdf-printing.spec.ts` re-run once as the no-regression check → **7/7 GREEN** (2 bugs filed, LOW+MEDIUM, neither app-blocking — see Bug Log). Covers: full ata lifecycle on a meeting driven to `signed` through the REAL `create_meeting`→`conclude_meeting`→`sign_meeting` flow (mint previews FINAL, panel chip "Ativo", authenticated download 200/`%PDF-`, logged-out `/verificar/<código>` shows "Ata de reunião" kind wording + no download link, revoke → "Anulado", re-verify shows "Documento anulado", overlay re-download bytes differ); RASCUNHO↔FINAL preview tracked across the SAME meeting's real lifecycle transition (held → RASCUNHO, driven to signed → FINAL, no stale mark in either dialog open); restricted `participants_only` visibility (no product RPC sets this column — flipped via a documented service-role scaffold, mirroring `setControlledDocsFlag`/`setAudioMinutesFlag`; asserted the RLS boundary directly via a raw PostgREST read under the excluded member's own JWT → `[]`, PLUS the app-level content boundary — no mint button/panel/meeting content renders); keyboard-only mint→download-link-focus on the meeting page (non-coordinator viewer keeps the tab-stop count tractable; `focusByTabbing` max raised to 200 for this richer page). **`mintViaDialog`/`articleForShortCode` reused UNCHANGED from `./helpers/pdf-printing`** (P1) — no edit needed, matching M-F1's "zero printing components changed" finding at the test layer too. Reused `e2e/helpers/minutes.ts`'s RPC/auth plumbing (`callRPC`/`getOwnerToken`/`addAttendee`) rather than re-deriving it. Run: `npm run build` → prod-standalone (`node .next/standalone/server.js`, :3000) → `npx playwright test e2e/pdf-printing-meetings.spec.ts --project=chromium --workers=1`, fresh `supabase db reset --local` before each run (321 migrations). Lint + `tsc --noEmit` both clean. One infra flake triaged per house rule (retry-once-on-clean-stack): the FIRST P1 no-regression run hit the documented PGRST002 schema-cache-not-ready race immediately after `db reset` (2 spurious fixture-pool failures, 0 real) — re-ran on a settled stack → clean 7/7, not filed as a regression. **2026-08-08 UPDATE — QA r1 fix-wave (ADR 0104 A7/A8) re-run + new A7 coverage:** fresh reset (322 migrations) · regression re-run of both current-phase specs on the rebuilt prod-standalone → **12/12 GREEN across two independent runs (24/24 total)** — the A7/A8 conjunction landing changed nothing for the existing fixtures (none are case-linked), confirming the coordinator's own prediction. **New test added to `e2e/pdf-printing-meetings.spec.ts`: "A7: the linked case respondent cannot mint or download the ata"** — composed entirely via real RPCs (`create_meeting` → `create_meeting_agenda_item` with `p_description` → `link_meeting_case` onto the SEEDED ETH·E1 ethics case, `ca000000-0000-0000-0000-0000000000e1`, on which `staff4.ccih@test.local` is already the seed's `respondent_doctor` and `chefe.ccih` already holds an explicit `read_case_deliberation` grant — the case + its participants chain is READ-ONLY seed data here, never mutated; only the meeting/agenda-item/link are freshly created). Asserts: coordinator mints fine (full sight via the seeded grant) AND the mint is presence-derived PHI (`contains_phi=true`, `storage_path` under `phi/` — DB-truth checked, the UI carries no PHI indicator) AND the coordinator downloads 200/`%PDF-`; the respondent (`staff4.ccih`) reaches the meeting page itself (200 — the base `can_reach_meeting` admits any commission member on a `commission_default` meeting, so this is NOT a repeat of BUG-PDF2-002) but the coordinator's minted ata is absent from their own panel (RLS denial, no leakage), their own mint attempt fails with the door's verbatim pt-BR 42501 ("sem autorização…") and never reaches the success state, and a direct probe of the coordinator's `downloadPath` under the respondent's own session returns 404 — both mint AND download denied, per D11/A7. Helpers added to `e2e/helpers/pdf-printing-meetings.ts`: `addAgendaItemWithDescription`, `linkMeetingCase`, `SEED_ETHICS_CASE_ID`, `RESPONDENT_STAFF4_EMAIL` (all doc-commented; composition pattern mirrors backend's own `scripts/smoke/pdf-mint.smoke.ts` A7 probe, reusing the SAME seeded fixture rather than re-deriving a participants-registry chain). **Coverage story requested by the lead: composing the A7 chain via legitimate helpers/RPCs was FEASIBLE** — no pgTAP-only fallback needed; E2E coverage now stands alongside pgTAP 313 t39–t48 and the backend smoke, not instead of them. New test: **3/3 GREEN across three runs** (1 solo + 2 full-suite). Lint + `tsc --noEmit` clean. One infra event: a combined 12-test single-server run hit the documented Windows prod-standalone collapse (background task itself reported failed/exit 127 mid-run, producing an `ERR_CONNECTION_REFUSED` cascade on the LATTER tests, not a Playwright-attributable failure) — split into two per-file runs on a restarted server, both clean; the split pattern was then repeated twice more for the stability figures above. Not filed as a regression (matches the documented `e2e-prod-build-flaky-baseline` / `e2e-standalone-server-not-next-start` class) |
| M-Q1 | qa | Phase review `docs/reviews/phase-PDF-P2-review.md` — THE question above, plus the standard security axes over the arm re-emit | ✅ **APPROVED (r2)** 2026-08-08 ([r2 addendum](docs/reviews/phase-PDF-P2-review.md#round-2--verification-of-the-package-a-fix-wave)) — **BLOCKER-1 CLOSED, re-proven by constructing the exploit class myself**: the seed holds no persona with `reach=t ∧ full_sight=f`, so a vacuously-true conjunct would have "closed" it silently. In a rolled-back txn I inserted the 4-row participants chain making `chefe.ccih` (a full member who reaches the meeting) a respondent of the linked case → `reach` **stays t**, `full_sight` **f**, arm **f**. New helper diffed term-by-term against the LIVE masking (respondent term mirrors; capability term adds a *presence* guard, refusing only where masking would actually null text); explicit-uid DEFINER, `search_path=''`. **MAJOR-1 CLOSED** by A8 presence-derived labeling — covers all four `PHI-BEARING` columns plus the process-number term, `phi/` bifurcation pinned (t45/t46/t50), `form_response` still refuses `contains_phi=true` (t49). **All 7 MINORs closed** (asymmetry pinned BOTH ways t52–t54 with the D11 citation; t51 pins `ha_b`'s tier; `meetingWatermarkFor` shared by provider+dialog; `ENUM_FALLBACK` + retyped map). Kind-conditional sites re-counted from the live door = **exactly 3** (A8 trio). Property diff vs my r1 baseline byte-identical across all three re-emits. Lint 0 · tsc 0 · 22 unit tests. **2 new INFO:** the helper is fail-open *standalone* (`not exists` over zero agenda rows → `t`; safe only because `can_reach_meeting` is the first conjunct — P3/P4 must not reuse it alone), and `template-fingerprints.ts` still calls `final_signed` the "quorum line" variant while MINOR-6 set `quorum: null` and the test now asserts `not.toContain('Quórum:')`. *(r1 below)* — ⛔ **CHANGES REQUESTED** 2026-08-08 ([review](docs/reviews/phase-PDF-P2-review.md)) — **the binding scope question is answered YES, P2 held** (all 20 files classified; ruling 1 re-counted from the live door = 2 semantic sites, no third; ruling 2 verified copy-only, no prop-shape change; types.ts purely additive; page is mechanical composition; no pipeline surgery). Property-diff vs my P1-recorded baseline: `prosecdef`/`proacl`/`proconfig` **identical** on both re-emits. Meeting arm = pure delegation to an explicit-uid DEFINER; NULL-propagation probed (returns **false**, not NULL — no fail-open); no-admin-arm delta proven **behaviourally** (same org_admin denied `meeting` / passes `form_response` / is genuinely commission-admin). **1 BLOCKER:** the ata freezes the MINTER's per-caller content projection and re-serves it at `can_reach_meeting`, which carries no case-capability term — a respondent of a linked case can download his own process number, which the DB refuses him at **column-grant** level (`title`/`description`/`discussion_notes`/`resolution` have no SELECT grant; `app._project_meeting_agenda_item` is the only read path). Violates D11's "the module never grants sight of anything". **1 MAJOR:** all four printed ata columns are catalog-classed `PHI-BEARING` while the payload hardcodes `containsPhi:false` → `std/` prefix, no confidentiality band; forms carry **zero** such comments, so the asymmetry is real — needs a PO ruling per D9's own "upstream ADR" clause. + 7 MINOR / 3 INFO. Rule 7 clean (23/23 sites, proven executably); provider FINAL⇔`signed\|distributed` total over the catalog's 6 statuses; signature filter is an allowlist. |
...
- **Ruling: BUG-PDF2-001 fix scope** — frontend swept the DEFECT, not the filed string: all 5 "desta resposta" literals in `src/components/printing/` parameterized via `labels.ts` (kind-aware, incl. kind-aware watermark rationales; neutral "deste registro" fallback; `case`/`interview` deliberately NOT enumerated) — accepted as the surfaced-P1-bug class, copy-only (`2e8ef7f`).
- **Full `e2e:prod` gate (lead, 2026-08-08): GREEN — 0 real failures / 1030 collected.** Run 1: 1002 passed · 2 failed · 7 flaky · 14 did-not-run · 17 batches · 0 `reset FAILED` · accounted 1025/1030; both failures in old baseline specs (`form-builder-enhancements` AC-1 — its batch stranded 14; `phase15-indicators` AC-1), neither touching PDF surfaces. Re-run of both batches' specs on a fresh stack: **80/80 · 0 failed · 0 flaky · GATE GREEN** — both transient. All 11 PDF specs (7 P1 + 4 P2) green in-suite.
- **QA r1: CHANGES REQUESTED** ([review](docs/reviews/phase-PDF-P2-review.md) `258e1a0`) — the binding question HELD (20/20 files classified in-formula), but BLOCKER-1 (ata bakes the minter's full view while `can_reach_meeting` lets masked callers download — the DB's column grants refuse what the PDF hands over) + MAJOR-1 (`PHI-BEARING`-commented columns minted as `contains_phi=false`) + 7 MINOR.
- **PO RULING (2026-08-08): Package A + conservative PHI labeling → ADR 0104 A7/A8/A9.** Meeting arm gains the full-sight conjunction (mint AND download); ata always complete; presence-derived `contains_phi=true` (band + `phi/`) when masked-class content present; the door's PHI gate becomes the THIRD sanctioned kind-site (a fourth = leak); plan formula gains "+ mint surface". Fix wave dispatched; QA r2 follows.
