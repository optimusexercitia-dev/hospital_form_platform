...
- 🟡 **FUP-LINT-STALE-SYMBOL-COMMENT** — propose a **6th lint gate**: flag a comment naming an identifier that no longer exists. Every existing gate was added after its class shipped a live defect; this class hit **~6× in DM3 alone** (plus 4 historically, one shipping a live bug). Worst specimen: `supersedeDocument`'s doc telling the frontend to upload *"via `addDocumentVersion`"* — **a deleted verb, in the doc of a verb they actively call**. Invisible to typecheck, eslint and all five gates. ⚠ **Lead-ruled 2026-08-13: deliberate TOMBSTONES stay** ("X is gone, use Y") — silence sends a reader who remembers `X` looking for the wrong file, so deleting that to make a check pass would worsen the codebase to satisfy the gate. **The gate's spec must therefore carry a machine-checkable tombstone convention**; the 4 existing tombstones are **not** to be churned before the gate exists. Body + discharge: [follow-ups.md](docs/progress/follow-ups.md) — lead/PO
