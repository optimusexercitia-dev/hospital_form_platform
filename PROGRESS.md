...
`tester`'s own `/padr\S+/`-tolerant regexes (deliberate while the wording was open, so the spec would not encode the wrong spelling) were **replaced with exact-literal assertions** citing the PO ruling — `phase16-accreditation-core.spec.ts` AC-1, both the `1 de 2` and `0 de 0` cases — since a tolerant matcher can't red a regression once the correct wording is settled. Swept all 5 specs for any OTHER assertion loosened around this bug or its `evidence-count-badge.tsx` sibling: none found (no spec fixture ever produces an `atencao` count > 1 on the aggregate badge, so no assertion was ever positioned to observe that half of the bug either). Re-verified: `core.spec.ts` + `freshness.spec.ts` 15/15, all 5 specs combined 30/30, `--project=chromium --workers=1`.
...
around either bug (none found). Re-verified `core.spec.ts` + `freshness.spec.ts` 15/15, then all 5
combined 30/30, `--project=chromium --workers=1`. Committed
`test(e2e): pin the PO-ruled readiness wording (BUG-P16-005)`.
