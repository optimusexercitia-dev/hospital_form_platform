...
>   says *"Use the Storage API instead"*). **The operative locks are the two ABSENT policies, SELECT and
...
| --- | --- | --- | --- |
| **DM5 · S4 — legacy storage-bucket retirement** (ADR [0120](docs/decisions/0120-dm5-wave-d-retirement-decisions.md) D8/D9) — ⚠ **SLICE review; DM5 phase QA still owed at S6** — **r1 ⛔ → r2 ⛔, r3 OWED** | ⛔ **CHANGES REQUESTED (r2)** — **B1 ✅ and B2 ✅ both fully discharged** (R15 re-pointed, and confirmed *executed* green in the gate at `phase14c-rca.spec.ts:736`); four authz ARMs **re-measured at HEAD**, not inherited (census 546/570 · wrapper 41 ⊆ allowlist · hat 3 · floor 74). New: **0 P0 · 1 MAJOR blocking · 4 MINOR · 4 INFO**. ⛔ **B3** — the B2 fix volunteered a finding that is **inverted on the product path**: *"`storage.protect_delete` is the OPERATIVE guard"* is now a **passing pgTAP assertion's label** (`143_capa.sql:313`), an R15 comment, `PROGRESS.md:169-171` and the S5.R plan, but the **Storage API sets the bypass GUC itself** — with the two RLS locks opened, the same authenticated HTTP DELETE returned `200 Successfully deleted` (measured). The label **demotes the only lock there is**. Record defect again; **no code or assertion change requested** | 2026-08-17 | [r2](docs/reviews/dm5-s4-review-r2.md) · [r1](docs/reviews/dm5-s4-review.md) |
