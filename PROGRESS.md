...
| Date | Run | Result |
| --- | --- | --- |
| 2026-08-21 | ⭐ **CASE SPLIT · Increment 1 — §6 steps 1+2** | **GATE GREEN, exit 0.** E2E **1176 p · 0 f · 2 flaky · 11 skip · did-not-run 0 · 1189 collected**; census **sums**, verified per-batch. pgTAP **6795** F=206 · lint(8) **0** · `tsc` **0** · vitest **1506** · 4 ARMs **HOLD**. ⛔ QA+PO outstanding → [archive](docs/progress/test-run-archive.md) |
...
- 🟡 **FUP-CASE-TAGS-AND-OUTCOME-SELECTOR-NO-E2E** — two case-wide write affordances (case tags, outcome selector) have **zero E2E on any route**, so Increment 1 relocated both with nothing asserting either side. A **coverage gap, not a stale assertion** — recorded so the Step-0 sweep's “0 additional” cannot read as “covered” — tester
