...
- 🔴 **FUP-DIFF-SCOPED-SWEEP-IS-HALF-AIMED** — the sweep CLAUDE.md §6 step 1 mandates every phase has a **three-part hole**, all three measured during AE1.5. **(1)** `scripts/door-sweep-cases.sh` derives **read AND write** policies but its paste-able command names only `p0-authz-door-audit.sh`, so an operator following the deriver’s own output sweeps half the list (22 of 52 unmatched). **(2)** ⛔ arm 2 (`p0-authz-writepath-audit.sh`) computes `(COVERED = the rest)` over an **empty set** and exits **0** having measured nothing — the `ARM=census` failure **recurring in a second harness**, where the sibling arm returns exit 3 UNPROVEN for the identical case. **(3)** **9 write policies sit in NEITHER arm’s domain.** ⚠ Surfaced only because the *read* harness refuses to end CLEAN with unmatched cases → [body](docs/progress/follow-ups.md) — backend/lead
