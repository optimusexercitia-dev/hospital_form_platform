...
> ### ✅ ROTATED 2026-08-14 — 154 KB → ~55 KB, discharging the OWED banner this file carried
>
> Every teammate spawn reads this file, so §7 targets "well under 60 KB". The three cuts the banner
> named were taken, plus five more — **each preserved verbatim in its archive first**, so this is
...
> mutation harness**) · 🔴 **FUP-DM5-STORAGE-ORPHANS** (**blocks S4**; method half ruled, remote half
...
| --- | --- | --- | --- |
| **DM5 · S3 — printed renditions onto the core substrate** (ADR [0120](docs/decisions/0120-dm5-wave-d-retirement-decisions.md) D1/D6/D7/D11/D12/D13/D17/D18) — ⚠ **SLICE review, not the phase gate; DM5 phase QA is still owed at S6** | ⛔ **CHANGES REQUESTED (r1)** — 0 P0 · **2 MAJOR (both blocking)** · 6 MINOR · 2 INFO. ⭐ The build is sound; both MAJORs are assurance/record fixes and **neither blocks S4 on behaviour**. Both discharged + four MINORs at `af9a894e` — ⏳ **r2 NOT YET RUN, so S3 has no APPROVED verdict** | 2026-08-14 | [dm5-s3-review](docs/reviews/dm5-s3-review.md) |
...
- 🔴 **FUP-QOB-3** — `dispose_event_phi` is the **only** Rule-12 disposal door still granting a bare tenancy admin; two of three PHI modules deny that tier, one grants it, purely by ruling order — PO, then backend
...
- 🟡 **FUP-VACUOUS-COVERAGE-1** — `phi-remediation` REM-8/REM-9 are honest `test.skip()`s, **outside** the vacuity property, so `lint:vacuous` will never catch them — tester/backend
