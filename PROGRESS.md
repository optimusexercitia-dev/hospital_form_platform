...
> Full task ledger + backend surface + the `e2e:prod` gate triage → **[docs/progress/ch-charters-cadence.md](docs/progress/ch-charters-cadence.md)**. QA → [phase-CH-review.md](docs/reviews/phase-CH-review.md).
...
| CH-BE-5 | `charters.ts` impl + regen types + seed (charter rows, a published regimento doc, meeting dates spanning the 4 states) | backend | ✅ `13750b1` — **lead-verified live**: 4 bodies wired (table read + 3 RPCs, camelCase, `mapCharterError`); `database.ts` regen'd from `--local`, **0 pgtap artifacts**; seed `charters`=ON + all 4 cadence states independently recomputed correct + regimento `d0c…f1` (effective, review-due 2027-06-20) linked to Farmácia. tsc/lint 0/0; full pgTAP 3623 (8 baseline). **Backend track CH-BE-1..5 COMPLETE.** |
| CH-FE-1 | `manage/charter` page (frequency + regimento link/create + cadence badge) | frontend | ✅ **lead-verified** — route `o/[org]/c/[commission]/manage/charter` (page+actions+loading+error), flag+staff_admin gated (mirrors `manage/documentos`); nav item "Regimento & Cadência" gated on `chartersEnabled()` (sidebar + commission layout). Single-form upsert (frequency + regimento id together → one `upsertCharter`, no clobber). `CadenceStatusBadge` (server, pure) + `CharterForm` (client, `useActionState`, type-only action import). Frequency/cadence pt-BR labels in `components/charters/labels.ts`. Regimento link via `listDocuments(…,{docType:'regimento'})`; create-new hands off to `documentos/novo?docType=regimento` (added additive `defaultDocType` prefill to the shipped Phase-17 editor — edit path unchanged). tsc/lint 0/0. **Preview-verified live** (dev): Farmácia `em_dia` (badge "Em dia", Mensal, regimento review-due 20/06/2027) + CCIH `sem_regimento` (empty states, placeholder freq); no console errors. Note: regimento `code` renders **DOC-0001** (actual DB value) not REG-0001 — seed/code-gen artifact, FE faithful. **Interactive save deferred to CH-TEST** (pane input flaky + a real save would mutate CCIH's `sem_regimento` seed fixture). — *Lead-verified*: Phase-17 doc-editor diff confirmed additive (optional `defaultDocType`, edit path unchanged, default stays `pop`); tsc + lint independently re-run 0/0; DOC-0001 code is the faithful auto-gen value → CH-TEST asserts regimento title/review-date, not the literal code. |
| CH-FE-2 | Meetings-list cadence indicator + schedule-flow carry-forward step | frontend | ✅ **lead-verified** — **Indicator:** `CadenceStatusBadge` (reused from CH-FE-1) in the meetings-list header, sourced from `getMeetingCadenceStatus` (RPC, not the visible rows), gated on `chartersEnabled()`, `null`→omit. **Carry-forward:** new `components/charters/carry-forward-panel.tsx` (client) rendered on the meeting **detail** page after `AgendaPanel` (the post-create nav target = "the schedule flow's shape"), gated `chartersEnabled() && canEdit`. Agenda items → checkboxes; ticked copied onto THIS meeting via the **existing** `createAgendaItem` (`create_meeting_agenda_item`) door looped client-side (no new backend door; originals untouched); self-source filter (`sourceMeetingId===meetingId`) so a source meeting never suggests its own items. Action items → read-only "ainda em aberto" + pt-BR status (reused `ACTION_ITEM_STATUS_LABEL`) + due date. Empty state when both empty. tsc/lint 0/0. **Preview-verified live** (dev): indicator Farmácia **Em dia** / Farmácia B **Reunião em atraso** / CCIH **sem_regimento**; carry-forward panel renders on CCIH held meeting with a **read-only open action item** ("Atualizar protocolo… — Aberto · vence 03/08/2026 — ainda em aberto"); no console errors. **Seed carry-forward data:** an open meeting-sourced action item EXISTS (CCIH); but no commission has a **scheduled** (non-source) meeting, so the **agenda-copy checkbox render + copy round-trip is deferred to CH-TEST** (would need a scheduled meeting; not driven live to avoid mutating the shared seed). — *Lead-verified*: `createAgendaItem` confirmed a `'use server'` action (module opens `'use server'`), so the client panel's value-import is build-safe (not the BUG-FBE-005 queries-module trap); tsc + lint independently 0/0. |
| CH-TEST | `e2e/charters-cadence.spec.ts` (§9) + one keyboard flow. **Carry-forward: exercise a literal `case_restricted`/`participants_only` action item ABSENT from the suggestion end-to-end** (CH-BE-3 pgTAP proved the filter mechanism via an `assignees_only` item; this is the integration-level confidentiality proof). Also prove the two deferred round-trips: charter save (freq + regimento link) + agenda-copy onto a new scheduled meeting (originals untouched). | tester | ✅ **10/10 chromium, 2× green** (fresh-reset run + a re-run on the same post-run DB with no reset — proves the file's own `beforeAll`/`afterAll` fixtures + AC-4's restore-to-seed are deterministic). AC-1a–d cadence indicator all 4 states (meetings list); AC-2 regimento title + review-due date (fetched live and formatted in-spec, NOT hardcoded — the date is `current_date`-relative at seed time; asserts title/date only, never the `DOC-0001` code per the plan's stale "REG-0001" note); AC-3 foreign-commission denial (Rede B user: RPC `meeting_cadence_status` → HC0K2 + route 404, response body asserted free of any status keyword); AC-4 charter save round-trip incl. **keyboard-only** (native-select type-ahead `mensal→bimestral`, `Enter` on the focused Save button, reload-persists, regimento link survives untouched, restored to `mensal` via the same UI door); AC-5 exactly-expected `charter.upserted` audit rows from AC-4, metadata keys `{has_regimento, meeting_frequency}` only (PHI-free); AC-6 carry-forward agenda tick+copy onto a new scheduled meeting — DB-verified (not just rendered) that the 2 new rows land on the new meeting and the 3 source rows (2 unresolved + 1 resolved) are byte-for-byte untouched — plus the resolved item is never suggested and the open action item renders read-only "ainda em aberto"; AC-7 confidentiality filter **exercised via the `case_restricted`+`case_id` branch** (a meeting-sourced item cross-linked to an `explicit_grants_only` case with no grant — the branch CH-BE-3's pgTAP did NOT cover, which only used `assignees_only`) — absent by id AND title for a denied plain member's real RPC call, with a `committee`-scope item present as the positive twin (not a blanket-empty response). Fixtures built service-role in-spec (mirrors ethics-e2's pattern); no seed addition needed. **0 bugs filed** — every assertion passed against the live app on the first authoring pass; RPC/schema shapes verified against the live local Postgres catalog (`docker exec … psql` + `pg_get_functiondef`), never migration-file text, per the CLAUDE.md graphify-does-not-index-SQL exception. — *Lead-verified*: commit `14c4381`, 10 tests confirmed, `next` 16.3.0-preview.5 (no version drift → correct prod-build gate). Full `e2e:prod` declare-green run launched. |
| CH-QA | Requirements + RLS conformance (`set local role`) | qa | ✅ **APPROVED** (2026-07-20, r1) — 0 P0/0 MAJOR/0 MINOR/3 info. Live `set local role`: member 1 row / foreign 0 / non-member 0; 1 SELECT policy + no write policy + `authenticated` SELECT-only; 3 RPCs t19-clean (prosecdef/owner-postgres/search_path/EXECUTE auth+svc, no anon). Mutation KS_AUTHORITY/KS_MEMBER/KS_FILTER **RED-PROVEN** + control green; pgTAP 260/261/262 = 11/29/10. D1–D11 + §9 met; PHI-free; BUG-FBE-005 avoided; phase17 `cb6a671` spec-only sound. → [review](docs/reviews/phase-CH-review.md) |
| Record | PROGRESS + `backend-state.md` + reconcile accreditation-track §21 + graphify `update .` + `phase(CH): complete` | lead | ✅ 2026-07-20 |
...
**✅ CH-TEST-fix complete (tester, 2026-07-20).** Two `phase17-documents.spec.ts` locator fixes, both the SAME root cause (DOC-0001 is no longer unique on the `/o/rede-a/manage/documentos` hospital-admin rollup, now that Farmácia's regimento is legitimately its own commission's DOC-0001 too — codes are per-commission, app behavior is correct):
- **AC-6 (the one flagged in the brief):** `getByText('DOC-0001')` → scoped to the `<tr>` that ALSO carries the "vencida" (overdue) chip, deterministically CCIH's política (Farmácia's regimento, review-due 2027, isn't overdue). Mirrors the existing `<li>`-based pattern a few lines above it, adapted to this page's `<tr>`-based table.
- **AC-10 (NOT flagged — found only by running the whole file, as asked):** same root cause, a different bare `getByText('DOC-0001')` immediately before an unambiguous `getByText('DOC-0002')`. Fixed by anchoring on the still-unique DOC-0002 row to locate CCIH's own per-commission `<table>`, then asserting DOC-0001 is inside that same table.

**`phase17-documents.spec.ts`: 14/14 green**, fresh-reset single pass (decisive — a same-DB *re-run without a reset* produced 2 different transient failures, AC-2/AC-13, that vanished on the next fresh reset; confirmed as re-run-without-reset pollution from my own validation loop, not real).

**`notifications.spec.ts`: 8/8 green** on a verified-healthy stack — but the FIRST attempt was **8/8 failures**, all in `getOwnerToken`/`signInAs` (password-grant `resp.ok()` false / login-navigation timeout), not the `openFreshCapaPlan` pattern the gate showed. A direct curl probe of `/auth/v1/token` confirmed genuine infra: **HTTP 502 on the auth endpoint itself**, even though `db reset` had reported success and the data was correctly seeded (psql-verified: 28 profiles, 3 charter rows, `charters` flag on). `supabase_vector` was still crash-looping (`Network unreachable` reaching the Docker socket) and at least once took the Kong/auth path down with it (502) despite `docker ps` reporting `auth`/`kong` "healthy". A second `stop`+`start`+reset, verified by a direct curl probe returning a real token *before* running Playwright, produced a clean 8/8 — **not a CH regression**. **Flag for the lead:** `supabase_vector`'s crash-loop is a standing stack-health risk beyond "reset failed, retry" — it can silently 502 requests through an apparently-healthy gateway (2 separate incidents this session, one auth-layer).

**Lead confirmation gate (2026-07-20) — done.** Both gate-#1 real-looking failures resolved (phase17 14/14 fixed · notifications 8/8 = infra). Rather than re-run the full 30-min gate against a still-crash-looping `vector`, re-running the **23 specs from gate-#1's reset-failed/collapsed batches** (b3/b4/b6/b10) on the healthy stack — combined with gate-#1's **299 stable-batch greens** (b1/b2/b7/b9/b11) + `charters-cadence` 10/10 (prod) + phase17 + notifications = full-suite coverage. Any `vector`-collapsed batch gets a targeted retry.

**✅ TEST-PASS GATE SATISFIED (2026-07-20).** Confirmation gate: batches 1–4 = **209p / 0f / 0 flaky** (16 specs, incl. every CH-adjacent one — `hospital-admin-tier`/sidebar, `meeting-held-time` + `phase10-meetings`/modified pages, form specs, `home`); the 7 that `vector`-reset-failed re-ran in a token-verified-healthy window = **66p / 0f / 0 flaky**. Combined with gate-#1's 299 stable greens + `charters-cadence` 10/10 (prod) + `phase17` 14/14 (fixed) + `notifications` 8/8: **every suite spec green across the runs; the lone regression (phase17 DOC-0001) fixed; all other reds were `vector`/infra ([[supabase-vector-crashloop-502]]).** No CH regression anywhere.

**✅ CH COMPLETE — human-approved 2026-07-20 (PO).** Phase-Gate steps 1–5 done: Build · Test (full suite green/triaged) · **QA APPROVED r1** · PO approval · Record. Local-only; origin push + Coolify + remote deploy + `charters` prod-ON deferred to the pilot. 3 QA INFOs accepted/by-design (non-blocking). **This closes S4 (ETH·E2 + RV2 R2–R5 + CH all done).**
