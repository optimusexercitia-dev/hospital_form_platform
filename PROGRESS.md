...
| **ETH·E4** | **Ethics participant seating & professional identity** [0108](docs/decisions/0108-eth-e4-participant-seating.md) · [plan](docs/phases/ethics-e4-participant-seating.md) · [locator contract](docs/phases/ethics-e4-locator-contract.md) — closes FUP-ETH-1 + FUP-FF5-2 | ⏸ **PAUSED 2026-08-11 — ⭐ RESUME FROM [docs/progress/eth-e4-handoff.md](docs/progress/eth-e4-handoff.md)**. Build complete; **nothing merged, nothing pushed** | ✅ lint 0/0 · typecheck · vitest 1218/1218 · `next build` exit 0 · pgTAP **5794/5794 ×2** · `ARM=hat`/`floor` HOLD · door sweep 0 BLIND/0 ERROR · write doors covered by the **neutralization oracle** + the standing write-path harness (⛔ never `ARM=census` — ADR 0079 Am. 5) | ⚠ full `e2e:prod` @ `7e55f01` **GREEN 1068/0 failed/0 unrun**; final run @ `2efa691` **RED (UNRUN ONLY)** — 999 passed · **0 assertion failures** · batch 16 died twice (`server_dead`) leaving **69 unproven**. ⛔ 4 of those 7 specs hit case-detail routes, which `45e68c5` just changed — **re-run batch 16 first** | **r1** 1 P0 · 6 MAJOR · 11 MINOR → **r2** CHANGES REQUESTED (narrow) → **all fixed; r3 NOT RUN** [review](docs/reviews/eth-e4-review.md) | ❌ not reached | – | 30 commits, branch only |
