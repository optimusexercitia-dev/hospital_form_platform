...
| A.8 | Case-page viewer branch + write-affordance suppression · oversight toggle in `/o/[org]/manage/comissoes` · nav/`qualidadeHref` | frontend | ✅ **built 2026-08-06** (`ae48b41` `c8379bd` `6ee5f47`) — gates widened at the **layout**, case page and narrative page; reduced `QualityViewerShell` returns before the member branch (the member sidebar never renders, so `role === null`'s show-everything branch is unreachable). 3 flag-gated write affordances closed via one `viewerKind` discriminator + host-side resolution: `NotifyEventDialog`, `correctionCaps.canFile`, `CasePatientPanel`. **Q3** → referral list kept, wizard fuel emptied by construction (`buildCaseReferralsModuleReadOnly`); **Q4** → interviews + meetings omitted; **Q1** → documents panel renders metadata, **no download affordance at all** (`bf0f824`). Standard-tier bytes are DB-denied at the storage policy by **M8**; reopening is Phase B+ with an audit emit. ⚠ **Two findings worth carrying forward:** ① `signedUrl: null` is **overloaded** — it meant only "PHI tier → audited door", and M8 added "storage refused to sign for you", so both nulls hit the same else-branch and the reviewer was being offered `OpenAttachmentButton` on *every* document (an affordance, not an absence). ② **M8's storage policy does not govern that door**: `openAttachment` resolves bucket+path via the `open_attachment` RPC then signs with the **service-role** client, which never consults `storage.objects` policies — so reachability rests entirely on `open_attachment`'s own in-body gate (**unverified; raised with backend** — the BUG-AUTHZ-001 shape, where a `prosecdef` door's gate replaces RLS). Not a live leak: the affordance is gone. Oversight toggle + `OrgCommissionList` un-nested (card was one `<Link>`). `ROLE_LABELS` label landed, role-label chain green. ⏳ **authenticated in-browser render not verified by `frontend`** — entering a password is outside this agent's permitted actions; needs a human sign-in or tester's A.9 run |
| A.9 | E2E `e2e/quality-oversight.spec.ts` + keyboard-only flow | tester | ✅ 2026-08-07 — new spec, 15/15 GREEN (2 consecutive runs, chromium/prod-standalone), reach-not-row-count discipline throughout (every negative paired against a real coordinator/org_admin on the SAME URL). 3 spec-authoring bugs found+fixed by re-run (not product defects): 2 Playwright strict-mode multi-match locators (the case page legitimately has TWO "Escritório da Qualidade" back-links — shell header + `CaseDetailView`'s own `backHref` slot; "Editar" also matches 4 unrelated per-narrative buttons, scoped to `header`) + 1 wrong not-found assumption (a `notFound()` thrown by `qualidade/layout.tsx` itself is caught by the GLOBAL `not-found.tsx`, not the console's own — Next.js boundary rule: a segment's co-located not-found.tsx can't catch its own layout's notFound()). `/manage/comissoes` locator sweep (`hospital-admin-tier.spec.ts` + `phase-multitenancy.spec.ts` + `phase3-admin-members.spec.ts`, 83 tests): **0 regressions** — none of the three specs key on the whole-card accessible name the `OrgCommissionList` restructuring narrowed, so the flagged re-scoping risk did not materialize. 2 findings reported to lead (not filed as bugs — no security/functional impact): (1) the qualidade-scoped `not-found.tsx` copy is effectively unreachable (its only caller, the layout, always resolves the same `cache()`-wrapped check first); (2) `QualityCommissionFilter` (the chip row) never mounts under the current seed — no persona reviews >1 oversight-visible commission at once, so the chip UI is untested by construction. Full inventory + repro notes in this session's report to lead. |
