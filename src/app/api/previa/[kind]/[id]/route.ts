...
const KINDS = new Set(['form_response', 'meeting'])
...
export async function GET(
  ...
  let pdf: Buffer
  try {
    // Under the CALLER'S session. Both providers throw a pt-BR error when the
  ...
  return new Response(new Uint8Array(pdf), {
    ...
    headers: {
      ...
      // prévia is a stale page the platform cannot invalidate — and for the
      // meeting kind it may carry PHI.
