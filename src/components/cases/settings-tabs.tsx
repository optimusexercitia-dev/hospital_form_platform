...
/**
 * Sub-navigation for the commission settings area (Cases-Extras): switch between
 * the case-status vocabulary and the tag vocabulary. Keyboard-operable links with
 * `aria-current` on the active tab.
 */
export function SettingsTabs({ slug }: { slug: string }) {
  const pathname = usePathname();
  const tabs = [
    { href: `/c/${slug}/manage/settings/statuses`, label: "Estados" },
