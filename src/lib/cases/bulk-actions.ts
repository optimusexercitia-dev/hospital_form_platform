...
function mapError(error: { code?: string; message?: string } | null): string {
  ...
  // The RPC (and every door it composes) raises user-facing pt-BR; prefer it.
...
export async function bulkCreateCases(
  ...
  // database.ts is regenerated by the lead after this migration applies; until then
  // the RPC is absent from the generated types union. The SQL signature (verified
  // against the LIVE catalog) is the contract, so cast the untyped call rather than
  // regenerate. `supabase.rpc` is invoked DIRECTLY (not via an extracted variable)
  // to preserve the client's `this` binding; only the fn name/args/result are cast.
  const { data, error } = (await supabase.rpc('bulk_create_cases' as never, {
