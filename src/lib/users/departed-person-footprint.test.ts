...
/**
 * AFF3 / ADR 0145 — the WRITE half of the offboarded-person change.
 *
 * ⭐ WHAT THIS FILE IS FOR. Migration 20261003002200 widened READ visibility so a
 ...
 * `id = auth.uid()`), so `368_offboarded_person_visibility.sql` §4 can only prove that the
...
// ===========================================================================
describe('§3 the read/write boundary ADR 0145 turns on', () => {
  /**
   * The one-sentence statement of the change, as an executable pair. READ widened (proven
   * in pgTAP 368 §1); WRITE did not (here). Both halves in one place so a future reader
