...
> middleware has never run against Supabase Cloud). **Only Phases 18–19** are then sequenced
> *after* the pilot so they're informed by real-user feedback — **Phases 20–21, Referrals v2,
...
> [0071](docs/decisions/0071-pre-pilot-release-scope-expansion.md)).** Rationale: ADR
...
remaining pre-pilot phases build in order 15 → 17 → 16** (the P0
core now includes document control), **plus the twelve initiatives pulled pre-pilot 2026-07-12
(ADR [0071](docs/decisions/0071-pre-pilot-release-scope-expansion.md)) — Phases 20–21, Referrals
v2, Interviews v2, Ethics E1–E3, action-items satellites + cross-link, §6.1, supersession engine —
plus the five Flexible-Forms feature phases FF-1…FF-5 pulled pre-pilot 2026-07-27 (ADR
[0086](docs/decisions/0086-flexible-forms-pre-pilot.md));
pilot after that whole block**, then sequence **18–19** on pilot feedback — see ADR
...
| 14e | Centralized Attachment Substrate *(planned; supersedes the narrow 14e PHI-classification plan; now built as **phase F2** of the [Pre-Pilot Foundations Program](docs/plans/pre-pilot-foundations-program.md) — **after** case-participants (F1), because `attachment_subjects` references the `participants` registry; see [phase-14e](docs/phases/phase-14e-attachment-phi-classification.md) + ADR [0063](docs/decisions/0063-centralized-attachments-substrate.md))* | `attachments` |
...
| 18 | Self-Assessment, Internal Audit & Mock Tracer *(**post-pilot**)* | `internal_audit` |
| 19 | Surveyor Access & Evidence Export *(**post-pilot**)* | `surveyor_access` |
| 20 | Notifications & Escalation *(**pulled pre-pilot** 2026-07-12 — ADR [0071](docs/decisions/0071-pre-pilot-release-scope-expansion.md))* | `notifications` |
| 21 | Committee Charters & Meeting Cadence *(**pulled pre-pilot** 2026-07-12 — ADR [0071](docs/decisions/0071-pre-pilot-release-scope-expansion.md))* | `charters` |
...
| NSP-per-org | **NSP-per-org** *(follow-up that lifts the multi-org PHI guard)* — the NSP/patient-safety + inter-committee **referral** PHI modules were **inert in any multi-org deployment** (their single global `pqs_members`/QPS roster couldn't safely span orgs — `app.is_pqs_member` returned false when `is_multi_org()`; modules absent in the 2-org seed; 124 E2E specs quarantined). This phase re-scoped the roster + event/referral PHI **to an organization** (per-org PQS roster, org-bound DEFINER doors + audit), restoring both modules under multi-tenancy. Superseded by the later **nsp-per-hospital** structural phase, which re-keys the same roster/doors org→hospital. ADR [0042](docs/decisions/0042-nsp-per-org.md); ADR [0041](docs/decisions/0041-multi-tenancy-organizations-hospitals.md) §Implementation amendments 10 + 13. | ✅ complete 2026-06-25 |
| nsp-per-hospital | **NSP-per-hospital** *(Phase B — re-keys the PQS roster + every PHI door one level finer)* — moves the org-scoped PQS roster + patient-safety/referral PHI doors from `NSP-per-org` down to **per-hospital** (`is_pqs_member_of(hospital)`), alongside the `hospital_admin` tier introduced in the same wave; adds `nsp_org_admin` (org-level, zero-PHI rollups + roster curation + coordinator appointment) while `nsp_coordinator` becomes the full local (hospital-scoped) operator; enables dual-hospital same-org referral reads and `dispose_referral_phi`. ADR [0052](docs/decisions/0052-nsp-per-hospital.md); design [nsp-per-hospital-design.md](docs/progress/nsp-per-hospital-design.md). | ✅ complete 2026-07-03 |
| DB-hardening | **Pre-pilot DB hardening program** *(consolidated remediation of the 2026-07 external DB audit — critical set + §4/§5 table)* — one gated Wave-1 push covering **C-1** (audit_log TRUNCATE/DML), **C-2** (default GRANT ALL), **C-3** (membership self-escalation → the write-path lockdown, item #1), **C-4** (forgeable audit rows — fixed by an **entitlement guard**, *not* a revoke: 6 legit app callers + server-derived actor), **C-5** (`answers` has no form FK), **C-6** (incomplete PHI disposal → LGPD erasure untruthful), **H-8** (cross-hospital CAPA write), plus the §4 performance (P1–P10) and §5 data-model (D1–D4, D6-flip, D7, D9) items. Three phase-sized refactors deferred to their own plans (Wave 3): patient master (§6.2), full metadata-driven item types (§6.3), and the full single-`memberships` collapse (§6.1). **Structural — no feature flag**; pre-pilot, reset-OK. Program plan: [docs/plans/pre-pilot-db-hardening-program.md](docs/plans/pre-pilot-db-hardening-program.md) (WS-1 detail: [docs/plans/membership-write-path-lockdown.md](docs/plans/membership-write-path-lockdown.md)); triage: [docs/reviews/external-db-audit-2026-07-evaluation.md](docs/reviews/external-db-audit-2026-07-evaluation.md) + [§4/§5 analysis](docs/reviews/external-db-audit-2026-07-perf-datamodel-analysis.md). | ✅ **Waves 1+2 complete + deployed 2026-07-05** (merged → main `27d9e5f`); **Waves 3+4 dispositioned 2026-07-10 into the [Pre-Pilot Foundations Program](docs/plans/pre-pilot-foundations-program.md)** — D12 closed, D5/§6.3 superseded/cancelled, D3/WS-8 → F-cleanup, P6/P7/§6.1 stay deferred |
| flexible-forms | **Flexible-Forms Foundation** *(disposition of the 45-item partner forms-model gap review — which to adopt now vs. later vs. drop)* — pre-pilot **create-now bones**: widen the `form_items.item_type` CHECK to reserve `group`/`repeating_group`/`matrix`/`risk_matrix`/`reference` (CHECK-enum kept, no data-driven catalog); `form_item_options.is_exclusive`+`risk_weight`; `form_versions.behavior_config`; **evaluator operators** `contains`/`not_contains`/`is_empty`/`is_not_empty` (dual SQL↔TS + golden parity, Rule 3); **repeating-group schema shape only** (position-uniqueness constraint; the write RPCs + resume plumbing move to FF-1, Rec C); `form_item_validations` reserved. Committed field types (repeating groups, matrix/grid, risk matrix, entity reference) + validation engine + power-authoring were deferred to post-pilot phases **FF-1…FF-5**, DB prepared — **then pulled pre-pilot 2026-07-27 (ADR [0086](docs/decisions/0086-flexible-forms-pre-pilot.md))**. **File-upload / PHI-attachment answers dropped 2026-07-07** (broad PHI ingress; evidence attaches via the document/evidence layer). **Structural — no feature flag**; additive, pre-pilot reset-OK; independent of Phase 16 but **sequenced after it, before the pilot reset** (confirmed 2026-07-07). ADR [0060](docs/decisions/0060-flexible-forms-foundation.md) (extends [0045](docs/decisions/0045-answer-model-v2.md)/[0046](docs/decisions/0046-forward-compat-form-capabilities.md)). | ✅ **Foundation complete (F3) 2026-07-12** — QA APPROVED (0B/0M/0m · 4 INFO, [review](docs/reviews/phase-F3-review.md)); merged → main `94f03c3`; remote-deployed on the pilot reset. Ships the pre-pilot **bones only** (mostly inert; the one live feature = the 4 dual-evaluator operators). **Feature phases FF-1…FF-5 — 🔜 pending, PRE-pilot (re-sequenced 2026-07-27, ADR [0086](docs/decisions/0086-flexible-forms-pre-pilot.md)): order FF-1 repeating groups → FF-2 matrix/risk → FF-3 validation (+`required_if`) → FF-5 entity reference (3 lanes) → FF-4 power authoring (library + defaults; calculated fields stay post-pilot); all five gate the pilot deploy → [program](docs/plans/flexible-forms-program.md).** |
