...
reviews: ["../reviews/ae5-role-catalog-compat-review.md"]
adrs: ["0197", "0207", "0208"]
handoff: ~
---
...
- [x] **AC-1 — step 1, the column.** `app.active_role_selections.role` is `text` (not the enum),
...
- [x] **AC-2 — step 2, the door.** `public.assume_role` is **ONE** routine, signature
...
- [x] **AC-3 — step 3, the enum.** `to_regtype('public.platform_role') IS NULL`; the `pg_depend`
      dependents are zero before the drop (the migration asserts it, not assumes it).
- [x] **AC-4 — step 4, the row and the domain.** `authz.roles` has no `administrativo` row (11
...
- [x] **AC-5 — step 5, the TypeScript.** `src/lib/role/role-catalog.ts` holds ONE ordered
      ...
      green.
- [x] **AC-6 — D4's generated pin, both halves.** `411`'s hand-maintained `MANIFEST-SNAPSHOT`
...
- [x] **AC-7 — step 6 NOT taken, proven.** `md5(pg_get_functiondef(...))` of
...
- [x] **AC-8 — the name-keyed sites.** Every artifact keyed on the signature
...
- [x] **AC-9 — gates.** Fresh `supabase db reset` + `npm run test:db`; `npm run lint` (all gates,
