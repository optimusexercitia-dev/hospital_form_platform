...
- QA reviewed run 6: CHANGES REQUESTED, corrected and every gate re-verified from scratch — 0 over-grants found.
...
- The `authz.scope_reaches` fix is folded back into this branch and verified (ADR 0180) — no separate branch or hub.
- Formal QA re-review of the IA-F9 statement-scoped corrections — self-verified, not yet re-reviewed by `qa`.
- Rollback runbook §6 worked example — still staged, PO-deferred until AE4 concludes and merges.

### Next

1. Fix `BUG-AE49-D6-REKEY-INCOMPLETE` (4 of 7 sites done) — matrix at `docs/design/authz-ae43-staff-admin-permission-matrix.md:291`.
2. QA re-review of the statement-scoped corrections; close F-BLOCK-3/F-MAJOR-1 on the broad Gate AE4 review.
3. One final `e2e:prod` run on a quiet machine, batching every remaining change into that window.
4. PO approval; Record step (AE4 row → `phase-ledger.md`, delete the handoff).

### Blockers

- `BUG-AE49-D6-REKEY-INCOMPLETE` still OPEN — `commission.forms.edit` re-keyed at only 4 of 7 sites.
- The broad Gate AE4 review stays CHANGES REQUESTED — F-BLOCK-1/2/3, F-MAJOR-1 filed, untouched.
- `e2e:prod` still owed since the single 2026-09-02 run; certain once the re-key lands (a migration).
