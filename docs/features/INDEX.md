...
**18 hubs** · in progress 1 · gated 3 · planned 2 · parked 0 · complete 12

| ID | Title | Status | Kind | Program | Branch | Hub |
|---|---|---|---|---|---|---|
| CAN-MANAGE-PROFESSIONAL-SELF-CHECK | `app.can_manage_professional`'s self-check arm — a third-party predicate whose first arm answers about the caller, given its reachability analysis, the PO's ruling, and (if ruled) the migration that makes it answer about `p_uid` (pre-AE5 Batch 8) | 🟢 in progress | feature | AUTHZ | `authz-can-manage-professional-self-check` | [can-manage-professional-self-check.md](can-manage-professional-self-check.md) |
