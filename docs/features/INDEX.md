...
**23 hubs** · in progress 1 · gated 3 · planned 3 · parked 0 · complete 16

| ID | Title | Status | Kind | Program | Branch | Hub |
|---|---|---|---|---|---|---|
| ADMIN-ARM-IS-ACTIVE | The admin arm follows the subject's state — `app.is_active` on `app.is_admin()`, `app.is_admin_for()` and `public.assume_role`, each RED-first; the `platform_admin` Class-2 write arm removed and relocated; R10's role-only audit stamp built with `315:212` rewritten; the four expected reds re-ruled, never silenced (pre-AE5 Batch 10 — the fix Batch 9 decided and deferred) | 🟢 in progress | feature | AUTHZ | `authz-admin-arm-is-active` | [admin-arm-is-active.md](admin-arm-is-active.md) |
...
| AE5-MATRIX-ARM3-CELLS | Arm-3 divergent cells enumerated — discharging ADR 0175 D3's forward promise, which is WORK and not a decision: 216 candidate rows carry zero divergence labels | 🔜 planned | feature | AUTHZ | — | [ae5-matrix-arm3-cells.md](ae5-matrix-arm3-cells.md) |
