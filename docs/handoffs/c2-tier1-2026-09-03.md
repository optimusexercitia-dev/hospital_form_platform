...
created: 2026-08-31
updated: 2026-09-03
expires: 2026-09-17   # branchless form (handoff skill): the local branch was deleted 2026-09-03; if C2's next increment has not started by then, promote § Dead ends and delete this file
status: live
---
...
⚠ **2026-09-03: the local branch `authz-c2-tier1` was deleted** — its 13 sweep commits reached
`main` through `authz-ae4-catalog` (folded at `3b21826b`, fast-forwarded into `main`, itself then
deleted). **Resume from `main`; cut a new branch for the next C2 increment.** `origin/authz-c2-tier1`
still exists on the remote and is stale; deleting it is a push, which is not yet authorized.

1. `git switch main` (⛔ NOT `c2-tier1-neutralizer`, NOT `authz-c2-tier1` — neither exists locally;
   everything both carried is on `main`)
...
5. Read **[the C2-TIER1 hub](../features/c2-tier1.md) § Current state** — it, not this file, is status truth
...
- ✅ **CLEARED 2026-09-01 — the AE3 schema-first cutover is DISCHARGED** (push → catalog-verified on the remote → Coolify green → §3 smoke PASSED; see docs/progress/2026-Q3.md). ⛔ Two operator obligations remain and leave NO artifact in the tree: rotate the remote DB password, and destroy `~/ae3-preimage.csv.gpg` **together with** its passphrase. The text below is kept as the record of what the blocker WAS.
