# 0147 — The audit organization is derived from the hospital, and leg 5 means the platform chain

**Status:** Accepted · 2026-08-25
**Supersedes:** nothing. **Amends:** 0146 (D4 — leg 5 as a deliberate non-decision).

## Context

ADR 0146 made `audit_log.organization_id` **authorization-load-bearing for the first time**:
...
The first row falsifies ADR 0146's Consequence that "an org_admin's audit reach is now a
...
A35). Test `369` §5.2 asserts exactly this cannot happen and passes anyway, because its
fixture guarantees a non-null organization; 369 §0.1 even *states* that guarantee as a
...
2. **It is a `coalesce`, not an override — derivation is added, validation is not.** An
   ...
   behaviour in test 370 §1.3 so the absence of validation is a recorded choice rather than
   an assumption.
...
4. **Leg 5 gains `hospital_id IS NULL`. This amends ADR 0146 D4** — which froze leg 5 as a
...
6. **Test 369 §6.4 was rewritten, and the replacement is strictly stronger.** It asserted
   ...
   | mutation | result |
   | --- | --- |
   | restore leg 4's `hospital_id IS NULL` | **369 §6.4 reds** (+ §1.1, §1.2, §1.3, §2.2, §3.2) |
   | remove leg 5's `hospital_id IS NULL` | **369 §6.8, 370 §4.2, 370 §5.1 red** |
...
- **A malformed hospital-tier row now fails CLOSED for everyone.** If `app.org_of_hospital`
  ...
  370 §4.3 so a future reader knows D4 is a confidentiality fix and not a redistribution of
...
- **The chain is untouched, and this was proven rather than argued.** Chain membership for a
  ...
  `v_org is not null`, so a now-non-null `v_org` cannot re-tier a row. Test 370 §3 measures
...
- **⭐ A fixture can be *more* well-formed than the platform, and that is invisible by
  construction.** 369 §0.1 asserts its hospital-tier row carries a non-null organization —
