...
| **D8** | A rollback section is **re-measured at the tip of the change that moves its counts**, and an interim banner is deleted rather than left beside corrected text. A live-twin cross-check has an **expiry**, and this ADR records that the `commission_of_version` twin **has already expired** — measured 2026-09-07, zero live policies carry that shape, so all four of those sites are record-only. |
