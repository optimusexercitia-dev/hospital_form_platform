...
> ⬅ **A later ADR changes this one** — it is **amended by** [0134](0134-case-surface-split-and-administrativo-case-read.md), [0153](0153-subset-sweeps-write-to-scratch-not-the-committed-baseline.md), [0157](0157-dominance-grid-population-bounded-by-schema.md), [0173](0173-door-sweep-deriver-blind-to-runtime-rewrite-migrations.md), [0174](0174-authz-holds-role-chokepoint-and-authoritative-state-gate.md), [0190](0190-the-door-sweep-deriver-selects-by-property-and-a-full-run-merges.md), [0191](0191-the-door-arms-domain-gains-a-schema-axis-a-targeted-home-and-a-fourth-outcome.md).
...
⛔ **The recipe is a SCRIPT — [`scripts/door-sweep-cases.sh`](../../scripts/door-sweep-cases.sh)
— and no longer this code block.** It was prose for three weeks, and inside that window
Amendment 8 ruled three changes to it that never arrived, so a second operator re-derived
the entire diagnosis by hand during AFF3 (`FUP-DOOR-SWEEP-RECIPE-STILL-BLIND-TO-ALTER-POLICY`).
...
- greps **`alter policy` alongside `create policy`** (Amendment 8 ruling 1). Measured on
  AFF3's own commit range: the old one-liner derives **0 rows**; the script derives the
  **3** policies AFF3's operator had to add by hand — `profiles_admin_select`,
  `profiles_select_self_or_admin`, `professional_credentials_select`.
- **Reds on a zero-row case list** when the diff touched `supabase/migrations/` (ruling 2),
  with a **different exit code** from "no migration in the diff at all" — so *"the recipe
  printed nothing"* and *"the phase changed no gate"* are no longer the same observation.
  There is deliberately **no acknowledgement env var**: an escape hatch for the unmeasurable
  also silences the measured. Exit 1 is an *obligation* (name the gate, or state in the gate
  record that the migration contains none), never a build break — do not wire it into `lint`.
- **Names the verdict an `ALTER POLICY` invalidates** (ruling 3): it looks the altered policy
  up in the committed findings file and prints what that row currently claims, so it cannot
  be read forward. Amendment 8 said *"until the tooling detects that, the operator names the
  altered policy in `CASES=` explicitly"* — it detects it, and puts it in `CASES` itself.
- Covers **working-tree and untracked** migrations, not only `<base>..HEAD`. A phase
  mid-flight has its migration uncommitted or untracked, so a committed-range-only recipe is
  blind precisely during the phase it exists to gate.
- **Partitions non-name-matching functions into a printed review list** rather than dropping
  them silently (see the ⚠ closing note of Amendment 8 — for an *altered* gate the name
  filter has no backstop), and **lifts the arm's domain regexes out of the audit script**
  instead of keeping a second copy — Amendment 9 decision 3's drift hazard, one layer out.

Measured 2026-08-04 on MEM W4: **4 gates → 4m20s**, verdicts `3 COVERED / 1 ERROR / 0 BLIND`.

⚠ **Four operational hazards, each observed rather than predicted:**
1. ~~**A subset run OVERWRITES `docs/reviews/authz-door-audit-findings.md`**~~ — ✅ **CLOSED
   2026-08-26** (`FUP-DOOR-SWEEP-DESTROYS-ITS-OWN-BASELINE`; ADR
   [0153](./0153-subset-sweeps-write-to-scratch-not-the-committed-baseline.md)). With `CASES=` set,
   all four sweeps now write their report **and** their BLIND `.tsv` to scratch under `$WORK`, never
   opening the committed file for write, and an EXIT trap `cksum`-verifies the committed baseline and
   escalates to exit 2 if it moved. ⛔ **Do not re-add "always `git checkout --` it afterwards."**
   It is now false for a subset run, and it was never a gate — it was operator memory, which is what
   let the file be truncated 300 → 2 lines once and **700 → 89** again during AFF3. The instruction
   that survives a revert is a **measurement**, not a sentence: run
   `git diff --stat -- docs/reviews/authz-door-audit-findings.md` after the sweep; empty means
   untouched. ⚠ **Still open in the other run mode:** a **full** sweep still writes through a
   truncating redirect, which destroys the file's hand-merged annotations
   (`FUP-DOOR-SWEEP-FULL-RUN-DESTROYS-HAND-MERGED-ANNOTATIONS`).
2. ~~**`WORK` is hardcoded to a stale session scratchpad.**~~ — ✅ **CLOSED.** The default is
   `${TMPDIR:-/tmp}/authz-audit` (matching `e2e-prod-gate.sh`) and the directory's *writability* is
   asserted rather than assumed, because the old committed Windows path made `mkdir -p` fail
   everywhere else, left every arm comparing **empty** sets, and printed `INVARIANT HOLDS` having
   measured nothing — while wearing the badge of CLAUDE.md §6 step 1. Still pass `WORK=` per run so
   two runs cannot land in each other's results.
...
4. **A function the name filter does not match is a REVIEW ITEM, not a non-gate.** The script
   prints the excluded set; ruling on it is the operator's, and the ruling belongs in the gate
   record. The door sweep can only neutralize a **boolean predicate**, so a DEFINER *writer* door
   named here owes a **targeted mutation case** instead — it is not sweepable, and putting it in
   `CASES` makes the whole run `UNPROVEN`. Live shape of the problem: AFF4 adds five gates
   (`affiliate_person_to_org`, `end_org_affiliation`, `void_affiliation`, `void_org_affiliation`,
   `update_org_affiliation`) and re-emits `list_org_people` — **not one matches the filter**, so
   AFF4's derivation is exit 1 with a six-name review list, which is the correct and intended
   outcome, not a tooling failure.
...
⭐ **LANDED 2026-08-26 — all three, in the artefact they govern**
([`scripts/door-sweep-cases.sh`](../../scripts/door-sweep-cases.sh); § *The recipe* above now
points at it). Ruling 3's *"until the sweep tooling can detect this"* clause is **discharged**:
the script puts the altered policy in `CASES` itself and prints the verdict the findings file
currently claims for it. ⛔ The gap this amendment describes was **not** closed by writing this
paragraph — it stood for two days *while this text existed*, and cost a second operator the same
diagnosis during AFF3 (ADR [0148](0148-ever-held-affiliation-read-visibility.md)). A ruling about
a recipe lives in the recipe; the ADR records **why**, and points.
