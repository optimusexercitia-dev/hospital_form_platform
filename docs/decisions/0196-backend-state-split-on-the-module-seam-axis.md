...
> ⬅ **A later ADR changes this one** — it is **amended by** [0199](0199-a-frozen-archive-does-not-belong-on-the-retrieval-path.md).
...
> ⚠ **AMENDED 2026-09-09 — the premise stands, the conclusion is reversed. See ADR 0199.** The chain
> is no longer in this directory and is no longer routed: it is
> `docs/progress/backend-state-stamp-history-archive.md`. Keeping it *routed* was the error — a frozen
> archive on the retrieval path reads as current state, which is how its stalest claims kept
> circulating. **"It holds facts found nowhere else" is a reason to EXTRACT those facts, not to route
> the archive**; they were verified against the live catalog and moved into the seams that own them.
>
> ⛔ Two corrections to this paragraph's own example list, both found by enumerating rather than
> reading. **`hospital_indicator_rollup` was never chain-only** — its lost `is_admin` arm is fully
> documented at `data-access.md:129`, with migration, bug id and pgTAP guard. And the list is not three
> items but roughly **150**, six of which *contradicted* a posted seam section, with the seam wrong in
> all six. The three named here were the ones someone remembered; a hand-list wearing the label of a
> survey is the failure this repo already names.
>
> The other three named facts all **verify as current** (2026-09-09, migration `20261003007350`) and were
> extracted with provenance. `verify_audit_chain`'s qualifier is the one that most justified D9: the
> seams named the function three times and nowhere said its `app.is_admin()` arm is the deliberate
> platform-tier branch, so a noun-rule sweep would have stripped it unopposed.
