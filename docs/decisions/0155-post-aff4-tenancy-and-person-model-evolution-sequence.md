...
2. **`EXPLAIN (ANALYZE, BUFFERS)` evidence on the hot read paths** — the ADR 0078 A5 discipline,
   gated *before* any policy is repointed.
...
1. **RLS performance.** The role helpers run **per row**, and **119 of 283 policies** call them.
   ...
   document read. Mitigable (`ltree`, a closure table) but not by assumption.
...
⚠ **Stated rather than hidden.** The sizing above was derived from the live catalog and from ADR
0151's plan document. **No step was attempted**, so every difficulty estimate is an inference, not a
measurement.
