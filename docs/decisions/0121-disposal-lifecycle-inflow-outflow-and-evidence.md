...
>   while **C1a itself stands** — § 3 is simply un-run. ⚠ *This line read "§ 3 is blocked by
>   `FUP-DISPOSAL-CHILD-LOCK-BLOCKS-PHI-ERASURE`" until 2026-08-19; **corrected — it never was.**
>   That defect is on the meeting-minutes path, while § 3 is the `file_objects`/Storage path, and
>   the two are disjoint in the catalog (ADR [0129](./0129-meeting-child-lock-disposal-flag.md)
>   Consequences).*
