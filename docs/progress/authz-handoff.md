...
| Item | State |
|---|---|
| ✅ **GATE 2 (Stage C · F1 · N1 · Stage G) — RE-GATE RESOLVED 2026-07-17 (version-drift audit).** The three "open items" at the pause are CLOSED: **① BUG-PROD-ACTIONS = environment drift, CONFIRMED** — `rm -rf node_modules && npm ci` from the committed lockfile yields Next **16.3.0-preview.5** (a clean / Coolify checkout cannot reproduce it); it was BUG-AIF-001 running on a stale local `node_modules@16.2.9`, so it is **not a Gate-2 code defect and cannot reach the pilot**. **② BUG-GATE2-243-FIXTURELOCK FIXED** — fixture reordered (plant the reserved session while the meeting is editable, then walk the real conclude→sign→distribute lifecycle under `app.in_meeting_rpc`); `243` now **31/31**, the MAJOR-2 child-lock keystones (28-31) now EXECUTE and PASS. ⚠ the pause note's proposed fix ("create the session before distributing") was INSUFFICIENT — `guard_meeting_status` rejects a raw `scheduled→distributed` UPDATE. **③ BUG-GATE2-235-PLANCOUNT FIXED** — the wave added 8 assertions but planned +9; restored the intended **base-table no-over-reach twin** (not lowered the plan); `235` now **42/42**. Full pgTAP **112 files / 3186 / PASS**; lint 0/0 · typecheck · vitest 369/369 · `next build` green on 16.3; `e2e:prod` re-run on a force-rebuilt 16.3 standalone with the ~18–27 flaky baseline collapsed to ≈0. Backend + FE independently audited (live catalog + full FE/E2E sweep): **NO 16.2-driven deviations or shortcuts; every fix-wave migration maps to a real catalog-verifiable authorization defect, and the FE keeps the idiomatic 16.3 `router.refresh()`-in-transition pattern with all four deliverables fully implemented.** One catalog hygiene finding: `app.can_reach_case_on_member_surface` is now orphaned (C1 removed its one consumer) — see plan §G1. Remaining before merge: PO/human approval + the reserved-PHI/disposal gate-close question below. **Historical pause detail kept for the record:** | **Committed on `feat/authorization-capability-model` (local, NOT pushed):** `fee5283` (Stage C) · `456d008` (F1+N1) · `ac57a20` (VOLATILE) · `01d9ef2` (Stage-C FE) · `bb22b45` (Stage G) · `17a8d08` (fix-loop) · **`b91b41a` (qa fix wave — P0 org-arm-in-DEFINER-doors + MAJOR-1 `description` + MAJOR-2 reserved child-lock + 228 recut `NOT is_case_respondent` + 2 sweep migs + the Gate-1 `list_cases_board` fast-path escape)** · **`cf93a15` (cases-board 404 for administration-only principals + `memberships` rulebook fix).** All lead-verified against the live catalog. **qa verdict on the wave was ⛔ CHANGES REQUESTED → all fixed & re-verified (see §7.14–7.16 for what it taught).** **⏳ RE-GATE ROUND 1 (tester) came back MOSTLY GREEN with 3 open items — THIS IS THE RESUME WORK:** ① **BUG-GATE2-243-FIXTURELOCK (MAJOR, test-only, GATE BLOCKER)** — MAJOR-2's keystones `243`/28–31 never run (fixture trips the guard; §7.15); fix = reorder the fixture (create closed session BEFORE distributing) → `backend`, then re-run so 28–31 execute+pass. ② **BUG-GATE2-235-PLANCOUNT (LOW-MED)** — `plan(42)` vs 41 actual; backend intended 42, so reconcile (missing assertion vs over-count) → `backend`. ③ **🔴 BUG-PROD-ACTIONS = likely ENVIRONMENT DRIFT not a code defect (§7.16)** — needs `rm -rf node_modules && npm ci` + repro to confirm, then **PO adjudication** to downgrade/close the P0. **Uncommitted at pause:** `e2e/cases-board-access.spec.ts` (tester's new spec — a deliverable, commit it), `PROGRESS.md`, `package-lock.json` (cosmetic `libc` diff from the tester's `npm install` — backend's call), `.claude/agents/frontend-engineer.md` (unrelated, user's). **Then:** backend fixes ①②, tester re-runs 243/235 + declares green, `qa` re-reviews (it will re-probe P0/MAJOR-1/MAJOR-2 behaviourally + require the 243 mutation actually RUNS), → Gate-2 summary → **PO approval** (Phase-Gate step 4). **Also open for the gate-close ruling:** reserved-session content has **no disposal path** (`dispose_meeting_minutes` skips the reserved tables) and the reserved columns carry **no `PHI-BEARING` comment** (so the MAJOR-1-style column sweep is blind to the most confidential table) — PO/qa: is reserved deliberation PHI (Rule 12 retention) or governance-record-only? |
...
### 7.14 ⛔ The keystone must exercise the surface the PRODUCT calls — "audit one layer, infer the next" is SYMMETRIC, and it happened FIVE times in one day (Gate-2 re-review + fix wave, 2026-07-17)

§7 already says *"prosecdef belongs beside pg_policies."* Here is that lesson generalized, and its cost measured:
**a test or verification that checks ONE authorization layer and INFERS the adjacent one is blind by
construction — in EITHER direction — and the blindness survives a green suite.** Five instances, one day, two of
them the lead's, two inside gates already RECORDED AS PASSED:

| # | Check | Verified | Blind to | Caught by |
|---|---|---|---|---|
| 1 | C7 K17 (`245`) | base tables = 0 | the 3 DEFINER doors (`prosecdef=t`, RLS never runs) that returned the data | lead re-probe — an org admin read the agenda |
| 2 | `241` K10 | reads through `get_meeting_cases` (`prosecdef=t`) | `meeting_cases_select` — mutate the policy to the `is_case_excluded` trap and **241 stays FULLY GREEN** | backend, only because the lead's brief wrongly claimed K10 guarded it |
| 3 | `235` K1 | `count(*) from public.cases = 0` | `list_cases_board` (the door the product calls) → org admin read 5 cases incl. the ethics one | lead, chasing a different finding |
| 4 | lead's 404-lockout claim | the DB predicate (`can_read_case` ≠ `is_member`) | the UI gate one layer up (`session.ts:441` already 404s NSP/PQS) | frontend REFUSING to comply and verifying |
| 5 | `243` MAJOR-2 keystones | the guard is attached (catalog) | whether the keystones RUN — the fixture trips the guard and aborts before them | tester reading the exit code, not the summary |

**Review caught it 0/5 times on the merits.** The two catches (2, 4) were an agent *declining to comply and
checking instead* — that is luck that the briefed agents argued back, not a method. **The fix is structural, not
vigilance:** a pgTAP invariant that every `authenticated`-reachable `prosecdef=t` function has ≥1 keystone that
CALLS it, and every policy has ≥1 keystone hitting the base table directly (not only through a DEFINER door).
**Apply: name the surface the PRODUCT calls, and make the keystone hit THAT surface.** A policy-shaped test over a
DEFINER-door product path — or a DEFINER-door test over a policy the product also exposes — is the empty set
wearing a green check. Filed as its own pre-pilot P0 (**AUDIT-DOOR-BLINDNESS**): a door-level re-audit of Gate 1 +
...
**a test that WOULD have failed but never EXECUTED — and a file that aborts early reads as a pass if you trust the
summary.** Two live this session:

1. **`243` (MAJOR-2 keystones).** The fixture creates a `distributed` meeting, then inserts a
   `meeting_closed_sessions` row into it *as un-guarded setup* — but MAJOR-2's new guard blocks exactly that, so
   the file **aborts at the setup insert, before its own 4 keystones (28–31) ever run.** The fix is correct and
   catalog-verified; its PROOF does not execute. A test whose own fixture trips the mechanism it tests is a
   keystone that isn't there. (Fix: set up the closed session BEFORE distributing — create non-locked, insert,
   then `UPDATE … 'distributed'`.)
2. **The `\b` sweep.** backend's first shape-sweep used `\b` for a word boundary — but **PostgreSQL's `\b` is a
   BACKSPACE, not a word boundary (`\y` is)** — so the pattern matched **nothing** and returned a clean bill of
   health. Verified: `'foo bar' ~ '\bbar\b'` = **false**, `~ '\ybar\y'` = **true**. A sweep with a broken pattern
   is the empty set again, as a false all-clear.

Both compound with the `tail -5` gate false-green (a real red pushed out of the summary window). **Apply: confirm
the assertion RAN, not just that the summary is green.** For pgTAP, check planned-vs-ran (`235`: planned 42, ran
**41** — one assertion silently absent) and that no file `exited N` before its plan completed. For a sweep, prove
the pattern is non-vacuous before trusting a zero — feed it one known match. A green you didn't watch execute is a
rumor.

### 7.16 ⛔ Before hunting a "code" defect that reproduces ONLY on the prod/standalone build, verify the TOOLCHAIN is what the lockfile says (2026-07-17)

**BUG-PROD-ACTIONS — a P0 that burned a multi-hour cross-agent hunt (five hypotheses killed, some the lead's, some
the tester's) as a Server-Action *dispatch* defect — is, on strong evidence, ENVIRONMENT DRIFT.** `node_modules/next`
had silently drifted to **16.2.9** while `package.json` and the lockfile both declared `16.3.0-preview.5`; `npm ls
next` flagged it **invalid**. And **16.2.9 is the exact BUG-AIF-001 version whose deferred-flush (`action →
router.refresh`) stall IS the BUG-PROD-ACTIONS signature** — the standalone build was compiling against stale deps
and faithfully reproducing a bug already fixed in the declared version. After `npm install` corrected it, the
100%-reproducible 21–31s hang became **8/8 green in 17.4s**, and the full `e2e:prod` came back **719 / 2 / 0** with
the historical *~18–27 flaky baseline gone*. **That "prod flaky baseline" was likely this drift, not real flake.**
⛔ **Not self-resolved — it is a PO-adjudicated unit, and "strong evidence" is how false P0s both die AND hide.**
Decisive confirmation = **`rm -rf node_modules && npm ci` (deterministic from the lockfile) + re-run the repro**,
proving a CLEAN checkout (≈ what Coolify builds) yields the right version so the P0 cannot reach the pilot. **Apply:
for any defect that reproduces ONLY on the prod/standalone build and NOT on `dev`, run `npm ls <framework>` before
the code archaeology** — `dev` uses a different compile path and can mask a stale `node_modules`; the standalone
build bakes in whatever is on disk. The toolchain is a suspect, and the cheapest one to clear.

> ⭐ **7.16·b — the rulebook is not truth either (`memberships`, 2026-07-17):** CLAUDE.md §3 and ARCHITECTURE.md §2
> named a canonical table **`commission_members(commission_id, user_id, role)`** that **does not exist** — the real
> table is **`memberships`** (multi-scope org/hospital/commission, keyed `principal_id`, 7-role text+CHECK with a
> scope-exclusivity CHECK). It cost the lead a `relation "public.commission_members" does not exist` probe
> mid-audit — the same coin the `case_patient` scar already spent in CLAUDE.md §1. **§7.2 reaches the binding
> rulebook itself: the canonical-schema block is prose; the catalog is the schema.** Fixed both files with the same
> *"verify against the catalog, never this sentence"* warning, because the lesson demonstrably does not transfer by
> being stated once.
