...
- **Predicate swap:** must not touch org-level-only sites (`is_org_admin_of(org)` stays). Backend surface reference: [docs/backend-state.md](../backend-state/tenancy-and-identity.md).
