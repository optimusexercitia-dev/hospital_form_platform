...
### 2026-09-05 — Record step (lead): PO approved at `d160aabf`; unit closed and merged

**Approval.** PO approved the unit 2026-09-05 on the QA sign-off ([door-sweep-deriver-rereview.md](../reviews/door-sweep-deriver-rereview.md), ✅ APPROVED at `8a6fe699` after a four-round loop; first review [door-sweep-deriver-review.md](../reviews/door-sweep-deriver-review.md), CHANGES REQUESTED at `de955981` on F-BLOCK-1) — ruling: **Approve → Record + merge**, which also **ratifies the three closures** whose register field read `PO to rule` (`…BLIND-TO-ALTER-FUNCTION`, `…SPANS-THE-WHOLE-WORKING-TREE`, `…FULL-RUN-DESTROYS-HAND-MERGED-ANNOTATIONS`). QA's F4-REC-1..3 folded in `addd780c` before this step.

**What this step did.** `docs/lead-playbook.md` §4 gained the two step-1 obligations ADR 0190 creates (quote the `SCOPE:` line with its `derivation:` mode; run `SELFTEST=1` beside the four arms) and the full-run merge nuance on "empty diff means untouched"; ledger row appended (id unbolded — the `LEDGER-ID-BOLD` workaround); hub → `status: complete`, `branch: ~`; the hub's `:90` histogram phrase relabelled by its instrument (398/1/0 unescaped; 397/1/1 raw pipes — QA F4-REC-1) **before** the block below was cut, so the archived copy is not the false clause; ADR 0190 status line carries the PO approval; indexes regenerated; `npm run lint` read bare; fast-forward merge into `main`. ⛔ Not done here: the CLAUDE.md review queue (non-empty since 2026-08-31 — still scheduled with the PO); graphify refresh skipped with reason (shell + docs only, nothing graphify indexes); the `PRED_DOMAIN` widening and the targeted case `9a4bbd22`'s door owes (Batch 2).

#### Hub `## Current state` at closure — cut from the hub 2026-09-05 (ADR 0186 D8)
...
`:112` and `:262`. Measured three independent ways, all 399 (commands and the 398/1/0 unescaped-separator (397/1/1 counting raw pipes)
