...
| --- | --- | --- | --- | --- | --- |
| 1 | `commission.forms.edit` | commission_content | write | none | ⛔ **CORRECTED 2026-09-03 against the live catalog** (Gate AE4 review F-REC-4; this cell read *"7 ALL"* and was wrong twice). **R — SIX `ALL` policies**, `forms` / `form_versions` / `form_sections` / `form_items` / `form_item_options` / `form_item_validations`, each `_staff_admin_write`, **all six now re-keyed onto `app.can_edit_commission_forms` in BOTH halves** (migration `20261003007340`). ⚠ Of the six, `form_item_validations`'s is an **unreachable backstop** — `authenticated` holds SELECT only on that table, so the policy gates nothing a client can reach; the real write path is the DEFINER `public.set_item_validations`, still layer-1. ⚠ **CORRECTED 2026-09-07** (pre-AE5 Batch 4, migration `20261003007350`, ADR 0193 D5): that DEFINER door is **no longer layer-1** — it now gates on `app.can_edit_commission_forms`, because a backstop no `authenticated` statement can reach left `commission.forms.edit` load-bearing on nothing for this whole table. Its behavioural differential is pgTAP `409` § 2.6f / § 2.10e (grant present -> succeeds, grant deleted -> `42501`), observed RED before the migration existed. ⛔ **`form_block_library` is a `D` site, not an `R` site** — it carries **no write policy at all** (only `form_block_library_select`); writes go through `save_block_to_library` / `update_block_library_entry` / `delete_block_library_entry`, none re-keyed. Adding a policy there would be inert today and a latent widening tomorrow. · **D** form fns — ⛔ **0 of 8 carry a permission literal**, and **1 of 8 is RE-KEYED**. ⚠ **CORRECTED 2026-09-07** (Q6, delegated by the PO): this cell read *"0 carry a permission literal; 22 DEFINER functions gate form-family tables on `is_staff_admin_of` and none is re-keyed"*. Carrying the literal and being re-keyed are **different questions**: `public.set_item_validations` now calls the authorizer, so it is re-keyed and still carries no literal — exactly as the six policies do, and the literal stays in `app.can_edit_commission_forms` alone. ⛔ **8 and 22 are DIFFERENT POPULATIONS and both are correct — neither is a correction of the other** (LEARN-079): **8** = SECURITY DEFINER functions that **write** a form-family table (the manifest row's own "D 8 form fns", reproduced against the live catalog 2026-09-07); **22** = DEFINER functions that **gate** form-family tables on `is_staff_admin_of`, i.e. the wider **read-and-write** population. The remaining **7 of 8** are AE5's and are declared as data in the manifest row's `definerSurface` (pgTAP `410` § 8.7 closes it both ways). So "the production door" for this row means the **policy** door **plus one re-keyed DEFINER door**. · **E2E** `phase4-builder.spec.ts:115-180`, `:261-389` |
...
| | change |
| --- | --- |
| **New** row 43 `org.professionals.create` | org-scoped, `class2_professional_identity`. ⛔ **CORRECTED 2026-09-04 against the live catalog** (Gate AE4 review F-REC-3; this cell read *"Granted to `staff_admin` **and `org_admin`**"*, which was legacy REACH written as a grant). **The catalog grant is `staff_admin`'s alone** — `authz.role_permissions` holds exactly one row for this code (`staff_admin`), and **zero** rows for `org_admin` on any code (42 rows, all `staff_admin`). `org_admin` reaches the door through the **PRESERVED LEGACY ARM** of `app.can_create_professional` (`app.can_manage_professional` → `is_admin()` / `is_org_admin_of`), never through a permission. ⚠ It could not hold one today even if a row were seeded: `authz.roles.org_admin.state = 'legacy'`, and `authz.has_permission` admits only `role_state = 'authoritative'` (ADR 0174 D2) — *"a role's `authz.role_permissions` rows are INERT until its state flips"* (`docs/progress/authz-ae4.md`, AE4.9, where pgTAP `401` §16.10 found it live). **In this oracle, "granted to" means a catalog row.** |
| Row 30 `org.professionals.manage` | ⛔ **`staff_admin`'s grant is REVOKED.** Retained by `org_admin` / `platform_admin` — ⚠ **as legacy REACH, not as a catalog row** (⛔ **CLARIFIED 2026-09-04 against the live catalog**, same finding): `authz.role_permissions` holds **zero** rows for `org.professionals.manage` for **any** role. The code exists in `authz.permissions` and is granted to nobody; both roles reach `app.can_manage_professional` through `is_admin()` / `is_org_admin_of`. |
