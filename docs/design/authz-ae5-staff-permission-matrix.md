...
| **12** | `public.cast_case_vote` | ⚠ **arm-3 (a)** — ⛔ **r1 marked this `—`** | ⛔⛔ **CORRECTED 2026-09-13 (lead ruling L5) — r1 AND r2 BOTH CALLED THIS A STATUS GUARD AND IT IS NOT.** Measured in the live body: the `HC0J0` guard is `if not exists (select 1 from public.ethics_case_details d where d.case_id = v_case_id)` — an **ethics-DETAILS EXISTENCE** test, evaluated before the membership test. It is arm-3 (a) for the same reason either way (the term is a property of the case row and no axis varies it), but the FIXTURE PAIR the correction changes: a case **WITH** an `ethics_case_details` row vs one **WITHOUT**, ⛔ not two case statuses. ⚠ The wrong reading also reached `424`'s header (*"votable status / non-votable status"*), which the tester corrects on its side — both documents were wrong in the same direction, and a fixture built to either would have constructed the wrong pair. The state column is declared as data in the manifest at `permissions["commission.cases.vote"].arm3Door.stateColumn`. |
