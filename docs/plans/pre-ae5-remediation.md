...
| 10 | `ADMIN-ARM-IS-ACTIVE` — [hub](../features/admin-arm-is-active.md) · [record](../progress/admin-arm-is-active.md) · [review](../reviews/admin-arm-is-active-review.md) · ADR [0201](../decisions/0201-the-keying-asymmetry-is-the-model.md) D4/D5/D2 amended in place (no new number) | ff-merged 2026-09-10 at the Record step, ancestry proven bare — tip `ef2625f2`, the phase commit itself (fast-forward, no merge commit); 25 commits unpushed at close, re-measure never quote. ✅ **PUSHED 2026-09-10** (`b87eac1e..44f69ff6`, 27 commits, a FIFTH one-push override scoped in writing to that push; Coolify state stated once more as not verifiable by the session, PO instructed *"push main"*) — superseded text: *"⛔ NOT pushed — PO approved Record step + ff-merge, no push; the standing instruction holds"*; ⛔ *"do not push"* is the standing instruction again from the next commit on | `FUP-CAN-MANAGE-PROFESSIONAL-SELF-CHECK-ADMIN-ARM-IGNORES-IS-ACTIVE` · `FUP-CAN-MANAGE-PROFESSIONAL-SELF-CHECK-PLATFORM-ADMIN-CLASS-2-WRITE` · `FUP-AE5-OPENING-ADR-R10-AUDIT-STAMP-HAS-NO-REGISTER-HOME`, each on its own clause | A deactivated or suspended `platform_admin` passes no admin arm and seats no hat (three sites, each RED-first); a `platform_admin` cannot write Class-2 professional content; the seating audit row names the role only — and, ruled R6, seating is an IDENTITY event whose home is the platform feed; the TS mirror follows the SQL; row 31 has its own differential representative; the ONE expected-red list was derived, not inherited (a fifth file) |
...
> ✅ **CONCLUDED 2026-09-09 — unit `CAN-MANAGE-PROFESSIONAL-SELF-CHECK`, ADR 0200 (amends 0193 + 0190), QA APPROVED in one round at `e696d107` (1 MINOR, closed by measurement); PO-approved, ff-merged, pushed on a second explicit PO instruction (§2 row 8).** The block below is **kept, not deleted**, and this banner is what stops a reader acting on it twice.
>
> ⛔ **The block's own description was wrong in BOTH directions, measured at pair `(20261003007350, 524)` from the live catalog — corrected here rather than left to be re-discovered:**
> **(a)** *"its first arm is `coalesce(app.is_admin(), false)`"* — **both** arms were caller-keyed (`app.is_org_admin_of(p_org)` reads `auth.uid()` too); after AE4.7c removed the staff_admin ascent, `p_uid` was a pure null guard, and the live body's own header comment said so (*"narrowed rather than fixed … Left alone deliberately"*).
> **(b)** the follow-up's *"reachable consequence"* (a `platform_admin` asking whether X may read a profile gets TRUE) had **0 reachable paths**: every entry point in the transitive closure passes `auth.uid()`; the one third-party caller it names, `app.can_read_professional_profile`, is itself only ever called with `auth.uid()` — a latent trap, not a live hole. The same function carried its **own** `is_admin()` first arm that fires before the named predicate is reached, so a fix to the named predicate alone would have left the follow-up's stated consequence true — PO ruling **R2** put both sites in one migration, each with its own bidirectional cells.
> **(c)** *"a migration → diff-scoped sweep, both arms"* — the deriver hands the **write arm 0 cases** for this diff (two `returns boolean` predicates, no raise guard); "both arms" was discharged as a checked claim, not a second sweep. ⚠ And the deriver's FIRST run exited 1 on a **false FINDING (1)** — a "targets cannot be read" verdict decided on the post-dedup residue — fixed in the unit (`ea92fbee`, self-test 42 → 46).
...
> ⚠ **EDITED 2026-09-09 — this checklist now reads BATCH 9.** It is an **operational instruction**,
> so it is edited rather than annotated (R45). The superseded checklist read BATCH 8; ⛔ its facts are
> not lost and are not repeated here — every measurement it carried now lives in **§2 row 8** and in
> the Batch 8 block's CONCLUDED banner in §3, which is that fact's one home. (The 2026-09-08 edit that
> preceded this one carried the Batch 7 push override; that override was scoped to Batch 7's push and
> is spent. Batch 8 was first approved **without** a push, then pushed on a second, explicit PO
> instruction in the same session — a THIRD one-push override, spent the moment it ran.)
>
> ⛔ **"Do not push" (§4 step 6) is the standing instruction again.** The two overrides on record
> (Batch 4, Batch 7) were each scoped in writing to one push, and ⛔ **neither is a standing licence.**
> Pushing `main` is deploy-safe *only while* Coolify's Automatic Deployment is off — a fact about an
> external system that `docs/deployment/coolify.md` RECORDS and cannot CHECK; a session that cannot
...
- **Batch 10 EXISTS as a named consequence and this plan deliberately holds no block for it** —
  ...
  **rewritten, not ticked**; and the **expected reds re-ruled, never silenced** (`228:630-634` ·
  ...
  ⚠ **2026-09-10 — Batch 10 CONCLUDED** as unit `ADMIN-ARM-IS-ACTIVE` (§2 row 10): every item above landed, plus what the build found (a fifth red file, a TS mirror, row 31's lost rep, R10's two readers). The successors still open are the three below; the bullet is kept because it is the derivation Batch 10 ran on.
...
1. **Measure, do not assume, where the tree stands.** `git status` clean on `main`;
   ...
   lease**, and Batch 6 proved a branch can appear mid-session; at Batch 8's close the distance was
   **large and growing** — it is stated in Batch 8's record, never quoted from here);
   `docs/features/INDEX.md` shows no `in_progress` hub. If any is false, stop and read the hub that
