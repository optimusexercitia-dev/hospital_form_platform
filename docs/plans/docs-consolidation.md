...
| 3.2 | Gate arms. HANDOFFS: `branch:` **or** `expires:` required (13.67 gains subjects either way); red when `expires` < today; the inbound-citation allowlist drops `docs/features/` (a hub links its handoff through frontmatter, which the gate reads). HUBS: `in_progress` / `gated` ⇒ `progress:` resolves and the record contains `## Session log`; the log's `### YYYY-MM-DD` dates are non-decreasing (append-only, mechanically). | self-test; both live hubs |
