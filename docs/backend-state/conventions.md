...
**Updated:** 2026-09-10 — a REPLACEABLE projection of the frozen sections below; the rules that govern it are [`README.md` § Maintenance rules](README.md#maintenance-rules) 7–8.
...
| `HC0U0` | a WRITE grant on a TERMINAL case, refused by `public.grant_case_access` (unit GRANT-PLANE-CONVENTION, ADR [0205](../decisions/0205-per-object-grant-plane-convention.md) D9). READ grants on a closed case stay LEGAL (ADR 0033 D6), so the message is about the LEVEL, never about the case. ⚠ The row directly below is **stale about this family and is not corrected in place** (a posted section is frozen — README maintenance rule 1): `HC0R*`, `HC0S*` and `HC0T*` were all allocated after it was written, and `HC0T5` is raised only inside migration-time `do $$` blocks so it is never persisted and a catalog-only derivation cannot see it. **Next free = `HC0U1`**, derived from the UNION of the live catalog, `supabase/migrations` + `src` + `supabase/tests`, and `docs/` — never from one of the three. | "Não é possível conceder edição em um caso encerrado." (`src/lib/case-access/actions.ts`) |
| `HC0U1` | a SELF-GRANT on the case grant door, refused by `public.grant_case_access` when `p_user = auth.uid()` (unit GRANT-PLANE-CONVENTION-A1, ADR [0205](../decisions/0205-per-object-grant-plane-convention.md) § Amendment 1 D6·5·1). Refused as an **ACT**, immediately after authority (42501) + the U1 exclusion (`HC0F1`) and **before** level, membership (`HC021`), expiry and the terminal check (`HC0U0`) — so every level and both SQL-only PHI parameters are refused alike, and the message is about **who the grantee is**, never about the level or the case. ⛔ The kernel `app._grant_case_access_unchecked` (`create_case`'s creator self-grant) and `revoke_case_access` are UNTOUCHED. ⚠ The `HC0U0` row above says **Next free = `HC0U1`**; that claim is now spent and the row is frozen, not corrected in place (README maintenance rule 1). **Next free = `HC0U2`**, re-derived here from the UNION of the live catalog (243 codes), `supabase/migrations` + `src` + `supabase/tests` (265) and `docs/` (271) — never from one of the three, and never from this sentence once it is a day old. | "Não é possível conceder acesso a si mesmo." (`src/lib/case-access/actions.ts`) |
