...
- **The catalog is authority-ELECT, not authority** — an *additional* authority beside `memberships_role_check`, the
  ...
  adding an arm reds the pin, **retiring** one reds it too. Frozen § AE4 carries the enumeration. ⚠ On the professional-profile READ door the case-committee arm (no org term; role-free at its S3/S4 case-grant sources only, so it survives an absent hat; `pending` reachable) is **ORACLED** by `403` with a PO-ruled value per derived class — ⛔ the admin arm there is **exercised, not oracled** (frozen § Arm 3 oracled).
