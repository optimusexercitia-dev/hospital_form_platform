...
| BUG-DOC-001 | 17 | BLOCKER | Systemic form↔action FormData field-name mismatch (`documentVersionId`/`documentId` vs `versionId`/`commissionId`) broke the whole UI write path ("Documento não encontrado." on every write). | phase17-documents.spec.ts AC-1/3/4/5/7/9/13 | frontend | ✅ RESOLVED 2026-07-06 — aligned the 4 write forms to the action contract (+ split a latent supersede-via-AddVersionForm mis-wire). → [phase-17.md](../progress/phase-17.md) |
| BUG-DOC-002 | 17 | MINOR (seed) | Seeded `storage_path` had no backing storage object → "Sem arquivo". | phase17-documents.spec.ts AC-2 | backend (seed) | ✅ RESOLVED 2026-07-06 — NULLed the dangling path (honest "Sem arquivo"; download coverage = AC-1 real upload). → [phase-17.md](../progress/phase-17.md) |
| BUG-DOC-003 | 17 | BLOCKER | `approvers` JSON serialized camelCase but `submit_document_for_approval` reads snake_case → NULL id → misleading HC091. | phase17-documents.spec.ts AC-1/4/5/7 | backend (action) | ✅ RESOLVED 2026-07-06 — mapped camel→snake in the action (TS↔SQL boundary; form stays camelCase). → [phase-17.md](../progress/phase-17.md) |
| BUG-DOC-004 | 17 | BLOCKER | Supersede UI dead-end on the coordinator detail page (new draft unreachable). | phase17-documents.spec.ts AC-5/7 | frontend | ✅ RESOLVED 2026-07-06 — separated in-force version from working draft (backend state correct, per lead). → [phase-17.md](../progress/phase-17.md) |
| BUG-DOC-005 | 17 | BLOCKER | Org approver-detail page signed the wrong version after a supersede (same class as DOC-004, sibling page). | phase17-documents.spec.ts AC-5 | frontend | ✅ RESOLVED 2026-07-06 — whole-UI sweep + extracted shared `version-select.ts` (both detail pages consume it); also fixed a 2nd latent cross-version `isPendingApprover` bug. → [phase-17.md](../progress/phase-17.md) |
| BUG-FBE-005 | Form-builder batch | BLOCKER | Client value-import of `OTHER_OPTION_LABEL`/`OTHER_OPTION_CODE` from server-only `queries/forms.ts` pulled `next/headers` into the client bundle → `next build` aborted + wizard-fill/builder routes errored on dev. | (build gate) | backend+frontend | ✅ RESOLVED 2026-07-07 — consts relocated to client-safe `src/lib/forms/option-constants.ts` (re-exported); 3 client imports repointed; `next build` EXIT 0. → [adjustments-batch.md](../progress/adjustments-batch.md) |
| BUG-FBE-006 | Form-builder batch | BLOCKER | Aggregate result rules couldn't save — `validate_template_result_ruleset` rejected `__total_score__`/`__flagged_count__` via HC016 (compute wired, save/validate seam wasn't). | flagged-aggregate-result.spec.ts AC-1/2/3 | backend | ✅ RESOLVED 2026-07-07 — migration `…001000` whitelists the 2 synthetic keys (mirrors `__phase_result__`), skips option-code assertion for numeric values; unknown key still HC016; pgTAP `202` §5. → [adjustments-batch.md](../progress/adjustments-batch.md) |
| BUG-FBE-007 | Form-builder batch | MAJOR | Masked time `930`→`93:0`→blur cleared instead of `09:30`. | wizard-others-ux AC-2 / views-labels AC-6b | frontend | ✅ RESOLVED 2026-07-07 — mask↔normalize made self-consistent; ultimately mooted by restoring the segmented time control (human decision). → [adjustments-batch.md](../progress/adjustments-batch.md) |
| BUG-FBE-008 | Form-builder batch | MAJOR | "Outro" free text never persisted — absent from the save payload. 4 iters chased upstream suspects; TRUE cause = `wizard-runner.tsx` `actions` adapter hand-listed forwarded fields and OMITTED `otherTextByItemId` in both saveSection/saveAndExit (BUG-FBE-004 recurrence; optional-field omission is tsc-invisible). | wizard-others-ux.spec.ts AC-4 | frontend | ✅ RESOLVED 2026-07-07 (iter-4) — spread-forward the whole input in both adapters (recurrence-proof) + adapter-layer regression test; wire-confirmed `otherTextByItemId` + non-NULL `other_text` on resume; 5-spec batch 29/29. → [adjustments-batch.md](../progress/adjustments-batch.md) |
| BUG-FBE-009 | Form-builder batch | NOT-A-BUG (test-selector) | Filed MAJOR vs the app; frontend proved app correct — AC-9's `.first()` observação locator grabbed an unrelated unanswered controller MC's disabled button. | form-builder-enhancements AC-9/12-13/K/14 | tester (test-side) | ✅ RESOLVED 2026-07-07 — reclassified NOT-A-BUG; app gained `aria-label="Adicionar observação — <pergunta>"`; tests scoped to per-item name + `toBeEnabled`. → [adjustments-batch.md](../progress/adjustments-batch.md) |
...
[ff-2-matrix-risk-matrix.md](../progress/ff-2-matrix-risk-matrix.md). The ETH·E2 targeted-lane fix
...
Full phase context → [ff-5-entity-reference.md](../progress/ff-5-entity-reference.md).
...
### Closed — rotated 2026-08-04 · 2026-08-06 → [bug-log-archive.md](./archive.md)
...
Earlier eras, all closed and rotated → [bug-log-archive.md](./archive.md):
...
▶ **Spawned [FUP-QOB-3](../followups/follow-ups-open.md): `dispose_event_phi` is now the only Rule-12
...
[authz-invoker-wrapper.md](../progress/authz-invoker-wrapper.md).
...
### Closed this phase → [bug-log-archive.md](./archive.md) (rotated 2026-08-14)
...
### Closed → [bug-log-archive.md](./archive.md)
...
(ARM 5; *Completed work* above → [authz-invoker-wrapper.md](../progress/authz-invoker-wrapper.md)),
...
[follow-ups-archive.md](../followups/follow-ups-archive.md); its full record was already
...
which was met on 2026-08-17 and never recorded** → [archive § "Rotated 2026-08-18"](./archive.md).
...
(filed 2026-08-14, fix shipped in the same slice) → [archive § "Rotated 2026-08-18"](./archive.md).
...
DM5·S6 gate step 2; fix `15396276`, RED-first pin `348acf5f`) → [archive § "Rotated 2026-08-18"](./archive.md).
...
and closure evidence → [archive § "Rotated 2026-08-18"](./archive.md). Verbose
narratives rotated 2026-08-18 → [archive § "Bug Log closure narratives"](./archive.md).
...
## ↩ Closed 2026-08-20 — DSR Slice 3 (10 bugs), VERBATIM apart from the link repoint

> All ten were **found and fixed inside Slice 3**, and every one carries an executed pin as of the
> declaring gate (pgTAP 6678/6678 · E2E 37/37 across the four DSR specs). Two are SPEC defects
> (-004, -005) found by the tester in its own work; eight are product defects. ⭐ **Four were
> visible only by EXECUTING something** — no static gate saw them.

🔴 **BUG-DSR-S3-006 — ✅ FIXED 2026-08-20 (`20261002000300`); unclosed until pinned.** A refused close left six tasks `pending` with a live "Executar descarte" — the workflow **instructing the opposite of its own decision**, failing **OPEN against a decision to RETAIN**. ⚠ The asymmetry that produced it is **INTENDED and stays**. Fix: non-erasing closes retire tasks to **`blocked`** (= *retired by decision*, **not** *waiting* — that lives one join away). ⭐ **The sweep found FIVE readers, not the one the brief named** — incl. the sibling `attest_dsr_task`, `getDsrOutcomeRecord` (parts stopped summing), and ⛔ **`list_my_executable_dsr_tasks`, which had NO status filter since Slice 2** and had been offering **`done`** tasks as executable. ⛔ **The over-grant twin the lead asked for is NOT CONSTRUCTIBLE and was DELETED, not kept green.** Body + sweep table → [dsr-slice-3.md](../progress/dsr-slice-3.md).
🔴 **BUG-DSR-S3-008 — ✅ FIXED 2026-08-20; unclosed until pinned.** A **retired** `attest_review` card still printed its D7 corridor text (*"reabra a reunião, edite o trecho e assine novamente"*) **directly above** the sentence saying not to execute — an executor following it would **reopen and re-sign a meeting on a request that ordered no erasure**, bumping the revision and invalidating registered prints (ADR 0126) on records the adjudication decided to KEEP. Found **only in a browser**; every static gate green. Suppressed on retired, still rendered live. ⭐ The same check gave **2 false positives and 1 true one** — *narrowing instead of deleting it is the only reason the defect surfaced*; a noisy detector is imprecise, not wrong.
⛔ **Both arms need pinning — and the obvious pin is VACUOUS:** the corridor renders in **TWO independent places** (the minted `note`, which the fix gates, and `dsrAttestProcedure` in the attest form, which it never touched), so matching the text passes on **either** while the governed arm regresses freely. Pin the `note`. ⚠ `frontend` has **NOT** proven both arms in one run — they were verified in **different builds** — so the pin is new coverage, not confirmation.
🔴 **BUG-DSR-S3-007 — ✅ FIXED 2026-08-20 (browser-verified, `retired`=2/3 so not vacuous); unclosed until pinned.** The retirement fix landed in the **data layer only**: `getDsrOutcomeRecord` computed `retired` and stated *"the parts must sum"* in its own comment, while `dsr-outcome-record.tsx` **never rendered it** — a refusal read *"Revisões solicitadas 3 / Revisões concluídas 0"*, work a documented decision retired shown as work abandoned, **in the artifact delivered to the data subject**. True in the code that computed it, false in the document that carried it. A 4th instance surfaced in the same audit (`outstanding` counted retired as still-to-do). Body → [dsr-slice-3.md](../progress/dsr-slice-3.md).
⛔ ***This row was DELETED by the lead's own -006 rotation and restored on discovery*** — a lookahead-bounded regex swallowed it; `cmp` at the destination proved the **move**, not the **cut**; `lint:progress` has no *"a row that existed must still exist"* check. Full incident → [dsr-slice-3.md](../progress/dsr-slice-3.md).

⬛ **FUP-DSR-ATTESTED-TIER-DERIVES-PENDING — ✅ RESOLVED 2026-08-20.** The attested tier exposed no `pending`, so the renderer **derived** it as the remainder, making an exact-sum check a **tautology that cannot fail**. Shipped first as a weaker but **falsifiable** at-most guard (falsifiable beats strong). `backend` then added `attested.pending` **counted from the rows, not derived** — verified by `frontend` rather than taken — so an exact check is now genuinely falsifiable. ⚠ The renderer tightens **after** `tester`'s pass: a mid-pass edit would surface as a UI red that is really an unannounced change.
🟠 **BUG-DSR-S3-004 — a SPEC defect: the outcome-record locator cannot reach values the render shows correctly.** `dsr-slice3-adjudication.spec.ts:822`, `element(s) not found` on an `.sr-only` chain while the artifact shows every asserted value right. ⛔ **The close-SUCCESS path remains UNEXECUTED** — the test dies on its FIRST assertion, before the close click; repairing the locator makes it *reachable*, not green. A second failure hides behind it (`Menções removidas` matches 4×). Repair written, **unrun**. Body → [dsr-slice-3.md](../progress/dsr-slice-3.md).
🟡 **BUG-DSR-S3-005 — a SPEC defect: an attestation loop builds its completion signal out of an ABSENCE.** `:714` is flaky; `router.refresh()` remounts the list, which passes transiently through **0**, and `toHaveCount` succeeds on the first matching poll — so a **remount gap is indistinguishable from completion**. The removing-a-subject vacuity family, in a legal-record corridor. Repair (identity-keyed poll on persistence) written, **unrun**. Body → [dsr-slice-3.md](../progress/dsr-slice-3.md).
🔴 **BUG-DSR-S3-001 — ✅ FIXED 2026-08-20, browser+DB proven; unclosed until `tester` re-runs the corridor.** The adjudication panel posted `taskId` where the door validates **meeting** ids, so escalation silently never happened (HCDS2) and — adjudication being the ONLY minter of `dispose_meeting` — **ADR 0056 Consequence (a) was not discharged**. Fixed structurally (`Omit<…,"taskId">` ⇒ compile error, not runtime refusal) and `taskId` dropped from the RPC jsonb too. Pass condition was the **minted row**, not a clean toast. ⚠ **The escalation fieldset does NOT render on seed data alone** — a spec that does not CONSTRUCT the meeting↔case link passes **vacuously**. Body → [dsr-slice-3.md](../progress/dsr-slice-3.md).
🟠 **BUG-DSR-S3-002 — ✅ FIXED 2026-08-20; unclosed until pinned in E2E.** The attested tier could not name the commission to review (RLS-filtered embed; the Encarregado is a plain member of one commission **by design**), so a named human would attest to an **unnamed scope** in a legal record. Fixed with a DEFINER lister gated by `dsr_tasks_select`'s own expression — `commissions`' read policy does **not** move. ⚠ `350` t58/t60 are **complementary, neither sufficient**. Body → [dsr-slice-3.md](../progress/dsr-slice-3.md).
🟡 **BUG-DSR-S3-003 — ✅ FIXED 2026-08-20; unclosed until pinned.** A by-design null-commission task rendered as an access failure. Split **three** ways (name shown · hospital-scoped, no commission · genuinely unreadable) — collapsing any pair loses a fact. Body → [dsr-slice-3.md](../progress/dsr-slice-3.md).
🟠 **BUG-XREF-CASE-ENTITYCODE-NULL — ✅ FIX LANDED 2026-08-20, pinned; unclosed until the slice gate passes.** `app.patient_trajectory_bundle` compared a `patient_participants` id to `cases.id`, so **every** case in **every** patient search rendered `—` — the PQS console too, outside this slice's subject. Same grain defect Slice 2 found, surviving in a second function; **no test pinned that value at all**. Body → [dsr-slice-3.md](../progress/dsr-slice-3.md).
🟠 **BUG-DSR-COMPLETE-OVERWRITES-NOTE — ✅ FIX LANDED 2026-08-20; unclosed until the slice gate passes.** `complete_dsr_task` overwrote `dsr_tasks.note` — the column carrying an `attest_review` task's **revoke-corridor instructions** — erasing what the reviewer was told to do, in a workflow whose output is a legal document. `note` is now the immutable procedure; `completion_note` carries the human's statement. Body → [dsr-slice-3.md](../progress/dsr-slice-3.md).
...
- 🟠 **BUG-ADM-APPOINT-CAPS-NOT-SYNCED** — appoint syncs `appointed` but not `caps`, so Amdt 5's auto-granted `read_cases` renders **UNCHECKED** until reload. DB correct — UI state-sync only, not an authz defect. ⛔ Do NOT fix by pre-checking client-side. Pinned red-on-purpose by B5, whose 2nd assertion has **never run**. [Detail](../progress/case-surface-split-increment-2.md) — frontend ✅ **FIXED 2026-08-22** (`a514d169`): `caps` was seeded from a prop with `useState`, so it ignored every later prop value — the grant landed, the props carrying it arrived, and the component discarded them. Now reconciled from the server, keyed on prop **content** not identity. ⚠ B5's second assertion (*unchecking empties the board*) had **never executed** before the fix and passed for the first time after it — not a return to green. MERGED `be546bbf`.
...
[bug-log-archive.md](./archive.md). ⛔ **It sat here wearing a 🔴 for hours after it was
...
closure narrative rotated verbatim → [bug-log-archive.md](./archive.md).
...
| BUG-NOTIF-EXPIRY-001 | AE4 | MINOR | Four bodies resolved `staff_admin` holders from `public.memberships` without `app.has_role`'s seat-expiry term, so a LAPSED coordinator kept receiving notifications: `app.compute_due_charter_notifications`, `app.compute_due_document_review_notifications`, `public.compute_due_notifications`, `public.save_section_answers`. Found by the AE4.3 matrix derivation (matrix § 1.2), dispositioned (a). | A lapsed seat resolves to nobody, matching the canonical predicate. | Lapsed coordinators received governance CONTENT, not a ping — measured verbatim: "A comissão {name} está com a cadência de reuniões em atraso" and the controlled document's code + title. ✅ No PHI, no Rule 12 consideration (nothing touches event_patient / referral_patient / patient_identifiers). ⛔ NOT in scope and not defects: `list_approver_candidates` (display label only; its door gate is already `is_staff_admin_of`) and `revoke_role_impl` (scope dispatch, not holder resolution). | `supabase/tests/402_d2_notification_expiry.sql` | backend | RESOLVED 2026-09-01 — migration `20261003007180` adds the canonical term to all four via the house `pg_get_functiondef` + `replace` + `execute` pattern, with landing assertions in BOTH directions (a no-op replace raises; every body is re-read from `pg_proc` after execute). pgTAP 402: catalog-positive per body plus BOTH polarities per body — live seat still notified (the over-narrowing guard, which is the failure mode of a narrowing) and lapsed seat no longer notified. Mutation-proven: reverting the term in one body reds exactly 1.1 and 3.2. |
| BUG-PROF-INACTIVE-001 | AE4 | MAJOR | `app.can_manage_professional` called `app.has_role` DIRECTLY, bypassing the `app.is_active` gate that `app.is_staff_admin_of_for` wraps around it, so a DEACTIVATED or SUSPENDED principal kept Class-2 professional-identity authority ORG-WIDE. Found by AE4.5's differential oracle (pgTAP 403 §4.1) on its first real run. ⚠ PREDATES AE4 — not a regression this phase introduced. | A deactivated or suspended principal resolves to no authority, matching `is_staff_admin_of_for`. | Measured, rolled back: deactivated → `is_staff_admin_of_for=false` but `can_manage_professional=TRUE`; suspended → the same. Blast radius: the 13 doors gating on it (create/update/redact_professional_profile — CPF, licence number, specialty — set_professional_link_state, the ethics + case-assignment vocabularies, create_external_participant), and 13 of 13 hold `authenticated` EXECUTE. The app-layer sign-out is NOT a defence: JWTs are bearer tokens and deactivation does not revoke them; the window is the JWT lifetime. ✅ Singleton, measured: sweeping app+public for has_role-without-is_active returns exactly one name. | `supabase/tests/404_bug_prof_inactive_001.sql` | backend | RESOLVED 2026-09-01 — migration `20261003007190` adds `app.is_active(p_uid)` to the staff_admin arm ONLY, matching `is_staff_admin_of_for`'s shape, via the house `pg_get_functiondef` + `replace` + `execute` pattern with landing assertions in BOTH directions. pgTAP 404 pins both polarities plus a catalog-positive. ⭐ POSITIVE CONTROL: AE4.5's oracle cell STOPPED DIFFING after the fix — the suite that found it going quiet is the proof it landed. ⛔ The staff_admin arm only: `is_org_admin_of` gates internally, and wrapping `app.is_admin()` would be actively wrong — filed as FUP-CAN-MANAGE-PROFESSIONAL-SELF-CHECK-ARM. |
...
was added on rotation. Batch record: [adr-0137-batch.md](../progress/adr-0137-batch.md).
...
- **The `is_active` print-door prohibition was a rule for one day and is now RETIRED** —
  ...
  reasoning → [rules-archive](../progress/rules-archive.md).
...
⚠ **What did NOT close with it.** [dm5-po-decisions.md](../progress/dm5-po-decisions.md) § "Open items from
...
[follow-ups-open.md](../followups/follow-ups-open.md). — tester
...
[follow-ups-open.md](../followups/follow-ups-open.md) carries the generalized addendum. That index line stays live in
PROGRESS.md; nothing here is its only witness.

✅ **BUG-AE47C-LINKAGE-001 — CLOSED 2026-09-02, both casualties (PROF-CREATE app-code fix + UNKNOWN-RESOLVE spec-fixture fix) tester-verified GREEN on a fresh reset (13/13, exit 0 read directly).** Full narrative rotated → [bug-log-archive.md](./archive.md) (search `BUG-AE47C-LINKAGE-001`). ⚠ **Unrelated finding, not a regression from this fix:** a deliberate second consecutive run of `ethics-e4-participants.spec.ts` (same DB, no reset) reproduces `EXT-REUSE` failing — this is `FUP-RETRY-CHANGES-THE-FAILURE-MODE-ON-NON-IDEMPOTENT-TESTS` (line below), not a new bug; generalized (non-retry-specific) addendum in its [follow-ups-open.md](../followups/follow-ups-open.md) entry. — tester
