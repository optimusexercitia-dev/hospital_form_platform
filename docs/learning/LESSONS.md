...
| LEARN-076 | Gate integrity | Reading a gate's exit code is not gating on it; chained with `;` the RED prints, is read correctly, and the commit lands anyway. | `docs/followups/follow-ups-open.md` | prose only |
| LEARN-077 | Paraphrase drift | A paraphrase can hold a numeral and swap the metric under it — layer for search depth — and a figure echoed back from your own spawn prompt reads as corroboration. | `docs/features/ae4.md` | prose only |
| LEARN-078 | Approval hygiene | An approval dated before the defect it authorises fixing reads as more rigorous than one dated after, so it is the more damaging date to get wrong. | ADR 0183 | prose only |
| LEARN-079 | Register hygiene | Two registers counting one subject differently can both be right over different populations; derive each side's population before ruling either wrong. | FUP-DOOR-SWEEP-DOMAIN-GAP-WIDENED-BY-SET-VALUED-RESOLVERS | prose only |
| LEARN-080 | Coverage vs verdicts | A coverage verdict keyed to a policy's name survives an ALTER of that policy and no arm asks, because a census sees no newcomer; only a re-measurement note on the row shows it was re-earned. | `docs/reviews/authz-door-audit-findings.md` | prose only |
| LEARN-081 | Mutation testing | Under mutation a deny arm SUCCEEDS and performs the door's effect, so a deny arm placed upstream of a bare call on the same object consumes that call's state, aborts the file, and the harness scores ERROR rather than COVERED — a correct keystone whose measurement is thrown away. | `docs/progress/c2-tier1.md` | `supabase/tests/mutation/c2-command-door-neutralizer.sh` |
| LEARN-082 | Harness crash safety | A recovery path can be disarmed by the very failure it exists to survive: a restore that clears its own sentinel unconditionally erases the evidence when the restoring process is killed alongside it, and a degeneracy preflight tuned to a different mutation shape cannot see the strand. | FUP-C2-TIER1-INFLIGHT-SENTINEL-ERASED-BY-ITS-OWN-RESTORE | `.claude/rules/mutation-harnesses-are-not-killable.md` |
| LEARN-083 | Test execution | A pgTAP value assertion (`is`/`isnt`/`ok`/`cmp_ok`) evaluates its subject expression before the assertion is entered, so a door that raises inside it ABORTS the file instead of failing the test — a mutation harness reads that as ERROR and the verdict is lost, never earned. Capture the value under `lives_ok`, then assert on it. | FUP-C2-TIER1-VALUE-ASSERTIONS-ABORT-ON-AN-INLINE-RAISE | `supabase/tests/mutation/c2-command-door-neutralizer.sh` |
