...
> Last updated: **2026-08-08 — QO·B (org_admin/hospital_admin CONTENT WALL; ADR 0100 D12 +
> PO rulings Q1–Q9; migrations `20260915000000`–`…000500`; NO flag — subtractive by design;
> **328 files = 328 registered** on a fresh reset). See the **QO·B** section immediately below.
...
`app.is_commission_admin_of(_for)` is **NOT** the commission's own admin. Catalog body:
...
`is_commission_admin_of` term subtracts *exactly* org_admin + hospital_admin. A rename to
...
**Match `is_commission_admin_of` without a trailing `\y`, or enumerate both explicitly.**
...
- **`_case_caps`** still routes `is_commission_admin_of_for`, and that is correct: A4 left
...
- **`authorizeStaffOps` mirrors `is_commission_admin_of`'s hospital leg** (+ an `isInactive` check).
