...
**Closes when:** ⚠ **RE-CLAUSED 2026-09-11 — both options this field originally offered are REJECTED by the ruling** (superseded text, kept so the change is visible: *"Either a stated ceiling on `D` per principal with something that reds when it is exceeded, or a ruling that the org→hospital→commission tenancy model makes a large `D` unreachable in practice — recorded together with the census that shows it, so the next reader does not have to re-measure to find out whether anyone ever checked."* — a fixture-derived ceiling is the wrong control, and *"large `D` is unreachable"* would overclaim). It now closes when the **six-clause shape assertion of ADR [0208](../decisions/0208-the-candidate-fanout-is-structurally-dominated-and-empty-search-path-is-the-sole-forward-convention.md) D2 lands in its named unit `AE4-D-SHAPE-ASSERTION`** — every candidate originating from an entitlement-provider fact · one fact yielding at most one candidate per resolution kind · dedup before confirmation · `U = D ≤ F` measured on the existing P2 instrument (`scripts/authz-ae4-p2-invocation-count.sql`, ⚠ whose counter returns NULL not 0 cold) · runtime and candidate resolvers proven to share a candidate producer **by comparing the two live bodies** (today they are byte-identical **copies**, so a re-stated clause would be a hand-written copy of production text, LEARN-024) · a new provider adapter failing the assertion until it is included. ⛔ Not closed by a numeric ceiling, ⛔ not by an "unreachable" ruling, ⛔ not by P2 alone (0183's P2 proves the instrument can *see* `D`; it does not bound it), and ⛔ not by this ADR, which states the invariant and builds nothing. The census and the accepted residual risk are recorded in 0208 D1; D3's five re-measurement triggers are `prose only` until clause 6 exists.
**Ruling:** 2026-09-11 (PO, unit `AE5-SUCCESSOR-ADRS`) — ADR 0208 D1: *"structurally dominated, with the residual risk explicitly accepted"*, a **parametric structural invariant plus accepted operational risk**, `D ≤ F` and `Dₖ ≤ min(F, |scopesₖ|)` over a provider-neutral fact set, with `M ≤ C + R_H·H + R_O·O + S` and `R_H = 6` / `R_O = 2` / `S ≤ 1` named as **catalog facts, not permanent constants**; ⛔ no numeric schema ceiling is imposed. ⚠ `F` is counted from the **producer** (`authz.assignment_facts` — min 0 · max 3 · avg **1.27** on the seed, the `is_active` gate), which is a different grain from this body's recorded `M` over **membership rows** (min 1 · max 3 · avg **1.30**); both figures stand, at their own grains.
**Status:** open
