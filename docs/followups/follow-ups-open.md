...
**Status:** open — MEASURED 2026-09-06 from the one full run: **exactly 5 rows**, against a bound of 51. ⭐ All five COVERED → BLIND transitions in the whole run are `(ALL)` policies (zero SELECT rows flipped — the discrimination the subset could not give), and all five are CAPA `_write` policies covered by the SAME file `252_authz_p0_isolation.sql`, so this is ONE coherent read-path gap rather than five. ⚠ 18 further `(ALL)` rows went COVERED → NOTICED — unclassifiable, not flipped, and NOT work items here. ⛔ A flipped row is never relabelled to keep it COVERED
