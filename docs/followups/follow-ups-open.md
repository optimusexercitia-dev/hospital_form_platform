...
### 🟡 FUP-AE5-STAFF-SELFTEST-WRONG-ARM-CHECK-COMPARES-ON-THE-ARM-PREFIX — `gen-authz-differential-cells.py --self-test` decides "the RIGHT arm fired" on the `arm14:` prefix, so a fixture caught by a SIBLING sub-check reads as caught by its own (owner: backend)

**Filed:** 2026-09-14 (unit `AE5-STAFF`, observed by backend while adding arm14(i); relayed by the lead) · **Owner:** backend · **Severity:** medium — LEARN-103 made the self-test assert WHICH arm fired; the assertion is now one level too coarse. arm14(i)'s fixture was claimed first by the keying sub-check and then by the bound-door sub-check, and both times the self-test reported it caught — by "arm14". A sub-check that stops firing is therefore invisible while any sibling still refuses its fixture: the wrong-arm control has the shape of the instrument it was added to control.
