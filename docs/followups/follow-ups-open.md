...
**Status:** open — AE4 is the **first hub whose completion depends on a ledger row**, which is why a mismatch this old surfaced only now. ⚠ **CORRECTED 2026-09-08 (QA r4):** this read *"`| AE4 |` **currently** sits unbolded alone among 76 rows"*. Both halves are now false and the second was never right — `| **AE4** |` was re-bolded by this unit, dozens of rows are unbolded (the pre-convention era), and the ledger total was refuted then moved four times. ⛔ Present-tense counts do not belong in a register entry; derive them
