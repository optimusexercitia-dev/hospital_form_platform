...
**Closes when:** every count printed in an executed banner across `p0-authz-{door,writepath,invoker,rowdoor}-audit.sh` and `c2-command-door-neutralizer.sh` is DERIVED from the list it describes **and measures the quantity its own sentence names**, with the absence of a fresh literal asserted by a check rather than by inspection. ⛔ Re-typing a correct literal does not close this — a new literal is the same defect with a newer number.
**Status:** open
