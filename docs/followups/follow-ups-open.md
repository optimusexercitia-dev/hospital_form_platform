...
⚠ **MERGE NOTE 2026-09-08 (Batch 4 merge session, lead): this is ONE mechanism carrying TWO entries, filed the same day by two machines that could not see each other.** The twin is `FUP-AUTHZ-EMPTY-CASES-RUNS-A-FULL-SWEEP` (owner lead, body file, opened on the ENFORCEMENT-MANIFEST tip gate). ⭐ The two are **not redundant — they are different evidence for the same defect**: this one was found **by construction** on the write arm, the twin by a **live incident** on the door arm (a multi-hour full sweep that had begun rewriting the committed baseline before it was killed). ⛔ **Close on the twin's condition, which is strictly stronger** — both harnesses, a `CASES=""` self-test scenario proving the FINDING exit fires, and the lead-playbook §4 recipe reading the deriver's exit code **before** substituting its stdout. ⛔ Do **not** close this entry on the write-arm fix alone: half the class is exactly the shape both entries warn about. Consolidating the two ids is a PO call and is deliberately **not** taken here — retiring an id mid-merge orphans every citation that names it.
