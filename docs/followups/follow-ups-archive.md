...
_Original entry rotated → [follow-ups-archive.md](./follow-ups-archive.md) (2026-08-05)._
...
[phase record](../progress/quality-office-oversight-phase-b.md) + the 2026-08-09 Decisions rows in PROGRESS.md.
...
[eth-e4-participant-seating.md](../progress/eth-e4-participant-seating.md), review:
...
3. **Deleted** the `meeting-attachments` bucket row (via the transaction-local
   ...
   the separate open item in [f2-attachments.md](../progress/f2-attachments.md) § *Open risks*.
...
- 🟢 **FUP-DM1-CEILING** — **DB HALF DISCHARGED 2026-08-13 by DM2·S1** (`5dbeb76`; migrations `20260924000100`+`…000200`, ADR [0117](../decisions/0117-dm2-s1-confidentiality-ceiling-decisions.md), record [dm2-orchestration-wave-a.md](../progress/dm2-orchestration-wave-a.md)). The ceiling is live and **lead-verified against the catalog, not from the report**: on one case with one reader, label as the sole variable → plain `t` / `legal_privileged` `f` while `can_read_case` stays `t`, so the denial is the ceiling and not lost home access; the seam guard admits `ethics_investigation` on a meeting home and refuses `legal_privileged` (HC0D6, pt-BR). **Remaining to close:** pgTAP `228` **t40** + the two open-door pins (S1-O1) ride S2's `open_document_version`, and E2E **AC-4a–d / AC-9** ride S4. — **RULED 2026-08-13 (ADR [0114](../decisions/0114-document-model-redesign.md) Amendment 1 D15/D16)**: option 1 — re-express the ADR 0072 D7 ceiling as a nullable confidentiality column on `documents` + a `can_read_document` kernel arm, **interim**; the general access plane is **scheduled at Phase 19** (landing point `documents.access_policy_id`). No longer a blocker — now a **DM2 Wave A PREREQUISITE** (build before Wave A re-points any document). Option 3 rejected. Discharge = D15 shipped + pgTAP `228` t36–40 and E2E AC-4a–d/AC-9 restored as an ADR 0114 amendment — PO/lead → [body](./follow-ups-open.md)
- 🟢 **FUP-DM1-E2E** — ✅ **DISCHARGED 2026-08-13 by DM2·S4** (parking done 2026-08-12; rewrite done at S4). All 6+1 attachment-touching specs/blocks rewritten against the document model per ADR 0114 D5 (rewritten, never merely deleted). **Lead-verified at the source, not from the S4 report** (QA r1 MINOR-6 was that this row contradicted the S2/S4 rows it should agree with): the M8 bytes-cut assertion is live in `e2e/quality-oversight.spec.ts` and was since **strengthened past its original form** — it now calls `open_document_version` directly via REST rather than asserting the download button's absence (QA r1 P0-1; `56e3989`). ⚠ One residue, routed to tester: that file's **header comment at `:39` still reads `⛔ PARKED`** while the assertion below it is restored — the repo's own "a comment is an assertion that goes stale silently" class. — tester + DM2 ✅ → [body](./follow-ups-open.md)
- 🟢 **FUP-DM1-DISPOSE** — ✅ **DISCHARGED 2026-08-13 by DM2·S2**: `dispose_case_phi` carries its case-homed-document arm again (the DM1 substrate drop had cost it the attachment-redaction step — a PHI erasure path losing a step; no-op at the time, zero rows carried bytes). **Lead-verified from the live catalog** (`pg_proc.prosrc`, per the standing catalog-over-migration-text rule), not from a commit message. Discharged **before** Wave A's flags flip, as required — backend ✅ → [body](./follow-ups-open.md)
...
> The item's own row is updated in place at [dm5-po-decisions.md](../progress/dm5-po-decisions.md)
...
([door-erasure-freetext-census.md](../progress/door-erasure-freetext-census.md)) is retained precisely
...
> [door-erasure-freetext-census.md](../progress/door-erasure-freetext-census.md): 133 candidate columns
...
[door-erasure-freetext-census.md](../progress/door-erasure-freetext-census.md). Headlines, both
...
_Moved when the ADR 0137 § Now bullet rotated to [2026-Q3.md](../progress/2026-Q3.md): that bullet's item 2 was the only live register indexing this body, so rotating it left the body orphaned — `lint:progress` caught it as residue in the same edit, which is the check existing._
...
### 🟠 FUP-RESET-ROLE-DOES-NOT-CLEAR-JWT-CLAIMS — a pgTAP premise 136 files can state falsely (owner: backend/tester; filed 2026-08-22, found inside the ADR 0134 S8 suite; ⭕ **PARTIAL 2026-08-22** — root verb `test_helpers.reset_role_and_claims()` + red-first gate landed and adopted in `356`/`357`; ⛔ **step 1 (derive the real population) and the 134-file sweep remain OPEN**. Capable population 136 → 134, still not a defect count. Record: [case-split-assertion-integrity.md](../progress/case-split-assertion-integrity.md))
...
> ⛔ **CORRECTED 2026-08-31 — that last sentence was FALSE for two of the six when written.**
> `FUP-DM5-CLOUD-ORPHAN-SURFACE` and `FUP-DM5-BACKUP-IS-PHI-EXPORT` still had their bodies in
> `follow-ups.md` and none here; they were rotated on 2026-08-31 (§ "the TWO bodies the
> 2026-08-27 note said were already here", at the end of this file). ⭐ **The note describes the
> exact mechanism that should have stopped it** — "the notes could not leave PROGRESS.md while
> the bodies were still in follow-ups.md" — and that mechanism did not fire, because the residue
> check accepted a **prose mention** (both ids appear inside § Critical FUP C1/C3 row bodies) as
> an index. A rotation was recorded complete, and the record became the evidence. ⚠ The claim
> was checkable the day it was written: `grep -c '^### .*<id>'` in each file, two commands.
...
>
> Detail: [authz-ae2.md](../progress/authz-ae2.md) · verdict: [authz-ae2-review-r3.md](../reviews/authz-ae2-review-r3.md).
...
## ↩ Rotated from follow-ups.md 2026-08-31 — the TWO bodies the 2026-08-27 note said were already here

⛔ **This completes a rotation that was RECORDED AS DONE ON 2026-08-27 AND WAS NOT.** The six-note
block above states, of the ids whose PROGRESS.md rotation notes it moved: *"for these six ids the
body is **in this file, above**."* Measured 2026-08-31, that was **false for two of them** —
`FUP-DM5-CLOUD-ORPHAN-SURFACE` and `FUP-DM5-BACKUP-IS-PHI-EXPORT` each still had exactly one body
heading in `follow-ups.md` and **zero** here. The bodies are moved below, verbatim; no repoint was
needed (all three of their links were already `docs/progress/`-relative).

⭐ **Why nothing caught it for four days — and it is the same hole, one level up.** That block
explains its own method: *"`lint:progress`'s residue check holds a body in place until some live
register indexes it, so the notes could not leave PROGRESS.md while the bodies were still in
follow-ups.md."* The notes left anyway and the gate stayed green — because both ids are named in
ordinary prose **inside § Critical FUP C1's and C3's row bodies** (C3: *"it INVERTS its parent:
`FUP-DM5-BACKUP-IS-PHI-EXPORT` graded an over-wide copy existing"*), and the residue check was
`progressText.includes(id)`. So the mention hole did not merely hide a closed item: it **certified a
half-finished rotation as finished**, and then the note recording it became the evidence. The check
was tightened to require a real index ENTRY on 2026-08-31; these two surfaced on its first run.

⚠ The third id of that group, **FUP-DM5-NO-ANSWER-VS-NOTHING**, is NOT affected — its body stays in
`follow-ups.md` by PO ruling 2026-08-28, and its PROGRESS.md note is a real entry (leading bold
token) under the tightened check, exactly as that ruling intended.
...
>
> Full run record: **[cloud-orphan-probe-2026-08-18.md](../progress/cloud-orphan-probe-2026-08-18.md)**.
...
### ✅ FUP-AUTHZ-HARNESS-TRANSACTIONAL — the door-audit harness neutralizes OUTSIDE a transaction, so process death leaves an authz gate OPEN (owner: lead + backend; filed 2026-08-14, DM5 S2, after it happened) — **RESOLVED 2026-09-04**

> **RESOLVED 2026-09-04 — by a PO ruling, not by a build.** ⛔ Nothing closed here makes a harness
> self-healing, and that is the decision rather than a shortfall of it: the **detect-only posture is
> accepted**. Design and the guards it now leans on: ADR
> [0189](../decisions/0189-one-crash-safety-protocol-across-the-mutation-harnesses.md) D7, built on
> `authz-harness-crash-safety` in commits `18e9d7db`, `fbe4954b`, `bbb86bca`, `3cb7a0d3`,
> `a8148126`, `e31af593`.
>
> **PO ruling 2026-09-04**, ruled by the **PO** on question Q2 of the HARNESS-CRASH-SAFETY plan,
> quoted verbatim:
>
> > "The filed fix (one rolled-back transaction) is unbuildable and would be worse than the bug —
> > the probe is a separate process, so every case would classify COVERED against the original
> > gate: a sweep 100 % green and 100 % vacuous. The detect-only posture is accepted for the
> > mutation harnesses: process death may still leave a gate open, and the guarantee is that it
> > cannot do so unnoticed — a verified restore, a sentinel that survives its own failed restore,
> > `RECOVER=1`, the degenerate-body preflight before every arm of `p0-authz-invariant.sh`, and
...
>
> **The lead's recommendation was the opposite, and it was CONSIDERED AND NOT TAKEN.** The lead
> recommended **building** the committed marker row (a `c2n_sentinel.inflight` row written inside
> the same `DO` block as the neutralization): a file sentinel can be separated from the damage by a
> different `TMPDIR`, a different machine or a cleaned scratch directory, the marker cannot, and it
> would date the neutralization — closing the forensic gap that `pg_proc` carries no mtime. ⛔ **The
> marker was measured BUILDABLE and was not built by decision, not by inability** — `mutate()` runs
> its DDL inside one transaction, so an `insert` in that block is atomic with the neutralization,
> and a home outside the dropped `c2n` schema was already chosen. Its cost — a persistent scratch
> schema living in the production migration lineage's database, surviving every reset that the
> `c2n` schema does not — is what the ruling declines to pay.
>
> **Re-open trigger, in the ruling's own words:** *re-open if a harness is ever run against a
> database with more than one owner.* Until then the residual is closed, not parked: single-owner
> local stacks are the only place these harnesses run, and `supabase db reset --local` is the blunt
> certain remedy there.
>
> ---
>
> **The entry as filed, verbatim** (its body file is retired with it):
