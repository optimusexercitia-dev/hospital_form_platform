...
| app.can_administer_person_for(p_capability text, p_user uuid, p_actor uuid) | predicate | positive | ERROR | run-shape!=baseline (Files=236 Tests=7848; baseline Files=236 Tests=7855 PASS). AE1.3's shared person-authority predicate, new at `63230a84`; `ARM=census` named it and nothing else. Subset run on a FRESH reset, `ARM-DOMAIN predicate=1/112 policy=0/226`; merged here as a CHANGED ROW — the subset file was never copied over this one, and the committed baseline was cksum-verified unchanged by the run (ADR 0079 Amdt 1). ⛔ **ERROR is not a pass.** Neutralizing the WHOLE function to `select true` makes a suite ABORT rather than fail — 7 assertions stop RUNNING, which is why the harness withholds a verdict, correctly. Same class as `app.event_current_custodian` above: `FUP-DOOR-SWEEP-BROAD-GATE-ABORTS-A-FILE`. ✅ **What DOES cover it** (CLAUDE.md §6 step 1's stated remedy for ERROR — "cover it in the phase's mutation audit"): `supabase/tests/mutation/ae13-person-doors-mutation-audit.sh`, whose cases 10–14 neutralize this function's INDIVIDUAL conjuncts (empty-footprint pin; `ended_on is null`; `voided_at is null`; the D2 tier check; the INTERSECTION↔SUBSET swap) rather than the whole body — all five KEYSTONE HOLDS, red under mutation, green restored, rollback hash exact (2026-08-27, quiet stack). |
