...
|---|---|---|---|---|
| public.rca_writer_can_write(p_rca_id uuid) | predicate | positive | BLIND | ⭐ **ARM WIDENED (ADR 0079 Amendment 9 / `FUP-DOOR-AUDIT-PREDICATE-ARM-BOUNDED-BY-A-NAME`).** This gate entered the predicate arm on 2026-08-24, when the arm stopped bounding its domain by NAME alone and began admitting a `prosecdef` boolean whose BODY references an identity primitive. It had never been swept in any direction before; it was in `authz-unswept-backlog.txt`. Measured by a subset run on a FRESH `supabase db reset`, baseline `Files=218, Tests=7223, PASS`, `ARM-DOMAIN predicate=8/110`; transcribed here because a subset run overwrites this file and is then reverted. ⛔ **BLIND: opening it reddened NOTHING across 218 files.** The first finding this widening produced, and the reason it was worth making — registered as `FUP-RCA-WRITER-CAN-WRITE-IS-BLIND`.  ⭐ **NO LONGER BLIND as of 2026-08-24 — hand-annotated, verdict column left alone.** Keystoned at `300_rowdoor_gate_keystones.sql` §1.10 + its §2.10 twin and proven by mutation: neutralize the body to `select true` and 1.10 reds ALONE (1/20), restore and the suite returns to 7230 PASS. ⛔ The verdict cell is NOT flipped by hand — it is this sweep's output and the next full run regenerates it; this cell is the record that the run is stale, not a substitute for it. ⚠ The keystone asserts the door's OWN output, not a row count through it, because **nothing in the catalog calls this wrapper** — all eight rca write policies call `app.can_write_rca` directly, so no write-through-the-door assertion could ever red. That duplication is filed as `FUP-UI-AUTHZ-WRAPPERS-DUPLICATE-THE-ENFORCING-PREDICATE`. Its entry in `authz-neverclled-door-allowlist.txt` was REMOVED in the same change — with the keystone calling it, the floor arm now reddens if that keystone is ever deleted (floor never-called count 73 → 72). |
