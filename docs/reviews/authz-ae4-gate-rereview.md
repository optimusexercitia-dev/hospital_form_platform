...
| A7 | **post-signature; the VERDICT IS UNCHANGED** | Recorded by the records pass, 2026-09-04, **not** by the reviewer. Two commits landed after this file's last edit (`f397ff8b`) and closed five items it still lists as open: `01628bb2` closed **LOW-1** (`413` test 5's caption narrowed to arity, pointing at § 3a/3b/3c for the behavioural half), **LOW-2** (`…7320`'s postflight narrowed to AGREEMENT, comment-only — zero SQL bytes), **LOW-8** (both records now state the split, the backlog's ONE marked DERIVED from the THREE) and the **LOW-3** partial (`409` three→four, `401` fifteen→27); `05ba7925` gave **ADR 0183** its `Approved:` line, closing LOW-5's second half. ⛔ So **§ 3's five `STILL OPEN` cells, its `13 · 1 · 4` headline, and the four repetitions of *"ADR 0183 still carries no `Approved:` line"* (§§ 3, 5, 7 and CV-list) are stale as of 2026-09-04** — annotated here rather than rewritten in place, because a signed verdict's tables are a record of what was true at `1ac811fe`. Live tail: **4 open** — 3 broad (one is C2) + 1 of my own — none blocking. ⚠ One further residual: CV4 still reads *"its **write arm holds no verdict from anyone** (N3)"* after N3 closed at `27ec066a`; § 3's MAJOR-5 row is the correct reading. |

⭐ **A7 is the class this report itself filed.** A verdict-bearing table goes stale the moment the
tree moves past its commit, and nothing gates it — the same defect as F-REC-6 and F1, one artifact
later. Counts here are as-of `1ac811fe`; **re-derive from the tables, never quote the headline.**
