...
ten lint gates, the three affected unit-test files (47 tests), and pgTAP 371+372 were re-run green by
...
### B1 · AUD1's fix does not reach one live writer, and ADR 0149's superset Consequence is false — *measured*
...
This directly falsifies `docs/decisions/0149-org-admin-reads-hospital-tier-audit.md` Consequences:
...
exact inversion ADR 0149 exists to remove, surviving in the one action class nobody looked at.
...
`affiliation.created, membership.granted`), so the 19/19 parity figure could not see it. pgTAP 372 is
blind for a subtler reason that is worth stating: `372_audit_org_leg_hospital_tier.sql:129-134` (§0.1)
...
2. A pgTAP arm in 372 that writes a hospital-tier row **the way the trigger does** (no
...
3. Correct or qualify the superset sentence in ADR 0149's Consequences.
...
| file:line | claim |
| --- | --- |
| `docs/decisions/0148-ever-held-affiliation-read-visibility.md:141` | "**`FUP-AFF2-ACTIVE-MEANS-TWO-THINGS` closes.**" — argued from "all three policies now contain zero `ended_on` **and** zero `expires_at`" |
...
Commit order explains it — `d150321b` (fix + ADR 0148) landed first, `dbf2efc2` reversed the ruling
after — but neither ADR 0148 nor `backend-state.md` was updated. The ADR is now the **only** authority
...
### B3 · `docs/backend-state.md` does not document ADR 0149 / migration `20261003003000` at all — *measured*

Zero occurrences of `0146`, `AUD1`, or `20261003003000` in the file. The AFF3 half is documented well
...
- **Does AFF3 cross hospitals or orgs?** No. The leg stays keyed to
  `app.is_hospital_admin_of(ha.hospital_id)`; the change is purely tense. pgTAP 371 §3.3/§3.4 pin the
  sibling-hospital boundary with a positive twin, and §3 of 372 pins cross-org with the control on the
  same session.
...
- pgTAP `371:290-320` §4 proves the RLS half with the right control — §4.1/§4.2 are silent
...
| mutation | result |
| --- | --- |
| restore `ended_on` on **only** `profiles_admin_select` (half-applied migration) | **1 arm red — §5.2 alone.** Every behavioural arm stayed green. This confirms ADR 0148 D2's claim verbatim, and it is the reason §5 exists |
...
| **widen leg 5** (drop `organization_id IS NULL`), leg 4 left correct | **3 red: §5.2, §5.3, §6.5** — exactly the three arms ADR 0149 D4 names |
...
ADR 0147 holds on all three requirements: `maskCpf` is **not exported** (`person-footprint.ts:197`),
...
**M1 · ADR 0149 amends `0041:99` without naming it** — *read*. `docs/decisions/0041-multi-tenancy-organizations-hospitals.md:99`
...
**M2 · ADR 0148 invokes a data class whose defining control it declines, citing two superseded ADRs** — *read*.
...
admin's read; after AFF3, nothing ever does. ADR 0148 D5 accepts "unbounded in time" for *legitimately
...
**M4 · ADR 0148 D6's "therefore" over-generalizes** — *read*. "A departed person therefore has an empty
...
components), but `person-footprint.ts` is precisely the module ADR 0147 designates as the sole
...
collapsing into (c). ADR 0147 D4 keeps `cpfPresent` for exactly this reason; the rail does not use it.
...
| | |
| --- | --- |
| **Blocking** | B1 (AUD1 incomplete for `standard_ownership.*`; ADR 0149's superset Consequence false — *measured*), B2 (three docs vs. two on a rejected closure), B3 (`backend-state.md` omits ADR 0149 entirely) |
| **Minor** | M1–M12 |
| **Verified sound** | Boundedness of both widenings (no cross-org / cross-hospital / cross-tenant admission); the read/write asymmetry; the falsifiability of both pgTAP suites and the new unit test; ADR 0147's CPF containment; Rules 1, 10; Tailwind v4 tokens; `PersonalDataCard`'s three states |
...
migration `20261003003100`, with preconditions asserted in the same session before the measurement.
...
The inversion ADR 0149 exists to remove is gone for this action class, and no tenant boundary moved.
...
retroactively, exactly as ADR 0150 D4 claims.
...
and **0** hospital-tier rows still org-NULL. ADR 0150 states "adds derivation and adds no validation"
and test 373 §1.3 deliberately pins that a caller-supplied disagreeing org **wins**. Honest and
...
| **B** — write-side derivation removed (`v_org := v_org`) | **PASS** — 372 is structurally blind, exactly as round 1 said | **§1.1 red** + **§2.1 red** |
...
Mutation **B** is the one I most wanted: it isolates the write half, and 372 stays green under it.
That is the finding of round 1 reproduced as a *measurement of the test suite*, and it is why 373 had
to exist rather than 372 being extended.

### 372 §6.4 is **strictly stronger**, not merely narrower — the concern was well placed and it holds
...
mutation C3 reds 372 §4.1 and 373 §2.2. This is a strengthening, not a relaxation-to-pass.

---

## 3 · The no-backfill reasoning — `backend` is right, and ADR 0150 already says so
...
**ADR 0150 D3 is correct as written** — it lists the guard *first* ("barred twice over"), then the
...
| **M1** | **CLOSED** — ADR 0149's header now declares `**Amends:** 0041 … , 0051`, and 0041 carries the generated back-pointer. `lint:adr-index` green (145 ADRs, next free 0147) |
...
`src/lib/users/person-footprint.ts:236-238` now says the field stays "…because ADR 0147 D4 requires
...
it. **One-line edit:** say the field is retained by ADR 0147 D4 as a fact with no current consumer.
...
read — *measured*.** `guard_audit_immutable` appears in ADR 0150 and **nowhere else**: zero occurrences
in `supabase/migrations/20261003003100_…sql` and zero in
...
migration `20261003003100`.** `p0-authz-door-audit.sh:565` writes the **tracked**
...
| **Verified by measurement** | B1 both halves, incl. chain neutrality and `verify_audit_chain`; the class-level nature of the fix; all four of 373's ⭐ keystones falsifiable; 372 §6.4's replacement **strictly stronger** via a five-policy truth table; 372's blindness to the write half reproduced; the guard-trigger bar and the hash break as two independent facts; the org/hospital agreement invariant and its absence of a constraint |
