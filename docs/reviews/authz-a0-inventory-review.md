...
# PART II — v2 review · the A0 delta

**Date:** 2026-07-15 · **Reviewer:** `qa` · **Subject:** `docs/progress/authz-capability-inventory.md` **v2**
**Method:** live catalog only — `pg_proc` (incl. `prosecdef`), `pg_policies`, `pg_policy`, `pg_class.relacl`,
...
followed** — it does not index SQL, and file text is stale by this program's own METHODOLOGY FINDING.

# VERDICT: CHANGES REQUESTED

**Is A0 v2 a sufficient foundation to author M1's SQL against? — NO. Not yet, and by a narrow, specific margin.**
...
> **A0 v2's fix set does not achieve that purpose.** A **sixth** exclusion-plane table —
> `case_participant_roles`, the second table in `is_case_respondent`'s **own join** — carries
...
> no RPC and no audit record** — and `is_case_excluded` flips **t to f** for **every case in the
> organization**. **Proven live, rolled back (V-3).**
>
> Ship M1 as scoped and Row zero is **still not durable** — while a keystone asserts that it is. That is
...
## V-0 · Corrections to my own v1 — visible, not silent

`backend` pushed back on two counts. **I re-ran both myself. On both, `backend` is right and I was wrong.**

### V-0.1 · C1a is CONFIRMED. My v1 fixture does not reproduce. (v1 §7 is WRONG)

I offered my probe as *"the exact fixture; it is seeded and ready (`staff4.ccih@test.local` / case `ca…e1`)."*
**It is not.** The catalog:

```
 cp_id  fd…e1 | case_id ca…e1 | resp_uid 00…0a | staff4.ccih@test.local
 is_staff_admin_row = 0 | seeded_roles = staff        <-- plain staff, NOT staff_admin
```

**PROBE 2 — my v1 fixture run verbatim:**
```
 BEFORE | respondent t | excluded t | can_read f | can_read_phi f
 set local role authenticated;  select public.remove_case_participant('fd…e1');
 ERROR:  apenas a coordenação pode gerenciar participantes deste caso   <-- correctly DENIED (HC0E4)
```
...
### V-0.2 · C4 is CONFIRMED. My "30" was a floor. (v1 §6·1 is WRONG)

I condemned Q7's `is_commission_admin_of` filter as A23's blind spot **and then re-ran it** to produce my
own number. **Q7·v2 re-run verbatim → 49 rows**, partitioning **exactly** as v2 claims:

| Bucket | v2 claims | Catalog | |
|---|---|---|---|
| **Population** (4 arms) | 49 | **49** | OK |
| **DEFINER · no exclusion · no read gate — THE FIX SET** | 35 | **35** | OK |
| DEFINER · no exclusion · has read gate | 4 | **4** | OK |
| DEFINER · **has** the exclusion gate | 3 | **3** | OK |
| INVOKER — DO NOT FIX | 7 | **7** | OK |
| **`platform_admin` (`is_admin()`) arms** | 4 | **4** | OK |

**C4 accepted; my 30 becomes 35, my 37 becomes 49.**

### V-0.3 · A **third** v1 error `backend` did not catch — and v2's stricter query is what exposed it
...
The string `can_read_case` appears in that function **only inside a comment** —
`-- COORDINATOR-ONLY write gate (not can_read_case — that is the broad READ scope)`. My looser pattern
matched the comment. **v2's `%can_read_case(%` is correct and mine was not** — and my error ran in the
*dangerous* direction: it credited a PHI-write door with a read gate it does not have.

### V-0.4 · Lead correction accepted — `is_admin()` is **not** a forgeable claim

v2 §1.6 writes that `is_admin()` *"also honours a **JWT claim**"* in a tone that reads as an escalation
...
until refresh) — already backlogged as session revocation. **v2 must re-word this; it must not ship as a
privilege-escalation finding.** *(This is a wording fix, not a gate item.)*

---

## V-1 · The three probes, re-run by someone who did not write them

Per `backend`'s own closing proposal — now a standing rule — **every probe offered as a fixture is re-run
by someone who did not write it.** I am that someone for v2, as `backend` was for me.

| Probe | Reproduces **exactly as documented**? |
|---|---|
| **PROBE 1 · `lift_recusal`** (recusal arm) | **YES** — `recused t to f`, `can_read f to t`. Post-rollback: 0 rows leaked. **But see V-1.1 — it under-states itself.** |
| **PROBE 2 · `qa` v1's fixture as documented** | **YES — it reproduces as a DISPROOF.** `HC0E4`, correctly denied. **v2's C1a is right.** |
| **PROBE 3 · respondent arm + the precondition** | **YES, exactly** — `respondent t/f · excluded t/f · can_read f/t · can_read_phi f/t`. Post-rollback: `respondent=t`, 0 leaked `staff_admin` rows, `case_participants` row intact. |
...
### V-1.1 · Both v1 and v2 **under-state `lift_recusal`** — it is also a PHI door

v2 §1.1 records: *"`lift_recusal` — PROVEN LIVE: … `can_read_case` f to t"*, and frames the respondent arm
as **"strictly worse"** *because* it reaches PHI. **My PROBE 1 says that framing is wrong.** Running it
against a **coordinator** (`chefe.ccih@test.local` — which is the only principal `lift_recusal` is *about*):
...
change (both are already in §3.6·A1) — **the severity narrative does.**

---

## V-2 · The five accepted corrections — all VERIFIED

| # | Claim | Catalog verdict |
|---|---|---|
| **C2** | The 10 "REMOVE-ARM only" DEFINER RPCs re-classified **FIX** | **CORRECT.** Confirmed in Q7·v2: all 10 are `definer=t, excl=f, readgate=f`. Stripping `is_commission_admin_of` leaves `is_staff_admin_of` unqualified — the A22 defect. v2 concedes v1 was wrong; the concession is right. |
...
| **C7** | `case_tag_report` category error | **CORRECT.** `case_tag_report(p_commission_id uuid, p_from date, p_to date)` — **no `case_id`**. The prescribed gate is genuinely unwritable. The gates-vs-per-row-filters split (§3.6·A/B) is the right structure. |
| **C3** | 13-vs-15 header corrected; exact partition published | **CORRECT** (V-0.2). |
| **C9–C12** | `case_interviews_insert` · `case_access` grant pattern · `record_recusal` · B7 write path | **ALL CORRECT.** `case_interviews_insert` `with_check = (is_staff_admin_of OR is_commission_admin_of)`, no case term. `record_recusal` splits self/other but has **no exclusion term** (`excl=f` in Q7·v2). B7: `update_professional_profile` has no `p_user_id`; FK is `ON DELETE SET NULL`. |

### V-2.1 · The action-column re-audit (§3.7) — claim of "0 further over-reach" is VERIFIED

Over-reach is the error that breaks legitimate admin surface, so I checked **both directions**:

| Direction | Result |
|---|---|
| Marked **KEEP** but genuinely case-scoped content | **0 — CONFIRMED.** `case_tags` / `case_outcomes` / `case_narrative_types` are keyed on `commission_id`; `case_types` / `case_participant_roles` on `organization_id` via `is_admin() OR is_org_admin_of`. All genuinely configuration. |
| Marked **FIX** but INVOKER (RLS already protects) | **0 — CONFIRMED.** All 7 INVOKER RPCs sit in bucket D. The load-bearing negative **still holds**: `close_case`/`cancel_case`/`set_case_outcome`/`update_case_narrative_body` are `prosecdef=f`. **Do not sweep them.** |
| Marked FIX but unwritable | **1, fixed** — `case_tag_report` to bucket B. |

**v2's "0 further over-reach" is accurate.** *(But see V-4: there is under-reach — a different error.)*
...
wrong."* It was. **I verified every leg independently. Every leg holds — on the five tables v2 chose.**

### V-3.1 · The legs, verified three ways

| Leg | v2 claims | My verification |
|---|---|---|
| 0 write policies | 0 | **0** — and I schema-qualified it (v2's Q15 did **not** filter `schemaname`). All 5 `relrowsecurity=t`. |
| **0 `authenticated` DML grants** | 0 | **0**, verified **three** ways: `role_table_grants` (SELECT only) · **`role_column_grants`** — the repo's `case_referral` precedent — **0 rows** · and decisively the raw ACL: **`authenticated=r/postgres`** (read only). `pg_auth_members` shows **no role inheritance**; no `PUBLIC` ACL entry. |
| All 6 mutators call `audit_write` | yes | **all 6** — and I checked it is a **real `perform app.audit_write(...)` statement**, unconditional, after the write, in each body — *not* a text match on a comment. (This mattered: the same false-positive class produced my own V-0.3 error.) |
...
### V-3.2 · But the five tables are not the exclusion plane. There is a sixth.

A27 is **binding** and v2 quotes it in its own §1.1:

> *"For **each** arm `is_case_excluded` resolves through, enumerate **EVERY** mutator of the rows it reads."*

`is_case_respondent` — from the live catalog — reads rows from **five** tables:
...
`case_conflict_declarations`, `professional_profiles`, `professional_participants` — and **omitted the
second table in its own join**, filing it instead on the **KEEP list** as *"org config, out of scope"*.

**Re-running C8's own three legs against the omitted table — all three FAIL:**
...
Raw ACL, decisive — note it differs from every other exclusion-plane table:
```
...
No trigger. **And no RPC writes `key`** — `set_participant_patient` / `add_case_participant` /
`set_case_participant_role` only read or insert role rows.

**C8's conclusion is exactly inverted here: the RPC is not the only door — there is no RPC door at all,
only unaudited direct DML.**

### V-3.3 · PROBE 4 — proven live, rolled back (I did not offer this unverified)

```sql
begin;
  -- BEFORE | respondent t | excluded t
  set local role authenticated;
  select set_config('request.jwt.claims', '{"sub":"…b1","role":"authenticated"}', true);  -- orgadmin.a@test.local
  -- DIRECT PostgREST DML. No RPC. No audit_write.
  update public.case_participant_roles set key='former_respondent' where id='fc…e1';
  -- UPDATE 1                                          <-- SUCCEEDS
  reset role;
...
organization that uses it. One `UPDATE` of one column dissolves the respondent deny **org-wide** —
...
`is_admin()` arm) — i.e. **this is simultaneously a 5th `platform_admin` arm on tenant data**, which A30
requires `backend` to enumerate **exhaustively**. The enumeration returned 4. **It is 5.**

### V-3.4 · Why this is CHANGES REQUESTED and not a note

Not because it is bigger than what v2 found — it is not. Because of **what M1 is for**:

- **A29 states M1's purpose:** *"Migration 1 buys exactly one thing: the exclusion keystones stop being
  vacuous."* Ship B7 + the 4 mutators + the 35 RPCs and **Row zero is still not durable** — the deny is
...
- It is the **exact error class v2 just corrected in C2**, one layer further out: v2 applied A27's lens to
  ...
  claim — which is now proven in **both** directions, including against me (V-0).
- **The fix belongs in M1 and fits its constraints**: freeze `key` once referenced (immutability trigger,
  ...
  the mutation must pass an audited RPC. **Net-tightening · `EXPLAIN`-neutral · no resolver · not an
  admin-arm removal**, so **D4·3 does not defer it**. It is M1's shape exactly.

---

## V-4 · Is 49 the population, or is it now a floor? — IT IS A FLOOR
...
four strings and is invisible to Q7·v2.** Sweeping for exactly that:

```
28 functions call an admin-arm-bearing wrapper (assert_meeting_staff_admin · can_read_case_or_admin ·
can_write_interview · can_write_attachment · can_read_attachment) while matching NONE of Q7·v2's four arms.
Of these, 10 are SECURITY DEFINER — and ALL 28 have excl = f.
```

**The 10 DEFINER ones are the population Q7·v2 misses:** `create_attachment` · **`dispose_attachment_phi`** ·
`soft_delete_attachment` · `open_attachment` · `interview_viewer_can_write` · `record_session_attendance` ·
`set_interview_confidentiality` · `set_interview_participant` · `set_interview_interviewer_participant` ·
...
**This is mostly, but not entirely, benign — and the exception is the point.** Fix the *helper* and every
DEFINER caller is fixed for free. v2 marks `can_write_interview` **FIX** — that propagates to the 6
interview RPCs correctly. **But v2 marks `can_write_attachment` "REMOVE-ARM (A14 to A21·1)" — arm removal
...
Strip `is_commission_admin_of_for` per A21·1 and **`is_staff_admin_of_for` survives unqualified** —
...
`pg_policies` and `prosecdef`. **Not M1-blocking** (M1 does not touch this helper) — **blocking for the
...
(enumerated separately, §3.4 — the correct treatment). **Views are genuinely empty** (0 in `public`) and
**capability checks inside a view are therefore structurally impossible** — that clearance stands.

---

## V-5 · Sequencing sanity check (A29) — the order is right; the contents are not yet complete
...
until `is_case_respondent` resolves). **A21's admin-arm removal correctly stays out** (D4·3).

**Is the 35-vs-49 split defensible? — YES.** The 14 non-fix rows are each defensible from the catalog:
7 INVOKER (RLS applies — the load-bearing negative, re-verified), 3 already gated, 4 with a read gate of
which 3 are separately filed (`list_cases_board` §1.3, `case_viewer_capabilities` §3.6·E, `record_recusal`
C11) and 1 (`get_case_detail`) genuinely fixed for free. **The split is correct.**

**But M1's contents are incomplete for M1's own purpose** (V-3) — and A30's platform_admin enumeration is
...
**rows read under `set local role authenticated`** — never on a predicate's return value.
...
> **THE PRECONDITION, WRITTEN NARROWLY — this is the part that goes vacuous if paraphrased:**
...
>   **`staff_admin` on the case's commission** (seed: `staff4.ccih@test.local` is `staff` — **the
...
>   `chefe.ccih@test.local` is already `staff_admin` — insert the recusal only).
> - **Assert the negative on state, not on the exception alone:** after the denied call,
>   `is_case_respondent` / `is_recused_from_case` **must still be `t`** — the row survives her.
...
### M1·3 — NEW: `case_participant_roles` — the 6th exclusion-plane table (V-3)
Freeze `key` once referenced (immutability trigger — the `guard_case_status`/`HC025` pattern) **and/or**
`REVOKE` the direct `arwd` grant from `authenticated` so mutation must pass an audited RPC. **Add an audit
trigger** — this table has none and no RPC door, so today the mutation is invisible to Rule 11.
...
- **Keystone (no over-reach):** an `org_admin` can still **create** and **rename the `display_name` of** a
  role — the staffing/config surface A18 protects must survive. *(Keystone 23.)*
...
- **§3.6·B — commission-scoped, a gate is unwritable; use a per-row filter**: `list_cases_board` (delete
  the fast-path) · `count_open_cases_for_board` · `case_tag_report` **(pending the PO ruling — a
...
### M1·5 — A30: `platform_admin` arms on tenant data — 5, not 4
`set_case_offered_outcomes` · `create_case` · **`dispose_referral_phi`** · **`can_dispose_referral_phi`**
· **`case_participant_roles_admin_write`** (V-3.2 — the `is_admin()` arm on the exclusion plane).
- **C5 CONFIRMED from the catalog** — `dispose_referral_phi` gates on
  ...
  `referral_shared_item` / `referral_messages`. **`platform_admin` destroys referral PHI in any tenant** —
  a direct contradiction of CLAUDE.md §1's *"walled off from all tenant data."* `backend` found this; I
  missed it. **Blast radius: irreversible, cross-tenant, PHI destruction** (it *is* audited, and the
  `HC056` re-dispose guard holds).

### V-6.1 · `set_participant_patient` — CONFIRMED. Blast radius, as requested.
```
set_participant_patient(p_case_id, p_participant_id, p_name, p_mrn, p_date_of_birth, …)   definer = t
  -- COORDINATOR-ONLY write gate (not can_read_case — that is the broad READ scope).
  if not app.is_staff_admin_of(v_case.commission_id) then raise … '42501';
  -> insert into public.patient_identifiers (…) on conflict (participant_id) do update set name=…, mrn=…
```
**DEFINER · `is_staff_admin_of` only · no `is_commission_admin_of` arm · no exclusion term · and — per
V-0.3 — no read gate** (my v1 wrongly credited it with one).
...
`attending`) on any case in her commission — via `on conflict do update`, i.e. **destructive of existing
PHI**, not merely additive. It auto-creates the participant chain and flips `cases.has_patient`. It is a
**Rule 12 write/tamper door on the evidentiary record of the case in which she is the accused** — the
...
prior sweeps missed it. Correctly filed in §3.6·A2.**

---

## V-7 · Required edits to A0 v2 (all mechanical — no redesign)

| # | Required | Why blocking |
|---|---|---|
| **1** | **Add `case_participant_roles` to the exclusion plane** (§1.1 table + §4.1). **Correct C8's disproof: it holds for 5 tables, and Rule 11 does NOT hold on the 6th.** State the direct-DML door + the missing audit trigger. | **M1's purpose fails without it** (V-3). A27 is binding and A0 v2 does not apply it to its own join. |
...
| **3** | **Reclassify `can_write_attachment` REMOVE-ARM to FIX** (§3.5). | The C2 defect at the helper layer, on **`dispose_attachment_phi`** (PHI destruction). *(Not M1; blocks the Stage-A/G sweep.)* |
| **4** | **State plainly that 49 is a FLOOR**, and add **indirection through a gate wrapper** as a third binding methodology axis (28 fns, 10 DEFINER — list them). | v2 presents 49 as the population. It is the third floor this program has mistaken for one. |
| **5** | **Correct the `lift_recusal` severity narrative** (V-1.1) — it flips `can_read_case_patient` too; the respondent arm is **not** "strictly worse". Both arms are PHI doors; the PO ruling is a Rule 12 decision. | The PO is ruling on this; the framing understates what is being decided. |
| **6** | **Re-word `is_admin()`'s JWT claim** (§1.6) — a **server-minted cache** (`custom_access_token_hook`, ADR 0002), **not** a forgeable escalation path. Residual = **staleness** only. | Lead correction; must not ship as a privilege-escalation finding. |
| **7** | Record the **narrow preconditions** (V-6·M1·2) in §6 so no keystone can go green asserting nothing. | C1a's own lesson, applied to every fixture in the migration. |

**What does NOT need to change:** the methodology (now the strongest in the program), the provenance
discipline, **all five accepted corrections C2/C3/C6/C7/C9–C12**, the KEEP list, the INVOKER negative, the
`prosecdef` axis, the gates-vs-filters split, the 35/49 partition, the sequencing order, and C8's three
legs **as scoped**.

## V-8 · PO / lead rulings still open
Unchanged from v2 §5.1 (1–6), **plus**: **(7)** who may mutate `case_participant_roles.key` once a case
references it — and is `key` immutable-once-referenced? **(8)** `platform_admin` and **referral PHI
destruction** (A30 / `dispose_referral_phi`) — the sharpest form of the CLAUDE.md §1 contradiction.
...
|---|---|
| **Verdict** | **CHANGES REQUESTED** |
| **Sufficient foundation for M1's SQL?** | **No** — by one item (V-3) + the A30 count. Everything else is ready. |
| v2 corrections **verified** | **C1a · C2 · C3 · C4 · C5 · C6 · C7 · C9–C12 all CONFIRMED · C8 legs CONFIRMED / scope FAILS** |
| v2 probes re-run by me | **3/3 reproduce exactly, preconditions included** |
| **My v1 errors, corrected visibly** | **3** — the vacuous fixture (C1a) · the "30" floor (C4) · the `readgate` comment false-positive (V-0.3) |
| New findings | **1 x P0 proven live** (`case_participant_roles`, V-3) · **1 x P1** (`can_write_attachment` misclass, V-4) · **1 x severity** (`lift_recusal` is a PHI door, V-1.1) |
| Over-reach in v2 | **0** — §3.7's claim verified in both directions |
| Blind spot remaining | **indirection through a gate wrapper** (28 fns / 10 DEFINER) |
| Re-review | **Fast** — 7 mechanical edits; no redesign |
...
**And the gate still fails — for the same reason, one turn further out.** v1 applied its A22 lens to
policies but not to its RPC list. I condemned the blind-spot query and then re-ran it. v2 published A27's
binding rule — *enumerate every mutator of every row the deny reads* — and did not apply it to the second
...
# PART III — v3 review · the final A0 round

**Date:** 2026-07-15 · **Reviewer:** `qa` · **Subject:** `docs/progress/authz-capability-inventory.md` **v3**
**Method:** live catalog only — `pg_proc` (incl. `prosecdef`, `prosqlbody`, `prolang`), `pg_policies`,
...
lead — no reset/restart/push. **Five probes re-run, each `ROLLBACK`ed; persistence re-verified.**
...
**Is A0 v3 sufficient to author M1's SQL against? — YES, for M1·1–M1·4, which is the durability core and
the whole of the PO-approved sequencing that matters. M1·5 (A30) needs one more mechanical sweep, which I
have run below and hand over rather than bounce.**

**A0 is CLOSED.** Every v3 claim I tested reproduced. Its central move — **stop counting callers; enumerate
the gate helpers** — is not just pragmatic, it is **provably closed**, and I give the closure argument in
§W-2 because v3 states the conclusion without stating the proof. **It corrected me twice more and both
corrections stand.** The two gaps I found are **additive scope items, not method failures** — and per this
mandate the authoritative M1 list is mine to state, so I have folded them in (§W-6) rather than spend a
sixth round on them.

---

## W-0 · Corrections to my own v2 — the fourth round of the same lesson, and it is mine again

### W-0.1 · ✅ **D2 CONFIRMED. My PROBE 4 over-stated the blocker.** (v2 §V-3.3 is WRONG)
...
measured `respondent` and `excluded`, then *asserted* the PHI consequence — one section after I had
...
### W-0.2 · ✅ **D2a CONFIRMED — exactly, including the audit claim, which is the sharpest in the document**
...
```
 ...
 POST-ROLLBACK | key='respondent_doctor' | audit 160 | staff_admin_rows 4        ← nothing persisted
```

**161 → 161 holds.** The `160 → 161` step is the fixture's own `memberships` insert emitting its audit row;
the **org_admin's direct DML emits none**. So the composed path is **neither preventable nor detectable** —
...
**And v3's blast-radius split is the correction that makes the keystone writable** — I collapsed it:
- **deny dissolution: ORG-WIDE** (one shared role row → every case in the org at once) ⟵ why it is P0
- **PHI consequence: PER-RESPONDENT** (gated on that respondent's positive arm) ⟵ why the keystone must be narrow

Conflate them and the keystone asserts an org-wide PHI read that does not exist, fails to reproduce, and
gets "fixed" by weakening the assertion. **Both halves are now in the keystone (§W-6·M1·3).**

### W-0.3 · ✅ **D4 CONFIRMED — my "10 DEFINER" was itself a floor**

I matched **five hand-picked wrapper names** — which is hand-picking the arms one level up, the same error
one layer out. v3's closure swept in every PHI door I missed. **Conceded.**

### W-0.4 · ✅ **D1a CONFIRMED — v3's self-criticism is right, and sharper than my finding**
...
**Exactly five tables — and `case_conflict_declarations` is `f` on all three.** v2 swept five and got the
**right cardinality with the wrong membership**: it omitted `case_participant_roles` *and* included a table
the deny never reads. **The two errors cancelled — which is precisely why the sweep looked complete.** I
...
## W-1 · D3 — the lead's instruction was wrong, and v3 disproved it with evidence. ✅ **CONFIRMED**
...
**Exact.** Every function uses an old-style string-literal body (`AS $$ … $$`); PostgreSQL records
...
> [[verify-dont-comply]]'s third instance in this program — and the first where the *lead* was the source.
...
### W-2.1 · The numbers — ✅ **exact**

Q19 re-run verbatim: **16 helpers · 5 carry the deny · 11 do not · 48 callers** (18+9+8+4+3+2+1+1+1+1+0).
Every cell reproduces.

### W-2.2 · **Is 16 the population, or a sixth floor? — It is a SCOPED set, not a derived one — and the scope holds.**

**v3's Q19 is a hand-written `IN` list of 16 names. There is no derivation rule.** That is a boundary drawn
by judgement — the exact thing the last five rounds got wrong — and v3 does not say so. **So I derived it
by a rule instead:** every `app` function returning `boolean`/`void`/`uuid` that carries a role arm **and**
has ≥1 function or policy caller.

**Result: 23 arm-bearing helpers exist outside v3's 16.** But the scope holds, and here is why:

| Outside the 16 | Verdict |
|---|---|
| `can_read_event` · `can_read_event_patient` · `can_write_rca` · `can_write_capa` · `can_read_capa` · `can_curate_pqs_vocab` · `is_pqs_operator_of` · `can_sign_meeting` · `can_sign_section` · `can_read_signoff` · `can_read_document_*` · `can_read_xref_row` · `is_pqs_writer_of` · `event_current_custodian` · `can_write_action_item_stake` · `can_manage_referral_*` | **Other domains** (NSP/PQS · RCA/CAPA · documents · signoffs). Not the case-content plane. **Correctly out of scope.** |
| `is_commission_admin_of` · `audit_write` · `_audit_access_authorized` | **Not gate helpers** — the arm itself / the audit writer. **Correctly out.** |
| **`can_read_professional_profile`** | Touches `professional_profiles` — a **deny-read table**. But its case arm gates on **`app.can_read_case`, which carries the deny** ✅. Its `is_admin()` arm is an **A30 item** (§W-4). **Not a deny hole.** |
| **`can_manage_professional`** | `is_admin() OR is_org_admin_of OR staff_admin-in-org`, **no deny, no case scope**. But `user_id` is **write-once** and `update_professional_profile` never writes it, so **it cannot flip the deny** ✅. **Not on the exclusion plane.** Its `is_admin()` arm is an **A30 item.** |

**Conclusion: 16 is sound for the case plane. It is not a floor — but v3 must WRITE DOWN the scoping rule**
...
defect — folded into §W-6.)*

### W-2.3 · ⭐ **Does fixing a helper fix its callers? — YES, and the closure argument is stronger than v3 states**
...
> **The partition is exhaustive by construction.** Every function either **carries an arm string** — in
> which case it is **text-findable**, because the string is literally in `prosrc` — or **it does not**, in
> which case its only route to an arm is **through a helper**, and fixing the helper fixes it. There is no
> third case. **The transitive caller set does not need to be closable, because it is not the population —
> the union `{gate helpers} ∪ {direct-arm-checkers}` is, and both halves are mechanically derivable.**

That is why the 37→30→35/49→10→57 regress ends here, and it ends for a **structural** reason, not a
pragmatic one. **D5 is correct and I endorse it as the frame.**

### W-2.4 · ⚠ **But the "for free" claim has NINE exceptions, and v3 states none of them**
...
```
 ...
 can_write_attachment  | public.reclassify_attachment    | t     ← NOT covered anywhere
 can_write_interview   | app.can_write_attachment        | t
```

**Eight are already covered** — `get_referral_detail` / `post_referral_message` are in the 35; the rest are
helpers in the set themselves (nested, fixed by their own fix). **One is not: `reclassify_attachment`.**

### W-2.5 · ⛔ **`reclassify_attachment` — a real defect v3's frame does not reach** (the direct-check residue)
...
=== public.reclassify_attachment   definer = true
  ...
  if p_new_tier = 'standard' and v_row.sensitivity_tier = 'phi' then
    if not (app.is_staff_admin_of(v_commission) or app.is_commission_admin_of(v_commission)) then
      raise exception 'apenas a coordenação pode reduzir a classificação de um anexo' using errcode='42501';
  else
    if not app.can_write_attachment(...) then …          ← this arm IS fixed by the helper fix
  …
  update public.attachments
     set sensitivity_tier = p_new_tier,                  -- 'phi' → 'standard'
         storage_bucket   = v_new_bucket,                -- 'attachments-phi' → 'attachments'
         confidentiality_label = v_new_label;            -- 'phi_standard' → 'non_phi_internal'
```

**The declassify arm checks the role directly — no deny, no exclusion.** A **recused coordinator** (or a
**respondent-coordinator**) **declassifies a PHI attachment** on the case she is excluded from: it moves
buckets `attachments-phi → attachments` and its label is rewritten to `non_phi_internal`, which **drops it
below `attachment_confidentiality_ok`'s ceiling** and into the broader-read bucket. **Rule 12 downgrade.**

**Why both sweeps missed it — and this is the actionable lesson:** Q7·v2's *content* predicate has no
...
v3's 35+4. Most are genuinely other domains (indicators · controlled documents · dashboards · committee
action items · NSP) or KEEP-list config — **`reclassify_attachment` is the one that plainly belongs to this
...
plane). **Folded into M1·4 (§W-6) rather than bounced.**

### W-2.6 · The 57 false alarms — spot-checked **both ways** ✅

v3 caught its own false alarms and I checked the self-correction in the direction that matters — **did it
wrongly clear a real defect?**

- `get_case_patient` → `get_participant_patient` → **does** gate on `can_read_case_patient` ✅ correctly cleared
- `get_referral_patient` → gates on `can_read_referral_phi` ✅ correctly cleared
- **`save_narrative_body` → gates on `can_write_case_narrative`** — v3 cleared the *caller* ✅ **and then
...
helper-delegations — which is exactly D5's point.

---

## W-3 · D6 — ✅ **CONFIRMED. Blast radius as requested.**

```
=== app.can_write_case_narrative   definer = true
  return
    app.is_staff_admin_of_for(v_commission, p_uid)          ← NO DENY. Short-circuits everything below.
    or (v_assigned_to is not null and v_assigned_to = p_uid)
    or (v_assigned_to is null and app.can_write_case_content(v_case_id, p_uid));   ← this arm HAS the deny
```

**The helper is internally inconsistent, which is the sharpest form of the defect:** its **third** arm
delegates to `can_write_case_content`, which **carries the deny** — so the function already "knows" the
excluded must be denied — while its **first** arm grants a recused coordinator write **before that arm is
ever evaluated**.

**`case_narratives.body_md`, from its own column comment:**
> *"PHI-BEARING free text (WS B; Rule 11/12). Case narrative prose (sanitized Markdown, Rule 7)…"*

**Blast radius:** a **recused coordinator** or a **respondent-coordinator** **writes and overwrites
PHI-bearing narrative prose** on the case in which she is recused or accused — the deliberative record of
an ethics case, authored by its subject. Sole caller `save_narrative_body` (DEFINER) **does not check a
role directly** (verified) ⇒ **fixing the helper fixes it for free**, no RPC edit needed. Invisible to
Q7·v2 (the RPC carries no arm string) **and** to my wrapper sweep (not among my five hand-picked names).
**`backend` found this; I did not. It is the best find in v3.**

---

## W-4 · A30 — ⚠ **the enumeration is NOT exhaustive, and A30 demands that it be**
...
22 policies). Most are legitimate platform surface — `organizations` · `hospitals` · `profiles` ·
`commissions` · `grant_role`/`revoke_role` · `verify_audit_chain` · config tables — **but a material
tenant-data subset is unlisted:**

| Unlisted site | Why it is tenant data |
|---|---|
| **`app.can_read_professional_profile`** | `if is_admin() then return true` ⇒ reads **Class-2 professional identity** (Rule 12, ADR 0064/0065) in **every** tenant |
| **`app.can_manage_professional`** | `is_admin()` arm ⇒ manages professional identity org-wide |
| **`app.attachment_confidentiality_ok`** · **`app.confidentiality_clearance_ok`** | the **confidentiality ceiling itself** ⇒ platform_admin bypasses the ceiling on case attachments + interviews |
| `public.participants.participants_select` (policy) | `participants` includes **patient** participants |
| `public.dashboard_distributions` · `dashboard_export_rows` | tenant form/response data |
| `public.hospital_document_register` · `hospital_indicator_rollup` | tenant governance data |

**This does not block M1·1–M1·4** — A30 is the **last** step in A29's sequence and is a **PO fold-in**, not
a durability item. But **M1·5 cannot be authored against "five"**: it would ship a keystone asserting
...
## W-5 · M1·3's fix shape — ✅ **UPDATE-freeze CONFIRMED; the premise is TRUE**
...
```
 ...
 public.set_participant_patient   | t       | f       | f        ← the ONLY writer, and it INSERTs
```

**Verified in both directions:**
- **A blanket write-freeze WOULD break a live path** — `set_participant_patient` INSERTs the
...
- **"No RPC ever UPDATEs `key`" is TRUE** — **0 functions UPDATE `case_participant_roles`, and 0 DELETE
  it.** So an **UPDATE-freeze costs nothing** and is free of live-path risk.

**v3's fix-shape correction is right.** *(Note for M1: the freeze should target `key` specifically —
...
v3's §6 with my two corrections folded in. **Order is binding** (A29, PO-approved).

### M1·1 — B7: respondent linkage · **LANDS FIRST**
Resolved state **+ the `user_id` write path** (`update_professional_profile` has no `p_user_id` ⇒ `unknown
→ linked` has no door) **+ the `ON DELETE SET NULL` PO ruling**.
...
- **Keystone:** an `unknown` profile is linkable through the public API; `is_case_respondent` → `t`.

### M1·2 — the five exclusion-plane **RPC** mutators
`lift_recusal` · `remove_case_participant` · `set_case_participant_role` · `set_primary_subject` ·
`record_recusal` — each gains `AND NOT app.is_case_excluded(<case>, auth.uid())` (+ the A18 Org-User arm,
pending the PO ruling).

> **PRECONDITION — WRITTEN NARROWLY. This is the part that goes vacuous if paraphrased.**
> - **respondent arm:** the principal must be **`is_case_respondent` = t** **AND** hold **`staff_admin` on
>   the case's commission**. Seed `staff4.ccih@test.local` is **plain `staff`** — **the fixture MUST insert
>   the membership row.** Without it the RPC raises `HC0E4` **for the wrong reason** and the test is green
>   and worthless. *(C1a — proven twice.)*
> - **recusal arm:** the principal must be **recused AND `staff_admin`** (`chefe.ccih@test.local` is
>   already `staff_admin` — insert the recusal only).
> - **Over-grant twin (each):** the denied party calls the door **on her own row** → **RAISES**; and
>   `is_case_respondent`/`is_recused_from_case` **is still `t`** — assert the **row survives her**, on rows
>   read under `set local role authenticated`, never on a predicate's return value.
> - **Positive twin (each):** a **non-excluded** coordinator calls the same door → **SUCCEEDS**. Without
...
>   `is_primary_subject` ⇒ keystone it as a **gate fix**, not a durability fix, or it cannot falsify.
> - **Both arms are Rule 12 doors** (D9) — `lift_recusal` flips `can_read_case_patient` too. Assert **PHI**,
>   not just reach.

### M1·3 — `case_participant_roles`: the 6th exclusion-plane table · **UPDATE-freeze**
**`UPDATE`-freeze on `key`** (immutability trigger, the `guard_case_status`/`HC025` pattern) — **NOT a
write-freeze** (breaks `set_participant_patient`'s INSERT, §W-5) — **and/or** `REVOKE` the direct `arwd`
...
> **Keystones — the two halves must be asserted separately or the test is vacuous (§W-0.2):**
> - **(a) org-wide dissolution:** `orgadmin.a@test.local`, under `set local role authenticated`, `UPDATE`s
>   `case_participant_roles.key` off `respondent_doctor` → **RAISES**; `app.is_case_excluded(<case>,<resp>)`
>   **is still `t`**. *(Today: `UPDATE 1`, then `f`.)*
> - **(b) per-respondent PHI — needs the FULL composed precondition:** respondent **AND `staff_admin`**
>   (M1·2's precondition) **AND** the actor is the **org_admin**, **AND the respondent never acts**. Assert
>   `can_read_case_patient` **stays `f`**. *(Today: `t`.)* **Do not assert PHI on (a) alone — it is `f→f`
>   and the test will "pass" while asserting nothing. That is exactly my v2 error.**
> - **(c) audit silence:** the permitted path emits an `audit_log` row. *(Today: 161→161 — none.)*
> - **(d) over-reach twin:** an `org_admin` can still **create** a role and **rename `display_name`** —
>   A18's staffing/config surface must survive.

### M1·4 — the sweep: **35 RPCs + `reclassify_attachment`**, split by remediation shape
- **§3.6·A — case-scoped → add the gate** `AND NOT app.is_case_excluded(<case>, auth.uid())`. Includes
  **`set_participant_patient`** (PHI write/overwrite) and the A3 grant/staffing doors **which KEEP their
  arm** (C6 — *keeping the arm* and *adding the deny* are orthogonal).
- ⬅ **NEW: `reclassify_attachment`** (§W-2.5) — its **declassify arm** checks the role **directly**, so the
  helper fix does **not** reach it, and Q7·v2's content filter never saw it.
  **Keystone:** a recused coordinator calls `reclassify_attachment(phi→standard)` → **RAISES**; the
  attachment stays in `attachments-phi` with its label intact. **Positive twin:** a non-excluded
  coordinator still declassifies.
- **§3.6·B — commission-scoped → a gate is UNWRITABLE; use a per-row filter**: `list_cases_board` (delete
  the fast-path) · `count_open_cases_for_board` · `case_tag_report` *(pending the PO ruling — a
  per-caller-varying count is a product decision)*.
- ⛔ **DO NOT TOUCH** the 7 INVOKER RPCs (RLS applies — the load-bearing negative, re-verified twice) or
  the 3 already-gated.
- **Keystone per shape**, not per RPC: one gate twin · one per-row-filter twin · one **over-reach twin for
  bucket D** (*an INVOKER RPC still works for a legitimate coordinator*).

### M1·4b — the **11 gate helpers** (D5) — *this is where the leverage is*
Fix the 11 → **48 callers fixed for free** (closure argument: §W-2.3). Priority order:
1. **`can_write_case_narrative`** ⭐ (D6) — **FIX**: a recused coordinator writes **PHI-bearing** prose. 1 caller.
2. **`can_write_attachment`** (D7) — **FIX**: callers include **`dispose_attachment_phi`** (PHI destruction). 4 callers.
3. **`can_write_interview`** — **FIX**: no deny, no `is_active` on the interviewer arm. 8 callers.
4. **`can_read_action_item`** — **FIX** (A22 + A24·5). 3 callers.
5. `can_read_attachment` (REMOVE-ARM ×2) · `assert_meeting_staff_admin` (REMOVE-ARM, A10; 18 INVOKER meeting RPCs) · `can_read_referral_phi`/`can_read_referral` (split, D7/F-min) · `attachment_confidentiality_ok` + `confidentiality_clearance_ok` (**B3 must repoint BOTH** — §1.8).
...
- ⚠ **STATE IT IN §6 SO M1 IS NOT MISREAD:** *"fix 11 helpers and we're done"* **ships with the P0s
  intact.** The self-serving mutators check `is_staff_admin_of` **directly** — the helper fix **never
  touches them**. **M1 = helpers ∪ direct-check doors.** Both are required; neither is sufficient.

### M1·5 — A30: platform_admin arms · ⚠ **BLOCKED pending an exhaustive enumeration** (§W-4)
**Do not author against "five."** Complete the enumeration first (my sweep is the input: 42 `is_admin()`
sites — 20 fns + 22 policies), classify each **platform-surface vs tenant-data**, and get the **PO ruling**.
Known-unlisted tenant-data arms: **`can_read_professional_profile`** · **`can_manage_professional`** ·
**`attachment_confidentiality_ok`** · **`confidentiality_clearance_ok`** · `participants_select` ·
`dashboard_distributions` · `dashboard_export_rows` · `hospital_document_register` ·
`hospital_indicator_rollup`. **Last in the sequence ⇒ does not block M1·1–M1·4 from starting now.**

---

## W-7 · Non-blocking corrections for the record (fold into §6; do not spend a round on them)

| # | Item |
|---|---|
| 1 | **Write down D5's scoping rule** — *"gate helpers of case/meeting/attachment/interview/referral content."* A hand-written `IN` list with no stated rule is indistinguishable from the five boundaries that were wrong (§W-2.2). |
| 2 | **State the closure argument** (§W-2.3). It is the justification for the whole frame and v3 asserts the conclusion without it. |
| 3 | **State the 9 sole-gate exceptions** (§W-2.4) — *"fixed for free"* is true **except** where a caller also checks a role directly. |
| 4 | **Add `reclassify_attachment` to M1·4** and note that the direct-check set must be derived **without** the hand-written content filter (§W-2.5). |
| 5 | **A30: five → not-yet-exhaustive** (§W-4). |
...
| | |
|---|---|
| **Verdict** | ✅ **APPROVED — A0 is CLOSED** |
| **Sufficient to author M1's SQL against?** | **YES for M1·1–M1·4** (the durability core). **M1·5 (A30) blocked** on an exhaustive enumeration — last in sequence, does not gate the rest. |
| v3 claims verified | **D1 ✅ · D1a ✅ · D2 ✅ · D2a ✅ (incl. 161→161) · D3 ✅ (0/660, 5 edges) · D4 ✅ · D5 ✅ (16/5/11/48 exact) · D6 ✅ · D7–D10 ✅ · M1·3 UPDATE-freeze ✅** |
| Probes re-run by me | **5/5 reproduce exactly**, preconditions included |
| **My v2 errors, corrected visibly** | **2** — PROBE 4's unmeasured PHI claim (D2) · the "10 DEFINER" floor (D4) |
| New findings | **1** — `reclassify_attachment` (direct-check residue, Rule 12 downgrade) · **1 scope gap** — A30's enumeration |
| Over-reach found | **0** |
| Method | **Closable at last.** `{gate helpers} ∪ {direct-arm-checkers}` is exhaustive **by construction** (§W-2.3). |

**Why this closes.** Five rounds each found a real P0, and the temptation is to assume a sixth exists. It
does not — not because we stopped looking, but because **the frame changed from one that could not close to
one that can**. Every prior round enumerated *call sites* and produced a floor (37 → 30 → 35/49 → 10 → 57)
because *"is this caller gated?"* is a judgement no filter makes. **D5's partition is mechanical and
exhaustive**, and the two gaps I found are **inside** it — one direct-check door and one incomplete
enumeration — both derivable by rule, both folded into §W-6 rather than bounced. **That is the difference
between a floor and a boundary.**

**On the record, plainly:** the last two rounds, the person who was wrong was **me** — an unverified PHI
...
proof — right cardinality, wrong membership).** Both should outlive this program. And **D3 should be
