...
say "         ⛔ ON A FULL RUN THAT DIFF IS AMBIGUOUS AND MUST BE READ WITH THE MERGE"
say "         BANNER (QA F-MAJOR-5, 2026-09-05). A full run MERGES into the baseline, so an"
say "         EMPTY diff means either 'no verdict moved' — the ordinary clean outcome — or"
say "         'the merge ABORTED and wrote nothing', which leaves a STALE file behind. The"
say "         two are byte-identical to git. The sweep now exits 2 with"
say "         '=== RESULT: ERROR — the findings MERGE ABORTED' in that case, so read the"
say "         EXIT CODE, and take an empty diff as evidence only when the run exited 0."
