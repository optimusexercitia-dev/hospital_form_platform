 ...
 *
 * ⛔ PARKED ENTIRELY — FUP-DM1-E2E (docs/progress/follow-ups.md). DM1
 * (ADR 0114 D5, migration `20260923000100`) dropped the centralized
 * attachments substrate this whole file exercises: the `attachments` /
 * `attachment_references` / `attachment_subjects` tables, the five public
 * RPCs (`create_attachment`, `open_attachment`, `dispose_attachment_phi`,
 * `reclassify_attachment`, `soft_delete_attachment`), the storage policies,
 * and the seed fixtures/flag this spec depends on. Every test below fails
 * or asserts against removed substrate — not a flaky quarantine, a deliberate
 * program park.
 *
 * Discharge condition: DM2 (Wave A) rewrites these contracts (PHI-tier
 * listing with no inline URL, audited open door, audit-row exactness, tier
 * bucketing) against the replacement document model
 * (`open_document_version` et al.) and un-parks/deletes this file in the
 * same change, per ADR 0114 D5 ("rewritten, never merely deleted").
 *
...
test.describe.skip('Phase F2 — Centralized Attachments (PARKED: FUP-DM1-E2E — substrate dropped by ADR 0114 D5 / migration 20260923000100; rewritten against the document model in DM2/Wave A, never merely deleted)', () => {
