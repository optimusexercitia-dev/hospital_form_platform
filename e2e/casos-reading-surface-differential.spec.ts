...
function caseHeader(page: Page): Locator {
 ...
 * 0 on `/casos` mean "the control is gone" rather than "the panel is gone" — the
 * failure mode that lets a bare absence assertion pass for the wrong reason.
 */
interface Member {
...
/** G5 — `effectiveCanManagePhaseResults`. PRE-EXISTING for EVERY class — see the header. */
const G5_MEMBERS: Member[] = [
  {
    gate: 'G5',
 ...
 * HERE, beside the rest, because an enumeration split across three files is a
 * hand-list again — and a hand-list is what this whole file exists to replace.
 */
const FULL_CLASS: Member[] = [...CASE_WIDE_CLASS, PLESS_MEMBER, CF_MEMBER]
 ...
 * error this whole file exists to stop repeating. The structure check stays
 * fail-fast: a missing panel is a broken fixture, not a finding.
 */
async function assertAbsentOnCasos(page: Page, members: Member[]) {
...
async function assertPresentOnManage(page: Page, members: Member[]) {
...
/** A both-hosts control: a class the viewer never held, absent here for its own reason. */
async function assertAbsentHere(page: Page, members: Member[], why: string) {
