...
test('R2-1: send wizard sets priority / requested-action / response_due_at; review + detail reflect them', async ({
  ...
  await wizard.getByLabel('Tipo de encaminhamento').selectOption({ index: 1 })
  await wizard.getByLabel('Comissão de destino').selectOption(COMM_B)
  await wizard.getByLabel('Assunto').fill(subject)
  await wizard.getByLabel('Prioridade').selectOption('urgent')
  await wizard.getByLabel(/Ação solicitada/).selectOption(r2RequestedAction.id)
  const dueLocal = futureLocalInput(5)
  await wizard.getByLabel(/Prazo de resposta/).fill(dueLocal)
...
test('R3-8: "Encaminhar adiante" creates a child referral carrying parent lineage', async ({
  ...
  await wizard.getByLabel('Tipo de encaminhamento').selectOption({ index: 1 })
  await wizard.getByLabel('Comissão de destino').selectOption(COMM_A)
  await wizard.getByLabel('Assunto').fill(subject)
  const responseToggle = wizard.getByLabel(/Aguardar resposta/)
