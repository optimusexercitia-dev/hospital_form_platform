...
export interface DoneCallbackRefs {
  ...
  /**
   * Service ADR 0003 shadow-comparison flag. Defaults to `true` (the platform's own
   * `deleteAudio` fires right on the callback) so every EXISTING call site keeps its
   * original coverage of that path unchanged. Pass `false` to exercise the pilot's
   * documented shadow-run mode instead — the callback must NOT delete the object, and
   * (BLOCKER B1, fixed in `0939437`) reclamation must happen at apply time instead.
   */
  audioRelease?: boolean;
}
...
export function buildDoneCallback(refs: DoneCallbackRefs): CallbackPayload {
  ...
  return {
    ...
    audio_release: refs.audioRelease ?? true,
    result,
    metrics: null,
    error: null,
    error_code: null,
