-- ADR 0147 (amends ADR 0146) — the audit organization is DERIVED from the hospital, and
...
--
-- Proof: supabase/tests/370_audit_write_derives_org_from_hospital.sql (§1 write side,
...
begin
  ...
  -- ⭐ ADR 0147 — DERIVE the organization from the HOSPITAL. Until now the hospital branch
  ...
  -- — invisible to the org admin (ADR 0146's widened leg 4 keys on that column) and
  ...
  -- Do not reorder the branches below without re-reading test 370 §3.
...
-- exercises it (test 369 §6.7 exists for exactly that reason).
...
using (
  ...
  -- ⭐ ADR 0147: `hospital_id IS NULL` added. The platform chain is ALL THREE scope keys
  -- NULL. ADR 0146 D4 froze this leg as a deliberate non-decision on the grounds that its
