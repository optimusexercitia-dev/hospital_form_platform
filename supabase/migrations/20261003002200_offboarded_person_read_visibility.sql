...
-- to a hospital they administer. ADR 0145 (amends ADR 0097 AFF, ADR 0133 AFF2 D13).
--
...
-- the directory listed people you could not open. Measured on the 368 fixture before this
...
--     or affiliations.
-- Proof: `supabase/tests/368_offboarded_person_visibility.sql` §4 (RLS layer) and
...
-- must move together; test 368 §5 asserts each BY NAME for exactly this reason.
-- ---------------------------------------------------------------------------
...
comment on policy profiles_admin_select on public.profiles is
  'AFF3/ADR 0145: the hospital-affiliation leg is EVER-HELD, not currently-held — a hospital_admin keeps read visibility of people who once worked at a hospital they administer, so offboarding via end_affiliation does not 404 its own actor. Read only; write authority is unchanged and still derives from the ACTIVE footprint (ADR 0133).';

comment on policy profiles_select_self_or_admin on public.profiles is
  'AFF3/ADR 0145: same ever-held affiliation leg as profiles_admin_select. Both profiles SELECT policies are permissive and OR''d, so they must be widened together or behavioural tests pass while one stays narrow.';

comment on policy professional_credentials_select on public.professional_credentials is
  'AFF3/ADR 0145: the affiliation leg is EVER-HELD, mirroring the two profiles SELECT policies. The mirror is the invariant (ADR 0133 D13 Amdt 2) — narrowing this policy alone reinstates the blank "Registro" em-dash that reads as "no permission".';
