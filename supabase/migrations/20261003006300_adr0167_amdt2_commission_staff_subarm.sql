-- ADR 0167 Amendment 2 — the commission `staff` sub-arm closes the same way.
--
-- PO ruling, 2026-08-28: delete `app.is_admin_for(p_actor)` from
-- `app.grant_role_impl`'s commission **`staff`** sub-arm, so grant matches the
-- revoke side it has always disagreed with. A `platform_admin` can now neither
-- SEAT nor REMOVE a commission `staff` — matching what ADR 0167 clause 1 already
-- did one role over for `staff_admin`.
--
-- ⛔ THE SITE WAS RE-DERIVED FROM `pg_proc` ON A FRESH RESET, NOT FROM MIGRATION
--    TEXT. At head 20261003006200 `app.grant_role_impl` carries THREE live
--    `is_admin_for(` call sites (comment-stripped `prosrc`; the raw body also
--    NAMES the predicate in four comments, which is why a line-filtered grep
--    over-counts). Exactly ONE moves here. The other two are PRESERVED and each
--    is out of scope for a named reason:
--      · organization/`org_admin` — the bootstrap chain ADR 0167 checked and kept.
--      · hospital/`hospital_admin` — AFF T2.5 / ADR 0097 D17 / audit BLOCKER-1.
--    pgTAP 397 § 0.3 pins the surviving TWO by name AND asserts the count, so a
--    later change that removes both cannot pass by satisfying only the
--    behavioural cells.
--
-- ⚠ THE ADR'S OWN PREDICTION WAS MEASURED FALSE, IN THE REASSURING DIRECTION.
--   Amendment 2 says "the `staff` population is larger than `staff_admin`, so
--   expect MORE fixture reliance, not less". Measured: the opposite — the
--   platform admin seats a commission `staff` in exactly two pgTAP files
--   (293, 397) and in ZERO Playwright specs and ZERO rows of `seed.sql`.
--
-- ⛔⛔ THE REAL WORK OF THIS MIGRATION IS THE COMPENSATION, NOT THE DELETION.
--   Refusing the platform admin ONE STATEMENT EARLIER makes three things stop
--   measuring WITHOUT going red — the shape this repo calls "vacuous rather than
--   red":
--     1. 293 § 3.1 (self-grant on the service path) compared SQLSTATE only. The
--        platform admin used to pass this sub-arm and be stopped by the inlined
--        `p_user = p_actor` guard; now it is stopped here, with the same 42501.
--        Re-cut to a MESSAGE assertion and re-homed to a commission
--        `staff_admin`, an actor that still reaches the self-grant guard.
--     2. 397 § 5.2 (the demotion grid) likewise. Re-cut to full-verdict
--        (SQLSTATE + message) so a refusal that MOVES becomes visible.
--     3. Site (b) — clause 1's own keystone — lost every behavioural assertion in
--        the repository: after this narrowing no actor in 397 reached its raise
--        at all. 397 gains two actors for it (a plain commission `staff_admin`,
--        and one who is ALSO `is_admin_for` — the only class for whom site (b)
--        decides anything), and 291 § 4.10 gains the message it never asserted.
--
-- ⚠ THE COMMENTS MOVED TOO, IN BOTH FUNCTIONS, AND THAT IS PART OF THE CHANGE.
--   `grant_role_impl`'s site-(b) note asserted "the 'staff' sub-arm above ADMITS
--   a platform admin" — false the moment this migration applies.
--   `revoke_role_impl` carried the narrowed QA m1 replacement note Amendment 1
--   required; Amendment 2 § Consequences 1 retires it outright, because a note
--   about an asymmetry that no longer exists is how the original m1 note misled.
--   397 § 0.5 pins both directions.
--
-- Both bodies below are the LIVE catalog text (`pg_get_functiondef` at head
-- 20261003006200), byte-for-byte apart from the one predicate and the three
-- comment blocks named above.

CREATE OR REPLACE FUNCTION app.grant_role_impl(p_actor uuid, p_scope_type text, p_scope_id uuid, p_role text, p_user uuid, p_title_id uuid DEFAULT NULL::uuid, p_expires_at timestamp with time zone DEFAULT NULL::timestamp with time zone, p_allow_anchorless boolean DEFAULT false)
 RETURNS void
 LANGUAGE plpgsql
 SECURITY DEFINER
...
begin
  ...
  -- └───────────────────────────────────────────────────────────────────────────┘
    ...
    if p_role = 'staff' then
      -- ⭐ ADR 0167 AMENDMENT 2, SITE (c) — PO-ruled 2026-08-28.
      --    `app.is_admin_for(p_actor)` was DROPPED here, closing the SECOND
      --    one-way door Amendment 1 found: `revoke_role_impl`'s 'staff' sub-arm
      --    has never carried one, so a platform admin could SEAT a commission
      --    `staff` and could not REMOVE one — the same
      --    escalation-without-de-escalation shape clause 1 closed for
      --    `staff_admin`, against the role clause 1 bounded itself out of.
      --
      -- ⭐ THE THIRD PARTICIPANT IS SYMMETRIC AND IS UNTOUCHED.
      --    `is_staff_admin_of_for` already appears on BOTH sides of this
      --    sub-arm, so dropping `is_admin_for` leaves grant and revoke reading
      --    the IDENTICAL predicate. That is why pgTAP 397 § 4's grant/revoke
      --    agreement property is now UNSCOPED across both commission sub-arms
      --    rather than bounded to `staff_admin`, and why 397 § 6 (which pinned
      --    the gap as a known one) was DELETED rather than repaired.
      --
      -- ⚠ IT MOVES THE PLATFORM ADMIN'S REFUSAL ONE STATEMENT EARLIER, which is
      --   a MEASUREMENT hazard rather than a behaviour change: a SQLSTATE-only
      --   assertion downstream keeps its 42501 while its subject silently
      --   changes. pgTAP 293 § 3.1 and 397 § 5.2 discriminate by MESSAGE for
      --   exactly that reason.
  ...
  if p_scope_type = 'commission' then
    ...
    if found and v_existing_role is distinct from p_role then
      ...
      --
      -- ⚠ WHO STILL REACHES THIS RAISE — RE-DERIVED AFTER AMENDMENT 2, BECAUSE
      --   THE OLD ANSWER WENT FALSE. This comment used to read "the 'staff'
      --   sub-arm above ADMITS a platform admin, and this guard is the only
      --   thing that stops it". Amendment 2 narrowed that sub-arm, so a plain
      --   platform admin is now refused EARLIER and never arrives here. The only
      --   class that reaches this line is
      --   `is_staff_admin_of_for AND NOT is_tenancy_admin_of_for` — a commission
      --   coordinator demoting a peer coordinator.
      --
      -- ⛔ AMENDMENT 2 THEREFORE COST THIS SITE ITS BEHAVIOURAL WITNESS, and the
      --   witness was RE-HOMED rather than dropped: pgTAP 397 § 5.3 now drives it
      --   with an actor who is BOTH `is_admin_for` and a commission
      --   `staff_admin` — the only actor for whom this line is the difference
      --   between admitted and refused, and therefore the only one that can red
      --   when it is reverted. 291 § 4.10 carries the plain-coordinator half.
      --   Both discriminate by MESSAGE; a SQLSTATE-only assertion cannot tell
      --   this raise from the 'staff' sub-arm's generic 'sem permissão'.
...
begin
  ...
  -- hospital with zero reviewers is a valid (deny-by-default) state.
    ...
    --    is gone.
    --
    -- ⭐ ADR 0167 AMENDMENT 2 (PO-ruled 2026-08-28) COMPLETED THE RETIREMENT.
    --    Amendment 1 found the same defect on the 'staff' sub-arm and required a
    --    NARROWED replacement note for it, because closing it was out of scope
    --    then. It is closed now, and closed the same way — by narrowing GRANT,
    --    never by widening this side — so both commission sub-arms read the
    --    IDENTICAL predicate and the replacement note has no subject either.
    --
    -- ⛔ NO NOTE ABOUT AN ASYMMETRY IS LEFT HERE, DELIBERATELY. Describing one
    --    that no longer exists is exactly how the original m1 note misled: a
    --    future reader "fixes" it in whichever direction they happen to prefer.
    --    pgTAP 397 § 4's agreement property is now UNSCOPED across both
    --    commission sub-arms, and 397 § 0.5 pins this comment in BOTH
    --    directions — the retired sentences absent, the ruling named.
    if p_role = 'staff' then
      if not (app.is_staff_admin_of_for(p_scope_id, p_actor)
              or app.is_tenancy_admin_of_for(p_scope_id, p_actor)) then
        raise exception 'sem permissão' using errcode = '42501';
