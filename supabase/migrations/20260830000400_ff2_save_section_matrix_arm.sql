-- FF-2 (ADR 0089) — the matrix arms of the save path.
--
-- DECISION: matrix answers EXTEND `save_section_answers` rather than landing as
-- sibling RPCs. Four reasons, in order of weight:
--   1. A matrix answer needs its parent `answers` row to exist (the child FK).
--      Splitting the writers splits that dependency across two transactions, so
--      a partial failure leaves a section half-saved.
--   2. The wizard's contract is "answers persist on EVERY navigation"
--      (Architecture Rule 3). A sibling RPC makes matrix answers persist on a
--      different beat from everything else in the same section.
--   3. FF-1 settled exactly this question for repeating groups — one payload,
--      one round trip (`p_instance_answers`) — and a second, contradictory
--      answer one phase later is the drift this program exists to prevent.
--   4. The instance dimension comes free: an instance entry gains the same two
--      keys, so a matrix inside a repeating group needs no third mechanism.
--
-- Both new params are OPTIONAL, so every existing caller is unaffected. The
-- function must be DROPped first: adding defaulted params to a plpgsql function
-- creates an OVERLOAD, not a replacement, and two candidates with the same name
-- make the PostgREST call ambiguous.

drop function if exists public.save_section_answers(uuid, uuid, jsonb, uuid[], jsonb, jsonb, jsonb, jsonb);
...
create or replace function public.save_section_answers(
  ...
  p_matrix_cells jsonb default null::jsonb,
  p_risk_matrix jsonb default null::jsonb
)
...
revoke all on function public.save_section_answers(uuid, uuid, jsonb, uuid[], jsonb, jsonb, jsonb, jsonb, jsonb, jsonb) from public;
grant execute on function public.save_section_answers(uuid, uuid, jsonb, uuid[], jsonb, jsonb, jsonb, jsonb, jsonb, jsonb) to authenticated;

-- ---------------------------------------------------------------------------
-- The INSTANCE arm gains the same two keys. Signature is unchanged (the entry
-- is a jsonb bag), so CREATE OR REPLACE suffices.
--
-- Placed BEFORE the per-instance `clear_item_ids` block for the same reason the
-- other arms are: a clear in the same entry must win over a write in that entry.
-- ---------------------------------------------------------------------------
...
begin
  ...
  -- The two FF-2 matrix maps are NOT folded in here: their helpers apply a
  -- STRICTER check of their own (version membership AND item_type), and
  -- weakening it to this one would let a `short_text` id through to a matrix
  -- writer.
  select (e.key)::uuid into v_bad_item
  from (
