...
create temp table tpl on commit drop as
  select (public.create_process_template((select comm_x from k), 'Anexos', null)).id as tid;
reset role;
...
values ((select iv from ivx), (select comm_x from k), (select cid from cse), 0, 'rascunho');
