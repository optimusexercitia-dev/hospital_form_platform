...
-- plan(67) = 59 + S3p:9 (ADR 0121 D11 inflow, 2026-08-17) = 3 preconditions + 5 coupling + 6 mint + 4 separation + 7 inactive-print
...
-- =============================================================================
-- S3p — ADR 0121 D3/D5, the D11 INFLOW: superseding a print marks its BYTES.
-- Migration 20260928000300. Closes FUP-DM5-D11-SUPERSEDED-NEVER-RETIRES.
--
-- ⭐ THIS BLOCK EXISTS BECAUSE THE STATE DID NOT. Before it, no test in this repo
-- had ever superseded a print — `mint_printed_document` flipped the REGISTRY row
-- to 'superseded' and left the PDF at disposal_state='none' forever, and nothing
-- could see it because nothing had ever built a second print of one source.
--
-- ⚠ ENTIRELY FRESH FIXTURES, and that is not tidiness. The first draft reused
-- `pd1`, and its PRE-CONTROL failed immediately: an earlier block already runs
-- `request_document_disposition(..., 'retention_expired')` over that print, so
-- pd1 arrives here ALREADY `disposal_pending`. Reusing it would have asserted
-- `disposal_pending` against a value another block wrote — green, and measuring
-- nothing of this migration. The pre-control is what caught it; without one this
-- block would have shipped as a vacuous pass.
--
-- Short codes obey the door's own alphabet `^[A-HJ-NP-Z2-9]{10}$` (no I/O/0/1),
-- read from the live body after a first draft used '0' and died HC0D1.
-- =============================================================================
create temp table sp on commit drop as
  select '00000000-0000-0000-0000-00000000f110'::uuid as resp_p,
         '00000000-0000-0000-0000-00000000f111'::uuid as resp_h,
         '00000000-0000-0000-0000-00000000f210'::uuid as pda,
         '00000000-0000-0000-0000-00000000f211'::uuid as pdb,
         '00000000-0000-0000-0000-00000000f212'::uuid as pdc,
         '00000000-0000-0000-0000-00000000f213'::uuid as pdd,
         'S3TOKENPAAAABBBBCCCCDDDDEEEEFFFFGGGG'::text  as toka,
         'S3TOKENQAAAABBBBCCCCDDDDEEEEFFFFGGGG'::text  as tokb,
         'S3TOKENRAAAABBBBCCCCDDDDEEEEFFFFGGGG'::text  as tokc,
         'S3TOKENSAAAABBBBCCCCDDDDEEEEFFFFGGGG'::text  as tokd,
         'RSTUVW6789'::text as sca,
         'RSTUVW2346'::text as scb,
         'RSTUVW2347'::text as scc,
         'RSTUVW2348'::text as scd;
grant select on sp to authenticated;

insert into public.responses (id, form_version_id, commission_id, created_by, status, started_at, submitted_at)
select sp.resp_p, k.ver_u, k.comm_x, k.st_x, 'submitted', now(), now() from sp, k;
insert into public.responses (id, form_version_id, commission_id, created_by, status, started_at, submitted_at)
select sp.resp_h, k.ver_u, k.comm_x, k.st_x, 'submitted', now(), now() from sp, k;

insert into storage.objects (bucket_id, name, metadata)
select 'documents-standard', app.printed_rendition_storage_path(v.x),
       jsonb_build_object('size', 4096, 'mimetype', 'application/pdf')
  from sp, lateral (values (sp.pda), (sp.pdb), (sp.pdc), (sp.pdd)) v(x);

select test_helpers.claims_for((select st_x from k), false);
set local role authenticated;
select lives_ok(
  $q$select public.mint_printed_document(
      (select pda from sp), 'form_response', (select resp_p from sp),
      'form_response', 1, repeat('ab', 32),
      (select toka from sp), (select sca from sp), false)$q$,
  'DM5·S3p1 [FIXTURE] a first print exists on a response no other block touches');
reset role;

-- PRE-CONTROL. Without it the 'disposal_pending' below could have been true all
-- along and the assertion would prove nothing about the re-mint.
select is(
  (select f.disposal_state
     from public.printed_documents pd
     join public.document_version_files vf on vf.document_version_id = pd.document_version_id
     join public.file_objects f on f.id = vf.file_object_id
    where pd.id = (select pda from sp)),
  'none',
  'DM5·S3p2 [PRE-CONTROL] that print''s bytes are NOT marked before the re-mint');

select test_helpers.claims_for((select st_x from k), false);
set local role authenticated;
select lives_ok(
  $q$select public.mint_printed_document(
      (select pdb from sp), 'form_response', (select resp_p from sp),
      'form_response', 1, repeat('ab', 32),
      (select tokb from sp), (select scb from sp), false)$q$,
  'DM5·S3p3 the RE-MINT of the same source+template succeeds (this is what supersedes pda)');
reset role;

select is(
  (select status from public.printed_documents where id = (select pda from sp)),
  'superseded',
  'DM5·S3p4 the registry half still holds: the old print is marked superseded');

select is(
  (select f.disposal_state || '/' || coalesce(f.disposal_reason_category, 'NULL')
     from public.printed_documents pd
     join public.document_version_files vf on vf.document_version_id = pd.document_version_id
     join public.file_objects f on f.id = vf.file_object_id
    where pd.id = (select pda from sp)),
  'disposal_pending/superseded',
  'DM5·S3p5 ⭐⭐ THE INFLOW: the superseded print''s BYTES are queued for disposal with the D3 reason — before 20260928000300 this stayed none/NULL forever');
...
select is(
  (select f.disposal_state
     ...
     join public.document_version_files vf on vf.document_version_id = pd.document_version_id
     join public.file_objects f on f.id = vf.file_object_id
    where pd.id = (select pdb from sp)),
  'none',
  'DM5·S3p6 [CONTROL] the NEW print''s own bytes are untouched — the marking targets the superseded row, not every file the mint touched (excludes a blanket UPDATE)');

-- ---------------------------------------------------------------------------
-- ⭐ THE LEGAL-HOLD ARM — the interaction that would otherwise be an OUTAGE.
-- `app.guard_file_object_transition` RAISES on none -> disposal_pending while an
-- unreleased `document_legal_holds` row covers the object. Inside the mint that
-- exception would abort the whole transaction, so **placing a legal hold on an
-- old print would make it impossible to issue a new one**. The inflow therefore
-- excludes held objects in its own WHERE clause. Both halves are asserted: the
-- mint must SUCCEED, and the held bytes must STAY retained.
-- ---------------------------------------------------------------------------
select test_helpers.claims_for((select st_x from k), false);
set local role authenticated;
select lives_ok(
  $q$select public.mint_printed_document(
      (select pdc from sp), 'form_response', (select resp_h from sp),
      'form_response', 1, repeat('ab', 32),
      (select tokc from sp), (select scc from sp), false)$q$,
  'DM5·S3p7 [FIXTURE] a first print exists on the hold response');
reset role;

insert into public.document_legal_holds (document_id, issued_by, reason_category)
select pd.document_id, k.sa_x, 'litigation'
  from public.printed_documents pd, k where pd.id = (select pdc from sp);

select test_helpers.claims_for((select st_x from k), false);
set local role authenticated;
select lives_ok(
  $q$select public.mint_printed_document(
      (select pdd from sp), 'form_response', (select resp_h from sp),
      'form_response', 1, repeat('ab', 32),
      (select tokd from sp), (select scd from sp), false)$q$,
  'DM5·S3p8 ⭐⭐ the mint SUCCEEDS even though the print it supersedes is under LEGAL HOLD — without the hold exclusion the trigger would raise and a hold would block issuing any new document');
reset role;
...
select is(
  (select f.disposal_state
     ...
     join public.document_version_files vf on vf.document_version_id = pd.document_version_id
     join public.file_objects f on f.id = vf.file_object_id
    where pd.id = (select pdc from sp)),
  'none',
  'DM5·S3p9 ⭐ ...and the HELD bytes stay retained rather than queued — a hold means keep, and the inflow marks around it instead of through it');
