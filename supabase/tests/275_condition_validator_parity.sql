...
from jsonb_array_elements(
  ...
  "_comment": [
    ...
    "SQL app.is_valid_condition  <->  TS isValidCondition (src/lib/forms/condition-shape.ts;",
    "`actions.ts` only IMPORTS it — the predicate was extracted there because a",
    "'use server' module may export only async functions, which is why it was untestable).",
    "",
    ...
    "",
    "Run by Vitest (src/lib/forms/condition-shape.test.ts) and by pgTAP",
    ...
    "justified, so that this file can never quietly absorb a divergence. There are",
    "currently THREE, all in the safe direction (TS accepts a strict SUBSET of SQL):",
    "a blank question_key, `in` with a non-array value, and an ordered op with an",
    "array value. Each is backstopped by an independent publish-time rejection in",
    "app.assert_condition_op_target, so refusing early costs the author nothing and",
    "buys an authoring-time message instead of a publish-time dead end."
  ],
  "vectors": [
