-- ADR 0150 (amends 0146) — `app.audit_write` derives the organization from the hospital,
-- and leg 5 of `audit_log_select` expresses the PLATFORM CHAIN exactly.
-- Migration 20261003003100.
--
...
-- callers in `pg_proc`):
--
--   org_admin of that hospital's org  sees 0   ← ADR 0149's superset Consequence, falsified
...
-- platform_admin reads tenant audit CONTENT. That is precisely what test 372 §5.2 asserts
-- cannot happen; 372 passes only because its fixture guarantees a non-null org (372 §0.1
...
--
-- ⛔ FIXTURE DISCIPLINE (372's, kept): every id is FIXED and self-contained (the
...
-- ===========================================================================
-- §1 ⭐ THE WRITE-SIDE CLASS FIX. 1.1 is RED before migration 20261003003100.
-- ===========================================================================
select is(
...
select is(
  ...
  '2.1 ⭐ KEYSTONE: O''s org_admin READS the trigger-shaped row. This is ADR 0149''s "an org_admin''s audit reach is a superset of each of its hospital admins''" — which was FALSE for this action class before the fix, because the widened leg 4 keys on `organization_id` and the row had none');
select test_helpers.reset_role_and_claims();
...
select is(
  ...
  '2.3 ⭐ KEYSTONE (the half nobody flagged): a platform_admin reads ZERO of this TENANT hospital-tier row. Before the fix its NULL `organization_id` satisfied leg 5''s `organization_id IS NULL` and a platform admin read tenant CONTENT — the noun rule (CLAUDE.md §1, ADR 0078 A35) that test 372 §5.2 believes it pins');
select test_helpers.reset_role_and_claims();
...
select ok(
  ...
  '5.1 ⭐ leg 5 carries its FULL platform-chain shape — all three scope keys NULL. Asserted as the whole leg rather than as a bare `hospital_id IS NULL` substring, which would also be satisfied by the conjunct sitting on the WRONG leg (that is exactly the defect ADR 0149 removed from leg 4)');

select ok(
  (select qual from pg_policies where tablename = 'audit_log' and policyname = 'audit_log_select')
    like '%((commission_id IS NULL) AND app.is_org_admin_of(organization_id))%',
  '5.2 ⭐ ... while leg 4 still carries NO hospital conjunct. Same leg-scoped form, opposite direction: ADR 0149 removed it from leg 4 and ADR 0150 added it to leg 5, and the two legs must not be confused for one another');
